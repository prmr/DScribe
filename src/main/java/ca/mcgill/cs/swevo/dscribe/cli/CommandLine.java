...
import ca.mcgill.cs.swevo.dscribe.cli.CommandLine.Help.Ansi.IStyle;
import ca.mcgill.cs.swevo.dscribe.cli.CommandLine.Help.Ansi.Style;
import ca.mcgill.cs.swevo.dscribe.cli.CommandLine.Help.Ansi.Text;
import ca.mcgill.cs.swevo.dscribe.cli.CommandLine.Model.*;
import ca.mcgill.cs.swevo.dscribe.cli.CommandLine.ParseResult.GroupMatchContainer;

import static ca.mcgill.cs.swevo.dscribe.cli.CommandLine.Help.Column.Overflow.SPAN;
import static ca.mcgill.cs.swevo.dscribe.cli.CommandLine.Help.Column.Overflow.TRUNCATE;
import static ca.mcgill.cs.swevo.dscribe.cli.CommandLine.Help.Column.Overflow.WRAP;
 ...
 * An example that implements {@code Callable} and uses the {@link #execute(String...) CommandLine.execute} convenience
 * API to run in a single line of code:
 * </p>
 * 
 * <pre>
 ...
 * </pre>
 * <p>
 * Another example where the application calls {@code parseArgs} and takes responsibility for error handling and
 * checking whether the user requested help:
 * </p>
 * 
 * <pre>
 * import static codit.CommandLine.*;
 *
...
public class CommandLine
{
	...
	/**
	 * Constructs a new {@code CommandLine} interpreter with the specified object (which may be an annotated user object
	 * or a {@link CommandSpec CommandSpec}) and a default {@linkplain IFactory factory}.
	 * <p>
	 ...
	 * {@code @Parameters}-annotated methods, picocli creates a {@link java.lang.reflect.Proxy Proxy} whose methods
	 * return the matched command line values. If the specified command object is a concrete {@code Class}, picocli
	 * delegates to the default factory to get an instance.
	 * </p>
	 * <p>
	 * If the specified object implements {@code Runnable} or {@code Callable}, or if it is a {@code Method} object, the
	 * command can be run as an application in a <a href="#checksum_example">single line of code</a> by using the
	 * {@link #execute(String...) execute} method to omit some boilerplate code for handling help requests and invalid
	 * input. See {@link #getCommandMethods(Class, String) getCommandMethods} for a convenient way to obtain a command
	 * {@code Method}.
	 * </p>
	 * <p>
	 ...
	 * 
	 * @param command
	 *            an annotated user object or a {@code CommandSpec} object to initialize from the command line arguments
	 * @throws InitializationException
	 *             if the specified command object does not have a {@link Command}, {@link Option} or {@link Parameters}
	 *             annotation
	 */
	...
	/**
	 * Constructs a new {@code CommandLine} interpreter with the specified object (which may be an annotated user object
	 * or a {@link CommandSpec CommandSpec}) and object factory.
	 * <p>
	 * The specified object may be a {@link CommandSpec CommandSpec} object, or it may be a {@code @Command}-annotated
	 * user object with {@code @Option} and {@code @Parameters}-annotated fields, in which case picocli automatically
	 * constructs a {@code CommandSpec} from this user object.
	 ...
	 * {@code @Parameters}-annotated methods, picocli creates a {@link java.lang.reflect.Proxy Proxy} whose methods
	 * return the matched command line values. If the specified command object is a concrete {@code Class}, picocli
	 * delegates to the {@linkplain IFactory factory} to get an instance.
	 * </p>
	 * <p>
	 * If the specified object implements {@code Runnable} or {@code Callable}, or if it is a {@code Method} object, the
	 * command can be run as an application in a <a href="#checksum_example">single line of code</a> by using the
	 ...
	 *            an annotated user object or a {@code CommandSpec} object to initialize from the command line arguments
	 * @param factory
	 *            the factory used to create instances of {@linkplain Command#subcommands() subcommands},
	 *            {@linkplain Option#converter() converters}, etc., that are registered declaratively with annotation
	 *            attributes
	 * @throws InitializationException
	 *             if the specified command object does not have a {@link Command}, {@link Option} or {@link Parameters}
	 *             annotation
	...
	/**
	 * Returns the {@code CommandSpec} config that this {@code CommandLine} was constructed with.
	 * 
	 * @return the {@code CommandSpec} config
	 * @since 3.0
	 */
	 ...
	 * {@code @Option} and {@code @Parameters}-annotated fields, in which case picocli automatically constructs a
	 * {@code CommandSpec} from this user object.
	 * </p>
	 * 
	 * @param name
	 *            the name by which the mixin object may later be retrieved
	 * @param mixin
	 *            an annotated user object or a {@link CommandSpec CommandSpec} object whose options and positional
	 *            parameters to add to this command
	...
	/**
	 * Returns a map of user objects whose options and positional parameters were added to ("mixed in" with) this
	 * command.
	 * 
	 * @return a new Map containing the user objects mixed in with this command. If {@code CommandSpec} objects without
	 *         user objects were programmatically added, use the {@link CommandSpec#mixins() underlying config}
	 *         directly.
	 * @since 3.0
	 */
	...
	/**
	 * Registers a subcommand with the name obtained from the {@code @Command(name = "...")} {@linkplain Command#name()
	 * annotation attribute} of the specified command.
	 * 
	 * @param command
	 *            the object to initialize with command line arguments following the subcommand name. This may be a
	 *            {@code Class} that has a {@code @Command} annotation, or an instance of such a class, or a
	 *            {@code ComandSpec} or {@code CommandLine} instance with its own (nested) subcommands.
	 * @return this CommandLine object, to allow method chaining
	 * @since 4.0
	 * @throws InitializationException
	 *             if no name could be found for the specified subcommand, or if another subcommand was already
	 *             registered under the same name, or if one of the aliases of the specified subcommand was already used
	 ...
	 *
	 * <p>
	 * The specified object can be an annotated object or a {@code CommandLine} instance with its own nested
	 * subcommands. For example:
	 * </p>
	 * 
	 * <pre>
	 * CommandLine commandLine = new CommandLine(new MainCommand()).addSubcommand("cmd1", new ChildCommand1()) // subcommand
	 * 		.addSubcommand("cmd2", new ChildCommand2()).addSubcommand("cmd3", new CommandLine(new ChildCommand3()) // subcommand
	 * 																												// with
	 * 																												// nested
	 * 																												// sub-subcommands
	 ...
	 * To ensure a custom type converter is available to all subcommands, register the type converter last, after adding
	 * subcommands.
	 * </p>
	 * <p>
	 ...
	 *
	 * @param name
	 *            the string to recognize on the command line as a subcommand. If {@code null}, the
	 *            {@linkplain CommandSpec#name() name} of the specified subcommand is used; if this is also
	 *            {@code null}, the first {@linkplain CommandSpec#aliases() alias} is used.
	 * @param command
	 *            the object to initialize with command line arguments following the subcommand name. This may be a
	 *            {@code Class} that has a {@code @Command} annotation, or an instance of such a class, or a
	 *            {@code ComandSpec} or {@code CommandLine} instance with its own (nested) subcommands.
	 ...
	 * @throws InitializationException
	 *             if the specified name is {@code null}, and no alternative name could be found, or if another
	 *             subcommand was already registered under the same name, or if one of the aliases of the specified
	 ...
	 *            {@code ComandSpec} or {@code CommandLine} instance with its own (nested) subcommands.
	 * @param aliases
	 *            zero or more alias names that are also recognized on the command line as this subcommand
	 * @return this CommandLine object, to allow method chaining
	 * @since 3.1
	 * @see #addSubcommand(String, Object)
	 * @throws InitializationException
	...
	{
		return interpreter.parseResultBuilder != null && interpreter.parseResultBuilder.usageHelpRequested;
	}
	...
	{
		return interpreter.parseResultBuilder != null && interpreter.parseResultBuilder.versionHelpRequested;
	}
	 ...
	 * <p>
	 * The specified setting will be registered with this {@code CommandLine} and the full hierarchy of its subcommands
	 * and nested sub-subcommands <em>at the moment this method is called</em>. Subcommands added later will have the
	 * default setting. To ensure a setting is applied to all subcommands, call the setter last, after adding
	 * subcommands.
	 * </p>
	 * 
	 * @param helpFactory
	 *            the new help factory. Must be non-{@code null}.
	...
	/**
	 * Returns the section keys in the order that the usage help message should render the sections. This ordering may
	 * be modified with {@link #setHelpSectionKeys(List) setSectionKeys}. The default keys are (in order):
	 * <ol start="0">
	...
	/**
	 * Returns the map of section keys and renderers used to construct the usage help message. The usage help message
	 * can be customized by adding, replacing and removing section renderers from this map. Sections can be reordered
	 * with {@link #setHelpSectionKeys(List) setSectionKeys}. Sections that are either not in this map or not in the
	 * list returned by {@link #getHelpSectionKeys() getSectionKeys} are omitted.
	 * <p>
	 * NOTE: By modifying the returned {@code Map}, only the usage help message <em>of this command</em> is affected.
	 * Use {@link #setHelpSectionMap(Map)} to customize the usage help message for this command <em>and all
	 * subcommands</em>.
	 * </p>
	 * 
	 * @since 3.9
	...
	/**
	 * Returns whether line breaks should take wide Chinese, Japanese and Korean characters into account for
	 * line-breaking purposes. The default is {@code true}.
	 * 
	 * @return true if wide Chinese, Japanese and Korean characters are counted as double the size of other characters
	...
	/**
	 * Sets whether line breaks should take wide Chinese, Japanese and Korean characters into account, and returns this
	 * UsageMessageSpec. The default is {@code true}.
	 * <p>
	 * The specified setting will be registered with this {@code CommandLine} and the full hierarchy of its subcommands
	 * and nested sub-subcommands <em>at the moment this method is called</em>. Subcommands added later will have the
	 * default setting. To ensure a setting is applied to all subcommands, call the setter last, after adding
	 ...
	 * </p>
	 * 
	 * @param adjustForWideChars
	 *            if true, wide Chinese, Japanese and Korean characters are counted as double the size of other
	...
	/**
	 * Returns whether the value of boolean flag options should be "toggled" when the option is matched. From 4.0, this
	 * is {@code false} by default, and when a flag option is specified on the command line picocli will set its value
	 * to the opposite of its default value. If this method returns {@code true}, flags are toggled, so if the value is
	 * {@code true} it is set to {@code false}, and when the value is {@code false} it is set to {@code true}. When
	 * toggling is enabled, specifying a flag option twice on the command line will have no effect because they cancel
	 * each other out.
	 * 
	 * @return {@code true} the value of boolean flag options should be "toggled" when the option is matched,
	...
	/**
	 * Sets whether the value of boolean flag options should be "toggled" when the option is matched. The default is
	 * {@code false}, and when a flag option is specified on the command line picocli will set its value to the opposite
	...
	/**
	 * Returns whether options for single-value fields can be specified multiple times on the command line. The default
	 * is {@code false} and a {@link OverwrittenOptionException} is thrown if this happens. When {@code true}, the last
	 * specified value is retained.
	 * 
	 * @return {@code true} if options for single-value fields can be specified multiple times on the command line,
	 *         {@code false} otherwise
	 * @since 0.9.7
	 */
	 ...
	 * {@link OverwrittenOptionException} being thrown. The default is {@code false}.
	 * <p>
	 * The specified setting will be registered with this {@code CommandLine} and the full hierarchy of its subcommands
	 * and nested sub-subcommands <em>at the moment this method is called</em>. Subcommands added later will have the
	 * default setting. To ensure a setting is applied to all subcommands, call the setter last, after adding
	 ...
	 * 
	 * @return {@code true} if short options like {@code -x -v -f SomeFile} can be clustered together like
	 *         {@code -xvfSomeFile}, {@code false} otherwise
	 * @since 3.0
	 */
	...
	/**
	 * Returns whether the parser should ignore case when converting arguments to {@code enum} values. The default is
	 * {@code false}.
	 * 
	 * @return {@code true} if enum values can be specified that don't match the {@code toString()} value of the enum
	 *         constant, {@code false} otherwise; e.g., for an option of type
	...
	/**
	 * Sets whether the parser should ignore case when converting arguments to {@code enum} values. The default is
	 * {@code false}. When set to true, for example, for an option of type
	 * <a href="https://docs.oracle.com/javase/8/docs/api/java/time/DayOfWeek.html">java.time.DayOfWeek</a>, values
	 * {@code MonDaY}, {@code monday} and {@code MONDAY} are all recognized if {@code true}.
	 * <p>
	 * The specified setting will be registered with this {@code CommandLine} and the full hierarchy of its subcommands
	 * and nested sub-subcommands <em>at the moment this method is called</em>. Subcommands added later will have the
	 * default setting. To ensure a setting is applied to all subcommands, call the setter last, after adding
	...
	/**
	 * Returns whether the parser should trim quotes from command line arguments. The default is read from the system
	 * property "picocli.trimQuotes" and will be {@code true} if the property is present and empty, or if its value is
	 * "true".
	 * <p>
	 * If this property is set to {@code true}, the parser will remove quotes from the command line arguments, as
	 * follows:
	 * <ul>
	 ...
	 * tries to process the parameter with the quotes intact. For example, the {@code split} regular expression inside a
	 * quoted region should be ignored, so arguments like {@code "a,b","x,y"} are handled correctly. For arguments with
	 * nested quotes, quotes are removed later in the processing pipeline, after {@code split} operations are
	 * applied.</li>
	 * </ul>
	 * </p>
	 * 
	 * @return {@code true} if the parser should trim quotes from command line arguments before processing them,
	 *         {@code false} otherwise;
	...
	/**
	 * Returns whether the parser is allowed to split quoted Strings or not. The default is {@code false}, so quotes are
	 * respected: quoted strings are treated as a single value that should not be broken up.
	 * <p>
	 * For example, take a single command line parameter {@code "a,b","x,y"}. With a comma split regex, the default of
	 * {@code splitQuotedStrings = false} means that this value will be split into two strings: {@code "a,b"} and
	 * {@code "x,y"}. This is usually what you want.
	 * </p>
	 * <p>
	 * If {@code splitQuotedStrings} is set to {@code true}, quotes are not respected, and the value is split up into
	 * four parts: the first is {@code "a}, the second is {@code b"}, the third is {@code "x}, and the last part is
	 * {@code y"}. This is generally not what you want.
	 * </p>
	 * 
	 * @deprecated Most applications should not change the default. The rare application that <em>does</em> need to
	 *             split parameter values without respecting quotes should use
	 *             {@link ParserSpec#splitQuotedStrings(boolean)}.
	...
	/**
	 * Sets whether the parser is allowed to split quoted Strings. The default is {@code false}, so quotes are
	 * respected: quoted strings are treated as a single value that should not be broken up.
	 * <p>
	 * For example, take a single command line parameter {@code "a,b","x,y"}. With a comma split regex, the default of
	 * {@code splitQuotedStrings = false} means that this value will be split into two strings: {@code "a,b"} and
	 ...
	 * </p>
	 * <p>
	 * However, if {@code splitQuotedStrings} is set to {@code true}, quotes are not respected, and the value is split
	 * up into four parts: the first is {@code "a}, the second is {@code b"}, the third is {@code "x}, and the last part
	 * is {@code y"}. This is generally not what you want.
	 * </p>
	 * <p>
	 * The specified setting will be registered with this {@code CommandLine} and the full hierarchy of its subcommands
	 * and nested sub-subcommands <em>at the moment this method is called</em>. Subcommands added later will have the
	...
	/**
	 * Returns the end-of-options delimiter that signals that the remaining command line arguments should be treated as
	...
	/**
	 * Sets the end-of-options delimiter that signals that the remaining command line arguments should be treated as
	 * positional parameters.
	 * 
	 * @param delimiter
	 *            the end-of-options delimiter; must not be {@code null}. The default is {@code "--"}.
	 * @return this {@code CommandLine} object, to allow method chaining
	 * @since 3.5
	 */
	...
	/**
	 * Sets a default value provider for the command and sub-cli
	 * <p>
	 * The specified setting will be registered with this {@code CommandLine} and the full hierarchy of its sub-cli and
	 * nested sub-subcommands <em>at the moment this method is called</em>. Sub-cli added later will have the default
	 * setting. To ensure a setting is applied to all sub-cli, call the setter last, after adding sub-cli.
	 * </p>
	 * 
	 * @param newValue
	 ...
	 * 
	 * @return {@code true} if all values following the first positional parameter should be treated as positional
	...
	/**
	 * Sets whether the parser interprets the first positional parameter as "end of options" so the remaining arguments
	 * are all treated as positional parameters. The default is {@code false}.
	 * <p>
	 * The specified setting will be registered with this {@code CommandLine} and the full hierarchy of its subcommands
	 * and nested sub-subcommands <em>at the moment this method is called</em>. Subcommands added later will have the
	 * default setting. To ensure a setting is applied to all subcommands, call the setter last, after adding
	 ...
	 * 
	 * @param newValue
	 *            {@code true} if all values following the first positional parameter should be treated as positional
	 *            parameters, {@code false} otherwise
	 * @return this {@code CommandLine} object, to allow method chaining
	 * @since 2.3
	 */
	 ...
	 * unmatched option. Unmatched options are arguments that look like an option but are not one of the known options,
	 * or positional arguments for which there is no available slots (the command has no positional parameters or their
	 * size is limited). The default is {@code false}.
	 * <p>
	 * Setting this flag to {@code true} automatically sets the {@linkplain #isUnmatchedArgumentsAllowed()
	 * unmatchedArgumentsAllowed} flag to {@code true} also.
	 * </p>
	 * 
	 * @return {@code true} when an unmatched option should result in the remaining command line arguments to be added
	 *         to the {@linkplain #getUnmatchedArguments() unmatchedArguments list}
	 * @since 2.3
	 */
	 ...
	 * size is limited). The default is {@code false}.
	 * <p>
	 * Setting this flag to {@code true} automatically sets the {@linkplain #setUnmatchedArgumentsAllowed(boolean)
	 * unmatchedArgumentsAllowed} flag to {@code true} also.
	 * </p>
	 * <p>
	 * The specified setting will be registered with this {@code CommandLine} and the full hierarchy of its subcommands
	 ...
	 * 
	 * @param newValue
	 *            {@code true} when an unmatched option should result in the remaining command line arguments to be
	 *            added to the {@linkplain #getUnmatchedArguments() unmatchedArguments list}
	 * @return this {@code CommandLine} object, to allow method chaining
	 * @since 2.3
	 */
	 ...
	 * 
	 * @return {@code true} arguments on the command line that resemble an option should be treated as positional
	 *         parameters, {@code false} otherwise
	 ...
	 * 
	 * @param newValue
	 *            the new setting. When {@code true}, arguments on the command line that resemble an option should be
	...
	/**
	 * Returns whether the end user may specify arguments on the command line that are not matched to any option or
	 * parameter fields. The default is {@code false} and a {@link UnmatchedArgumentException} is thrown if this
	 * happens. When {@code true}, the last unmatched arguments are available via the {@link #getUnmatchedArguments()}
	 * method.
	 * 
	 ...
	 * 
	 * @param newValue
	 *            the new setting. When {@code true}, the last unmatched arguments are available via the
	...
	{
		return interpreter.parseResultBuilder == null ? Collections.<String> emptyList()
				: UnmatchedArgumentException.stripErrorMessage(interpreter.parseResultBuilder.unmatched);
	}

	/**
	 * Defines some exit codes used by picocli as default return values from the {@link #execute(String...) execute} and
	 * {@link #executeHelpRequest(ParseResult) executeHelpRequest} methods.
	 * <p>
	 * Commands can override these defaults with annotations (e.g.
	 * {@code @Command(exitCodeOnInvalidInput = 64, exitCodeOnExecutionException = 70)} or programmatically (e.g.
	 * {@link CommandSpec#exitCodeOnInvalidInput(int)}).
	 * </p>
	 * <p>
	 * Additionally, there are several mechanisms for cli to return custom exit codes. See the javadoc of the
	 * {@link #execute(String...) execute} method for details.
	 * </p>
	 ...
	 * standard</a>. The specific set of codes returned is unique to the program that sets it. Typically an exit code of
	 * zero indicates success, any non-zero exit code indicates failure. For reference, here are a few conventions:
	 * </p>
	 * <ul>
	 ...
	 * <h3>Valid Ranges</h3>
	 * <p>
	 * Note that *nix shells may restrict exit codes to the 0-255 range, DOS seems to allow larger numbers. See this
	 * <a href="https://stackoverflow.com/questions/179565/exitcodes-bigger-than-255-possible">StackOverflow
	 * question</a>.
	 * </p>
	 * 
	 * @since 4.0
	...
	{
		/**
		 * Return value from the {@link #execute(String...) execute} and {@link #executeHelpRequest(ParseResult)
		...
		/**
		 * Return value from the {@link #execute(String...) execute} method signifying internal software error: an
		 * exception occurred when invoking the Runnable, Callable or Method user object of a command.
		 * <p>
		 * The value of this constant is {@value}.
		 * </p>
		 */
		...
		/**
		 * Return value from the {@link #execute(String...) execute} method signifying command line usage error: user
		 * input for the command was incorrect, e.g., the wrong number of arguments, a bad flag, a bad syntax in a
	...
	/**
	 * Interface that provides the appropriate exit code that will be returned from the {@link #execute(String...)
	 * execute} method for an exception that occurred during parsing or while invoking the command's Runnable, Callable,
	 * or Method.
	 * <p>
	 * Example usage:
	 * </p>
	...
	public interface IExitCodeExceptionMapper
	{
		 ...
		 * 
		 * @param exception
		 *            the exception that occurred during parsing or while invoking the command's Runnable, Callable, or
		 *            Method.
		 * @return the exit code
		 */
	...
	/**
	 * Returns the color scheme to use when printing help. The default value is the
	 * {@linkplain CommandLine.Help#defaultColorScheme(CommandLine.Help.Ansi) default color scheme} with
	 * {@link Help.Ansi#AUTO Ansi.AUTO}.
	 * 
	 * @see #execute(String...)
	...
	/**
	 * Returns the writer used when printing user-requested usage help or version help during command
	 * {@linkplain #execute(String...) execution}. Defaults to a PrintWriter wrapper around {@code System.out} unless
	 * {@link #setOut(PrintWriter)} was called with a different writer.
	 * <p>
	 * This method is used by {@link #execute(String...)}. Custom {@link IExecutionStrategy IExecutionStrategy}
	 * implementations should also use this writer.
	 * </p>
	 * <p>
	 * By <a href="http://www.gnu.org/prep/standards/html_node/_002d_002dhelp.html">convention</a>, when the user
	 * requests help with a {@code --help} or similar option, the usage help message is printed to the standard output
	 * stream so that it can be easily searched and paged.
	 * </p>
	 * 
	 * @since 4.0
	 */
	...
	/**
	 * Sets the writer to use when printing user-requested usage help or version help during command
	 * {@linkplain #execute(String...) execution}.
	 * <p>
	 * This method is used by {@link #execute(String...)}. Custom {@link IExecutionStrategy IExecutionStrategy}
	 * implementations should also use this writer.
	 * </p>
	...
	/**
	 * Returns the writer to use when printing diagnostic (error) messages during command
	 * {@linkplain #execute(String...) execution}. Defaults to a PrintWriter wrapper around {@code System.err}, unless
	 * {@link #setErr(PrintWriter)} was called with a different writer.
	 * <p>
	 * This method is used by {@link #execute(String...)}. {@link IParameterExceptionHandler IParameterExceptionHandler}
	 * and {@link IExecutionExceptionHandler IExecutionExceptionHandler} implementations should use this writer to print
	 * error messages (which may include a usage help message) when an unexpected error occurs.
	 * </p>
	 * 
	 * @since 4.0
	 */
	...
	{
		return err != null ? err : new PrintWriter(System.err, true);
	}

	/**
	 * Sets the writer to use when printing diagnostic (error) messages during command {@linkplain #execute(String...)
	 * execution}.
	 * <p>
	 * This method is used by {@link #execute(String...)}. {@link IParameterExceptionHandler IParameterExceptionHandler}
	 * and {@link IExecutionExceptionHandler IExecutionExceptionHandler} implementations should use this writer to print
	 ...
	 * 
	 * @return the mapper that was {@linkplain #setExitCodeExceptionMapper(IExitCodeExceptionMapper) set}, or
	...
	/**
	 * Returns the execution strategy used by the {@link #execute(String...) execute} method to invoke the business
	 * logic on the user objects of this command and/or the user-specified subcommand(s). The default value is
	 * {@link RunLast RunLast}.
	 * 
	...
	/**
	 * Sets the execution strategy that the {@link #execute(String...) execute} method should use to invoke the business
	 * logic on the user objects of this command and/or the user-specified subcommand(s).
	 * <p>
	 * The specified setting will be registered with this {@code CommandLine} and the full hierarchy of its subcommands
	 * and nested sub-subcommands <em>at the moment this method is called</em>. Subcommands added later will have the
	 * default setting. To ensure a setting is applied to all subcommands, call the setter last, after adding
	 ...
	 *            the new execution strategy to run the user-specified command
	 * @return this CommandLine for method chaining
	 * @since 4.0
	 */
	...
	/**
	 * Returns the handler for dealing with invalid user input when the command is {@linkplain #execute(String...)
	 * executed}.
	 * <p>
	 * The default implementation prints an error message describing the problem, followed by either
	 * {@linkplain UnmatchedArgumentException#printSuggestions(PrintWriter) suggested alternatives} for mistyped
	 * options, or the full {@linkplain #usage(PrintWriter, Help.ColorScheme) usage} help message of the
	 * {@linkplain ParameterException#getCommandLine() problematic command}; it then delegates to the
	 * {@linkplain #getExitCodeExceptionMapper() exit code execution mapper} for an exit code, with
	...
	/**
	 * Sets the handler for dealing with invalid user input when the command is {@linkplain #execute(String...)
	 * executed}.
	 * <p>
	 * The specified setting will be registered with this {@code CommandLine} and the full hierarchy of its subcommands
	 * and nested sub-subcommands <em>at the moment this method is called</em>. Subcommands added later will have the
	 ...
	 * </p>
	 * 
	 * @param parameterExceptionHandler
	 *            the new handler for dealing with invalid user input
	 * @return this CommandLine for method chaining
	 * @since 4.0
	 */
	...
	/**
	 * Returns the handler for dealing with exceptions that occurred in the {@code Callable}, {@code Runnable} or
	 * {@code Method} user object of a command when the command was {@linkplain #execute(String...) executed}.
	 * <p>
	 * The default implementation rethrows the specified exception.
	 * </p>
	 * 
	 * @return the handler for dealing with exceptions that occurred in the business logic when the
	 *         {@link #execute(String...) execute} method was invoked.
	 * @since 4.0
	 */
	...
	/**
	 * Sets a custom handler for dealing with exceptions that occurred in the {@code Callable}, {@code Runnable} or
	 * {@code Method} user object of a command when the command was executed via the {@linkplain #execute(String...)
	 * execute} method.
	 * <p>
	 * The specified setting will be registered with this {@code CommandLine} and the full hierarchy of its subcommands
	 * and nested sub-subcommands <em>at the moment this method is called</em>. Subcommands added later will have the
	 ...
	 * </p>
	 * 
	 * @param executionExceptionHandler
	 ...
	 * </pre>
	 * <p>
	 * All this method does is parse the arguments and populate the annotated fields and methods. The caller is
	 * responsible for catching any exceptions, handling requests for usage help or version information, and invoking
	 * the business logic. Applications may be interested in using the {@link #execute(String...)} method instead.
	 * </p>
	 *
	 * @param command
	 *            the object to initialize. This object contains fields annotated with {@code @Option} or
	 *            {@code @Parameters}.
	 * @param args
	 ...
	 * instantiate the interface and the getter methods will return the option and positional parameter values matched
	 * on the command line.
	 * </p>
	 * <p>
	 * This is equivalent to
	 ...
	 * </pre>
	 * <p>
	 * All this method does is parse the arguments and return an instance whose annotated methods return the specified
	 * values. The caller is responsible for catching any exceptions, handling requests for usage help or version
	 * information, and invoking the business logic. Applications may be interested in using the
	 * {@link #execute(String...)} method instead.
	 * </p>
	 *
	 * @param spec
	 *            the interface that defines the command specification. This object contains getter methods annotated
	 *            with {@code @Option} or {@code @Parameters}.
	 * @param args
	 *            the command line arguments to parse
	 * @param <T>
	 *            the type of the annotated object
	 ...
	 * returned list may contain more elements if subcommands were {@linkplain #addSubcommand(String, Object)
	 * registered} and these subcommands were initialized by matching command line arguments. If parsing fails, a
	 ...
	 * @throws ParameterException
	 *             if the specified command line arguments are invalid; use {@link ParameterException#getCommandLine()}
	...
	/**
	 * Parses the specified command line arguments and returns a list of {@code ParseResult} with the options,
	 * positional parameters, and subcommands (if any) that were recognized and initialized during the parsing process.
	 * <p>
	...
	/**
	 * Returns the result of calling the user object {@code Callable} or invoking the user object {@code Method} after
	 * parsing the user input, or {@code null} if this command has not been {@linkplain #execute(String...) executed} or
	 * if this {@code CommandLine} is for a subcommand that was not specified by the end user on the command line.
	 * <p>
	 ...
	 * @return the result of the user object {@code Callable} or {@code Method} (may be {@code null}), or {@code null}
	 *         if this (sub)command was not executed
	 * @since 4.0
	 */
	@SuppressWarnings("unchecked")
	 ...
	 * 
	 * @param result
	 *            the business logic result, may be {@code null}
	 * @see #execute(String...)
	...
	/**
	 * Clears the {@linkplain #getExecutionResult() execution result} of a previous invocation from this
	 * {@code CommandLine} and all subcommands.
	 * 
	 * @since 4.0
	 */
	 ...
	 * interface</a> whose functional method is {@link #handleParseResult(List, PrintStream, CommandLine.Help.Ansi)}.
	 * <p>
	 ...
	 * CommandLine::parseWithHandler} methods to take some next step after the command line was successfully parsed.
	 * </p>
	 * 
	 * @see RunFirst
	 * @see RunLast
	...
	{
		/**
		 * Processes a List of {@code CommandLine} objects resulting from successfully {@linkplain #parse(String...)
		 * parsing} the command line arguments and optionally returns a list of results.
		 * 
		 * @param parsedCommands
		 *            the {@code CommandLine} objects that resulted from successfully parsing the command line arguments
		 * @param out
		 ...
		 * @throws ParameterException
		 *             if a help command was invoked for an unknown subcommand. Any {@code ParameterExceptions} thrown
		 *             from this method are treated as if this exception was thrown during parsing and passed to the
		 *             {@link IExceptionHandler}
		 * @throws ExecutionException
		 *             if a problem occurred while processing the parse results; use
		 *             {@link ExecutionException#getCommandLine()} to get the command or subcommand where processing
		 *             failed
		 */
	 ...
	 * <a href="https://docs.oracle.com/javase/8/docs/api/java/util/function/package-summary.html">functional
	 * interface</a> whose functional method is {@link IParseResultHandler2#handleParseResult(CommandLine.ParseResult)}.
	 * <p>
	 * Implementations of this function can be passed to the
	 * {@link #parseWithHandlers(IParseResultHandler2, IExceptionHandler2, String...) CommandLine::parseWithHandlers}
	 * methods to take some next step after the command line was successfully parsed.
	 * </p>
	 * <p>
	 * This interface replaces the {@link IParseResultHandler} interface; it takes the parse result as a
	 * {@code ParseResult} object instead of a List of {@code CommandLine} objects, and it has the freedom to select the
	...
	public static interface IParseResultHandler2<R>
	{
		 ...
		 * 
		 * @param parseResult
		 *            the {@code ParseResult} that resulted from successfully parsing the command line arguments
		 * @throws ParameterException
		 *             if a help command was invoked for an unknown subcommand. Any {@code ParameterExceptions} thrown
		 *             from this method are treated as if this exception was thrown during parsing and passed to the
	...
	/**
	 * Implementations are responsible for "executing" the user input and returning an exit code. The
	 * {@link #execute(String...)} method delegates to a {@linkplain #setExecutionStrategy(IExecutionStrategy)
	 ...
	 * </p>
	 * <ul>
	 * <li>From the {@code ParseResult}, select which {@code CommandSpec} should be executed. This is especially
	 * important for cli that have subcommands.</li>
	 * <li>"Execute" the selected {@code CommandSpec}. Often this means invoking a method on the spec's
	 * {@linkplain CommandSpec#userObject() user object}.</li>
	 * <li>Call {@link CommandLine#setExecutionResult(Object) setExecutionResult} to make the return value of that
	 * method invocation available to the application</li>
	 * <li>Return an exit code. Common sources of exit values are the invoked method's return value, or the user object
	 * if it implements {@link IExitCodeGenerator}.</li>
	 * </ul>
	 * <p>
	 * Implementors that need to print messages to the console should use the {@linkplain #getOut() output} and
	 * {@linkplain #getErr() error} PrintWriters, and the {@linkplain #getColorScheme() color scheme} from the
	 * CommandLine object obtained from ParseResult's CommandSpec.
	 * </p>
	 * <p>
	 * <b>API Note:</b>
	 * </p>
	...
	{
		/**
		 * "Executes" the user input and returns an exit code. Execution often means invoking a method on the selected
		 * CommandSpec's {@linkplain CommandSpec#userObject() user object}, and making the return value of that
		 * invocation available via {@link CommandLine#setExecutionResult(Object) setExecutionResult}.
		 * 
		 * @param parseResult
		 *            the parse result from which to select one or more {@code CommandSpec} instances to execute.
		 * @return an exit code
		 * @throws ParameterException
		 *             if the invoked method on the CommandSpec's user object threw a ParameterException to signify
		 *             invalid user input.
		 * @throws ExecutionException
		 *             if any problem occurred while executing the command. Any exceptions (other than
		 *             ParameterException) should be wrapped in a ExecutionException and not thrown as is.
		 */
	 ...
	 * Implementations of this function can be passed to the
	 * {@link #parseWithHandlers(IParseResultHandler, PrintStream, Help.Ansi, IExceptionHandler, String...)
	 * CommandLine::parseWithHandlers} methods to handle situations when the command line could not be parsed.
	 * </p>
	 * 
	 * @deprecated see {@link #execute(String...)}, {@link IParameterExceptionHandler} and
	...
	public static interface IExceptionHandler
	{
		 ...
		 * 
		 * @param ex
		 *            the ParameterException describing the problem that occurred while parsing the command line
		 *            arguments, and the CommandLine representing the command or subcommand whose input was invalid
		 * @param out
		 *            the {@code PrintStream} to print help to if requested
		 * @param ansi
		 *            for printing help messages using ANSI styles and colors
	...
	/**
	 * Classes implementing this interface know how to handle {@code ParameterExceptions} (usually from invalid user
	 * input) and {@code ExecutionExceptions} that occurred while executing the {@code Runnable} or {@code Callable}
	 * command.
	 * <p>
	...
	{
		/**
		 * Handles a {@code ParameterException} that occurred while {@linkplain #parseArgs(String...) parsing} the
		 * command line arguments and optionally returns a list of results.
		 * 
		 * @param ex
		 *            the ParameterException describing the problem that occurred while parsing the command line
		 *            arguments, and the CommandLine representing the command or subcommand whose input was invalid
		...
		/**
		 * Handles a {@code ExecutionException} that occurred while executing the {@code Runnable} or {@code Callable}
		 * command and optionally returns a list of results.
		 * 
		 * @param ex
		 *            the ExecutionException describing the problem that occurred while executing the {@code Runnable}
		 *            or {@code Callable} command, and the CommandLine representing the command or subcommand that was
		 *            being executed
		 * @param parseResult
	 ...
	 * Implementors that need to print messages to the console should use the {@linkplain #getOut() output} and
	 * {@linkplain #getErr() error} PrintWriters, and the {@linkplain #getColorScheme() color scheme} from the
	 * CommandLine object obtained from the exception.
	 * </p>
	 * <p>
	 * <b>Implementation Note:</b>
	 * </p>
	...
	public interface IParameterExceptionHandler
	{
		/**
		 * Handles a {@code ParameterException} that occurred while {@linkplain #parseArgs(String...) parsing} the
		 * command line arguments and returns an exit code suitable for returning from {@link #execute(String...)}.
		 * 
		 * @param ex
		 *            the ParameterException describing the problem that occurred while parsing the command line
		 *            arguments, and the CommandLine representing the command or subcommand whose input was invalid
	 ...
	 * {@code Runnable}, {@code Callable} or {@code Method} user object of the command.
	 * <p>
	 * <b>Implementation Requirements:</b>
	 * </p>
	 * <p>
	...
	public interface IExecutionExceptionHandler
	{
		 ...
		 * and returns an exit code suitable for returning from {@link #execute(String...)}.
		 * 
		 * @param ex
		 *            the Exception thrown by the {@code Runnable}, {@code Callable} or {@code Method} user object of
		 *            the command
		 * @param commandLine
		 *            the CommandLine representing the command or subcommand where the exception occurred
		 * @param parseResult
		 *            the result of parsing the command line arguments
		 * @return an exit code
		 */
	 ...
	 * <p>
	 * Note that {@code AbstractHandler} is a generic type. This, along with the abstract {@code self} method, allows
	 * method chaining to work properly in subclasses, without the need for casts. An example subclass can look like
	 * this:
	 * </p>
	 * 
	 * <pre>
	...
	public static abstract class AbstractHandler<R, T extends AbstractHandler<R, T>>
	{
		 ...
		 * <a href="http://www.gnu.org/prep/standards/html_node/_002d_002dhelp.html">convention</a>, when the user
		 * requests help with a {@code --help} or similar option, the usage help message is printed to the standard
		 * output stream so that it can be easily searched and paged.
		 * </p>
		 */
		...
		/**
		 * Returns the stream to print diagnostic messages to. Defaults to {@code System.err}, unless
		 * {@link #useErr(PrintStream)} was called with a different stream.
		 * <p>
		 * {@code IExceptionHandler2} implementations should use this stream to print error messages (which may include
		 * a usage help message) when an unexpected error occurs.
		 * </p>
		 */
		...
		/**
		 * Convenience method for subclasses that returns the specified result object if no exit code was set, or
		 * otherwise, if an exit code {@linkplain #andExit(int) was set}, calls {@code System.exit} with the configured
		...
		/**
		 * Convenience method for subclasses that throws the specified ExecutionException if no exit code was set, or
		 * otherwise, if an exit code {@linkplain #andExit(int) was set}, prints the stacktrace of the specified
		 * exception to the diagnostic error stream and calls {@code System.exit} with the configured exit code to
		...
		/**
		 * Indicates that the handler should call {@link System#exit(int)} after processing completes and sets the exit
		 * code to use as the termination status.
		 * 
		 * @deprecated use {@link CommandLine#execute(String...)} instead, and call {@code System.exit()} in the
		 *             application.
		 */
		@Deprecated
	...
	/**
	 * Default exception handler that handles invalid user input by printing the exception message, followed by the
	 ...
	 * </pre>
	 * <p>
	 * {@code ExecutionExceptions} that occurred while executing the {@code Runnable} or {@code Callable} command are
	 * simply rethrown and not handled.
	 * </p>
	 * 
	 * @deprecated see {@link #execute(String...)}, {@link #getParameterExceptionHandler()} and
			...
			implements IExceptionHandler, IExceptionHandler2<R>
	{
		...
		{
			internalHandleParseException(ex, new PrintWriter(out, true), Help.defaultColorScheme(ansi));
		...
		{
			internalHandleParseException(ex, new PrintWriter(err(), true), colorScheme());
	 ...
	 * 
	 * @param parseResult
	 *            contains the {@code CommandLine} objects found during parsing; check these to see if help was
	 *            requested
	 ...
	 *            the {@code PrintStream} to print help to if requested
	 * @param err
	 *            the error string to print diagnostic messages to, in addition to the output from the exception handler
	 * @param ansi
	 *            for printing help messages using ANSI styles and colors
	 * @return {@code true} if help was printed, {@code false} otherwise
	 * @since 3.0
	 */
	@Deprecated
	public static boolean printHelpIfRequested(List<CommandLine> parsedCommands, PrintStream out, PrintStream err,
			Help.Ansi ansi)
	{
			...
			Help.ColorScheme colorScheme)
	{
		...
		{
			cmd.setOut(new PrintWriter(out, true)).setErr(new PrintWriter(err, true)).setColorScheme(colorScheme);
		}
	 ...
	 * {@linkplain #usage(PrintWriter, Help.ColorScheme) usage help} to the {@linkplain CommandLine#getOut() configured
	 * output writer} if {@linkplain #isUsageHelpRequested() requested} or
	 * {@linkplain #printVersionHelp(PrintWriter, Help.Ansi, Object...) version help} to the
	 * {@linkplain CommandLine#getOut() configured output writer} if {@linkplain #isVersionHelpRequested() requested}
	 * and returns {@link CommandSpec#exitCodeOnUsageHelp()} or {@link CommandSpec#exitCodeOnVersionHelp()},
	 * respectively. If the command is a {@link Command#helpCommand()} and {@code runnable} or {@code callable}, that
	 * command is executed and this method returns {@link CommandSpec#exitCodeOnUsageHelp()}. Otherwise, if none of the
	 * specified {@code CommandLine} objects have help requested, this method returns {@code null}.
	 * <p>
	 ...
	 * </p>
	 * <p>
	 * When an error occurs while processing the help request, it is recommended custom Help cli throw a
	 ...
	 * To use this method, the annotated object that this {@code CommandLine} is constructed with needs to either
	 * implement {@link Runnable}, {@link Callable}, or be a {@code Method} object. See
	 * {@link #getCommandMethods(Class, String) getCommandMethods} for a convenient way to obtain a command
	 * {@code Method}.
	 * <p>
	 * This method replaces the {@link #run(Runnable, String...) run}, {@link #call(Callable, String...) call} and
	 * {@link #invoke(String, Class, String...) invoke} convenience methods that were available with previous versions
	 * of picocli.
	 * </p>
	 * <p>
	 * <b>Exit Code</b>
	 * </p>
	 * <p>
	 * This method returns an exit code that applications can use to call {@code System.exit}. (The return value of the
	 * {@code Callable} or {@code Method} can still be obtained via {@link #getExecutionResult() getExecutionResult}.)
	 * If the user object {@code Callable} or {@code Method} returns an {@code int} or {@code Integer}, this will be
	 * used as the exit code. Additionally, if the user object implements {@link CommandLine.IExitCodeGenerator
	 * IExitCodeGenerator}, an exit code is obtained by calling its {@code getExitCode()} method (after invoking the
	 * user object).
	 * </p>
	 * <p>
	 * In the case of multiple exit codes the highest value will be used (or if all values are negative, the lowest
	 * value will be used).
	 * </p>
	 * <p>
	 ...
	 * </p>
	 * <p>
	 * If the user specified invalid input, the {@linkplain #getParameterExceptionHandler() parameter exception handler}
	 * is invoked. By default this prints an error message and the usage help message, and returns an exit code.
	 * </p>
	 * <p>
	 * If an exception occurred while the user object {@code Runnable}, {@code Callable}, or {@code Method} was invoked,
	 * this exception is caught and passed to the {@linkplain #getExecutionExceptionHandler() execution exception
	 * handler}. The default {@code IExecutionExceptionHandler} will rethrow this Exception.
	 * </p>
	 * <p>
	 ...
	 * </p>
	 * <p>
	 * If an {@link CommandLine.IExitCodeExceptionMapper IExitCodeExceptionMapper} is
	 * {@linkplain #setExitCodeExceptionMapper(IExitCodeExceptionMapper) configured}, this mapper is used to determine
	 * the exit code based on the exception.
	 * </p>
	 * <p>
	 * If an {@code IExitCodeExceptionMapper} is not set, by default this method will return the {@code @Command}
	 * annotation's {@link Command#exitCodeOnInvalidInput() exitCodeOnInvalidInput} or
	 * {@link Command#exitCodeOnExecutionException() exitCodeOnExecutionException} value, respectively.
	 * </p>
	 * <p>
	 * <b>Example Usage:</b>
	 * </p>
	 ...
	 * </pre>
	 * <p>
	 * Since {@code execute} is an instance method, not a static method, applications can do configuration before
	 ...
	 * {
	 * 	&#64;code
	 * 	CommandLine cmd = new CommandLine(new MyCallable()).setCaseInsensitiveEnumValuesAllowed(true) // configure a
	 * 																									// non-default
	 * 																									// parser option
	 ...
	 * </pre>
	 * <p>
	 * If the specified command has subcommands, the {@linkplain RunLast last} subcommand specified on the command line
	 * is executed. This can be configured by setting the {@linkplain #setExecutionStrategy(IExecutionStrategy)
	 * execution strategy}. Built-in alternatives are executing the {@linkplain RunFirst first} subcommand, or executing
	 * {@linkplain RunAll all} specified subcommands.
	 * </p>
	 * 
	 * @param args
	 *            the command line arguments to parse
	...
	public int execute(String... args)
	{
		...
		catch (ExecutionException ex)
		{
			try
			{
				...
				return getExecutionExceptionHandler().handleExecutionException(cause, ex.getCommandLine(),
						parseResult[0]);
			}
			catch (Exception ex2)
			{
	...
	private <T> T enrichForBackwardsCompatibility(T obj)
	{
		...
		if (obj instanceof AbstractHandler<?, ?>)
		{
			...
			{
				setOut(new PrintWriter(handler.out(), true));
			}
			if (handler.err() != System.err)
			{
				setErr(new PrintWriter(handler.err(), true));
			}
			if (handler.ansi() != Help.Ansi.AUTO)
			{
				setColorScheme(handler.colorScheme());
			}
		}
		return obj;
	}
	...
	/**
	 * Command line parse result handler that returns a value. This handler prints help if requested, and otherwise
	 * calls {@link #handle(CommandLine.ParseResult)} with the parse result. Facilitates implementation of the
	 * {@link IParseResultHandler2} interface.
	 * <p>
	 * Note that {@code AbstractParseResultHandler} is a generic type. This, along with the abstract {@code self}
	 * method, allows method chaining to work properly in subclasses, without the need for casts. An example subclass
	 * can look like this:
	 * </p>
	 * 
	 * <pre>
	 ...
	 *             {@link #getParameterExceptionHandler()}, {@link #getExecutionExceptionHandler()}
	 * @since 3.0
	 */
	@Deprecated
	...
	{
		/**
		 * Prints help if requested, and otherwise calls {@link #handle(CommandLine.ParseResult)}. Finally, either a
		 * list of result objects is returned, or the JVM is terminated if an exit code {@linkplain #andExit(int) was
		 * set}.
		 *
		 * @param parseResult
		 *            the {@code ParseResult} that resulted from successfully parsing the command line arguments
		 ...
		 * @throws ParameterException
		 *             if the {@link HelpCommand HelpCommand} was invoked for an unknown subcommand. Any
		 *             {@code ParameterExceptions} thrown from this method are treated as if this exception was thrown
		...
		/**
		 * Processes the specified {@code ParseResult} and returns the result as a list of objects. Implementations are
		 * responsible for catching any exceptions thrown in the {@code handle} method, and rethrowing an
		 * {@code ExecutionException} that details the problem and captures the offending {@code CommandLine} object.
		 *
		 * @param parseResult
		 *            the {@code ParseResult} that resulted from successfully parsing the command line arguments
	...
	/**
	 * Command line {@linkplain IExecutionStrategy execution strategy} that prints help if requested, and otherwise
	 * executes the top-level {@code Runnable} or {@code Callable} command. For use by the {@link #execute(String...)
	 * execute} method.
	 * 
	 * @since 2.0
	 */
	...
	public static class RunFirst extends AbstractParseResultHandler<List<Object>> implements IParseResultHandler
	{
		 ...
		 * Finally, either a list of result objects is returned, or the JVM is terminated if an exit code
		 * {@linkplain #andExit(int) was set}. If the top-level command does not implement either {@code Runnable} or
		 * {@code Callable}, an {@code ExecutionException} is thrown detailing the problem and capturing the offending
		...
		/**
		 * Executes the top-level {@code Runnable} or {@code Callable} subcommand. If the top-level command does not
		 * implement either {@code Runnable} or {@code Callable} and is not a {@code Method}, an
		 * {@code ExecutionException} is thrown detailing the problem and capturing the offending {@code CommandLine}
		 * object.
		 *
		 * @param parseResult
		 *            the {@code ParseResult} that resulted from successfully parsing the command line arguments
	...
	/**
	 * Command line {@linkplain IExecutionStrategy execution strategy} that prints help if requested, and otherwise
	 * executes the most specific {@code Runnable} or {@code Callable} subcommand. For use by the
	 * {@link #execute(String...) execute} method.
	 * <p>
	 * Something like this:
	 * </p>
	 ...
	 * {@link #call(Callable, PrintStream, PrintStream, Help.Ansi, String...) call} convenience methods.
	 * </p>
	 * 
	 * @since 2.0
	 */
	...
	{
		/**
		 * Prints help if requested, and otherwise executes the most specific {@code Runnable} or {@code Callable}
		 * subcommand. Finally, either a list of result objects is returned, or the JVM is terminated if an exit code
		 * {@linkplain #andExit(int) was set}. If the last (sub)command does not implement either {@code Runnable} or
		 * {@code Callable}, an {@code ExecutionException} is thrown detailing the problem and capturing the offending
		 * {@code CommandLine} object.
		 *
		 * @param parsedCommands
		...
		public List<Object> handleParseResult(List<CommandLine> parsedCommands, PrintStream out, Help.Ansi ansi)
		{
			...
			return returnResultOrExit(
					executeUserObject(parsedCommands.get(parsedCommands.size() - 1), new ArrayList<Object>()));
		}

		/**
		 * Executes the most specific {@code Runnable} or {@code Callable} subcommand. If the last (sub)command does not
		 * implement either {@code Runnable} or {@code Callable} and is not a {@code Method}, an
		 * {@code ExecutionException} is thrown detailing the problem and capturing the offending {@code CommandLine}
		 * object.
		 *
		...
		protected List<IExitCodeGenerator> extractExitCodeGenerators(ParseResult parseResult)
		{
			...
			{
				return Arrays.asList((IExitCodeGenerator) userObject);
			}
			return Collections.emptyList();
		}
	...
	/**
	 * Command line {@linkplain IExecutionStrategy execution strategy} that prints help if requested, and otherwise
	 * executes the top-level command and all subcommands as {@code Runnable}, {@code Callable} or {@code Method}. For
	 * use by the {@link #execute(String...) execute} method.
	 * 
	 * @since 2.0
	 */
	@SuppressWarnings("deprecation")
	public static class RunAll extends AbstractParseResultHandler<List<Object>> implements IParseResultHandler
	{
		 ...
		 * {@code Runnable}, {@code Callable} or {@code Method}. Finally, either a list of result objects is returned,
		 * or the JVM is terminated if an exit code {@linkplain #andExit(int) was set}. If any of the
		 * {@code CommandLine} cli does not implement either {@code Runnable} or {@code Callable}, an
		 * {@code ExecutionException} is thrown detailing the problem and capturing the offending {@code CommandLine}
		 * object.
		 *
		 * @param parsedCommands
		 ...
		 * @param ansi
		 *            for printing help messages using ANSI styles and colors
		 * @return an empty list if help was requested, or a list containing the result of executing all cli: the return
		 *         values from calling the {@code Callable} cli, {@code null} elements for cli that implement
		 *         {@code Runnable}
		 * @throws ParameterException
		 *             if the {@link HelpCommand HelpCommand} was invoked for an unknown subcommand. Any
		 *             {@code ParameterExceptions} thrown from this method are treated as if this exception was thrown
		...
		/**
		 * Executes the top-level command and all subcommands as {@code Runnable} or {@code Callable}. If any of the
		 * {@code CommandLine} cli does not implement either {@code Runnable} or {@code Callable} and is not a
		 * {@code Method}, an {@code ExecutionException} is thrown detailing the problem and capturing the offending
		 * {@code CommandLine} object.
		 *
		 * @param parseResult
		 *            the {@code ParseResult} that resulted from successfully parsing the command line arguments
	...
	/**
	 * Returns the result of calling {@link #parseWithHandlers(IParseResultHandler2, IExceptionHandler2, String...)}
	 * with a new {@link DefaultExceptionHandler} in addition to the specified parse result handler and the specified
	 * command line arguments.
	 * <p>
	 * This is a convenience method intended to offer the same ease of use as the
	 * {@link #run(Runnable, PrintStream, PrintStream, Help.Ansi, String...) run} and
	 * {@link #call(Callable, PrintStream, PrintStream, Help.Ansi, String...) call} methods, but with more flexibility
	 * and better support for nested subcommands.
	 * </p>
	 * <p>
	 ...
	 * </pre>
	 * <p>
	 * Picocli provides some default handlers that allow you to accomplish some common tasks with very little code. The
	 ...
	 * <li>{@link RunFirst} handler prints help if requested, and otherwise executes the top-level command as a
	 * {@code Runnable} or {@code Callable}.</li>
	 * <li>{@link RunAll} handler prints help if requested, and otherwise executes all recognized cli and subcommands as
	 * {@code Runnable} or {@code Callable} tasks.</li>
	 ...
	 *            the return type of this handler
	 * @param handler
	 *            the function that will handle the result of successfully parsing the command line arguments
	 * @param args
	 ...
	 *             parse results; use {@link ExecutionException#getCommandLine()} to get the command or subcommand where
	 *             processing failed
	 * @see RunLast
	 * @see RunAll
	 * @deprecated use {@link #execute(String...)} and {@link #getExecutionResult()} instead
	 ...
	 */
	@Deprecated
	public List<Object> parseWithHandlers(IParseResultHandler handler, PrintStream out, Help.Ansi ansi,
	...
	/**
	 * Tries to {@linkplain #parseArgs(String...) parse} the specified command line arguments, and if successful,
	 * delegates the processing of the resulting {@code ParseResult} object to the specified
	 * {@linkplain IParseResultHandler2 handler}. If the command line arguments were invalid, the
	 * {@code ParameterException} thrown from the {@code parse} method is caught and passed to the specified
	 * {@link IExceptionHandler2}.
	 * <p>
	 * This is a convenience method intended to offer the same ease of use as the
	 * {@link #run(Runnable, PrintStream, PrintStream, Help.Ansi, String...) run} and
	 ...
	 *            the function that can handle the {@code ParameterException} thrown when the command line arguments are
	 *            invalid
	 * @param args
	 *            the command line arguments
	 * @return an object resulting from handling the parse result or the exception that occurred while parsing the input
	 * @throws ExecutionException
	 *             if the command line arguments were parsed successfully but a problem occurred while processing the
	 *             parse result {@code ParseResult} object; use {@link ExecutionException#getCommandLine()} to get the
	 *             command or subcommand where processing failed
	 * @param <R>
	 ...
	 */
	@Deprecated
	public <R> R parseWithHandlers(IParseResultHandler2<R> handler, IExceptionHandler2<R> exceptionHandler,
			String... args)
	{
		clearExecutionResults();
		ParseResult parseResult = null;
	...
	{
		return String.format("%s, JVM: %s (%s %s %s), OS: %s %s %s", VERSION, System.getProperty("java.version"),
				System.getProperty("java.vendor"), System.getProperty("java.vm.name"),
				System.getProperty("java.vm.version"), System.getProperty("os.name"), System.getProperty("os.version"),
	...
	/**
	 * Equivalent to {@code new CommandLine(command).usage(out, ansi)}. See {@link #usage(PrintStream, Help.Ansi)} for
	 * details.
	 * 
	 * @param command
	 *            the object annotated with {@link Command}, {@link Option} and {@link Parameters}
	 ...
	 *            the print stream to print the help message to
	 * @param colorScheme
	 *            the {@code ColorScheme} defining the styles for options, parameters and cli when ANSI is enabled
	 * @throws IllegalArgumentException
	 *             if the specified command object does not have a {@link Command}, {@link Option} or {@link Parameters}
	 *             annotation
	 */
	...
	/**
	 * Delegates to {@link #usage(PrintStream, Help.ColorScheme)} with the {@linkplain #getColorScheme() configured}
	 * color scheme.
	 * 
	 * @param out
	...
	/**
	 * Delegates to {@link #usage(PrintWriter, Help.ColorScheme)} with the {@linkplain #getColorScheme() configured}
	 * color scheme.
	 * 
	 * @param writer
	...
	/**
	 * Similar to {@link #usage(PrintStream, Help.Ansi)} but with the specified {@code PrintWriter} instead of a
	 * {@code PrintStream}.
	 * 
	 * @since 3.0
	 */
	...
	/**
	 * Prints a usage help message for the annotated command class to the specified {@code PrintStream}. Delegates
	 * construction of the usage help message to the {@link Help} inner class and is equivalent to:
	 * 
	 * <pre>
	 ...
	 * </pre>
	 * <p>
	 * Annotate your class with {@link Command} to control many aspects of the usage help message, including the program
	 * name, text of section headings and section contents, and some aspects of the auto-generated sections of the usage
	 * help message.
	 * <p>
	 ...
	 * {@linkplain Help.Layout layout}, and/or a custom option {@linkplain Help.IOptionRenderer renderer} for ultimate
	 * control over which aspects of an Option or Field are displayed where.
	 * </p>
	 * 
	 * @param out
	...
	/**
	 * Similar to {@link #usage(PrintStream, Help.ColorScheme)}, but with the specified {@code PrintWriter} instead of a
	 * {@code PrintStream}.
	 * 
	 * @since 3.0
	 */
	...
	/**
	 * Similar to {@link #usage(PrintStream)}, but returns the usage help message as a String instead of printing it to
	...
	/**
	 * Similar to {@link #usage(PrintStream, Help.Ansi)}, but returns the usage help message as a String instead of
	 * printing it to the {@code PrintStream}.
	 * 
	 * @since 3.2
	 */
	public String getUsageMessage(Help.Ansi ansi)
	{
		return usage(new StringBuilder(), getHelpFactory().create(getCommandSpec(), Help.defaultColorScheme(ansi)))
				.toString();
	}

	/**
	 * Similar to {@link #usage(PrintStream, Help.ColorScheme)}, but returns the usage help message as a String instead
	 * of printing it to the {@code PrintStream}.
	 * 
	 * @since 3.2
	 */
	...
	/**
	 * Delegates to {@link #printVersionHelp(PrintStream, Help.Ansi)} with the ANSI setting of the
	 * {@linkplain #getColorScheme() configured} color scheme.
	 * 
	 * @param out
	 *            the printStream to print to
	...
	/**
	 * Delegates to {@link #printVersionHelp(PrintWriter, Help.Ansi, Object...)} with the ANSI setting of the
	 * {@linkplain #getColorScheme() configured} color scheme.
	 * 
	 * @param out
	 *            the PrintWriter to print to
	 ...
	 * @return {@code null} if an error occurred while parsing the command line options, or if help was requested and
	 *         printed. Otherwise returns the result of calling the Callable
	 * @see #execute(String...)
	 * @since 3.0
	 * @deprecated use {@link #execute(String...)} and {@link #getExecutionResult()} instead
	 */
	 ...
	 * @param ansi
	 *            including whether the usage message should include ANSI escape codes or not
	 * @param args
	 *            the command line arguments to parse
	 * @param <C>
	 *            the annotated object must implement Callable
	 ...
	 * 
	 * @param callableClass
	 *            class of the command to call when {@linkplain #parseArgs(String...) parsing} succeeds.
	 * @param factory
	 *            the factory responsible for instantiating the specified callable class and potentially inject other
	 *            components
	 * @param args
	 *            the command line arguments to parse
	 * @param <C>
	 ...
	 *            the return type of the most specific command (must implement {@code Callable})
	 * @throws InitializationException
	 *             if the specified class cannot be instantiated by the factory, or does not have a {@link Command},
	 ...
	 *            class of the command to call when {@linkplain #parseArgs(String...) parsing} succeeds.
	 * @param factory
	 *            the factory responsible for instantiating the specified callable class and potentially injecting other
	 *            components
	 * @param out
	 *            the printStream to print the usage help message to when the user requested help
	 * @param args
	...
	/**
	 * Convenience method to allow command line application authors to avoid some boilerplate code in their application.
	 * The specified {@linkplain IFactory factory} will create an instance of the specified {@code callableClass}; use
	 * this method instead of {@link #call(Callable, PrintStream, PrintStream, Help.Ansi, String...) call(Callable,
	 * ...)} if you want to use a factory that performs Dependency Injection. The annotated class needs to implement
	 * {@link Callable}.
	 * <p>
	 * Consider using the {@link #execute(String...)} method instead:
	 * </p>
	...
	/**
	 * Delegates to {@link #run(Runnable, PrintStream, PrintStream, Help.Ansi, String...)} with {@code System.err} for
	 * diagnostic error messages and {@link Help.Ansi#AUTO}.
	 * 
	 * @param runnable
	 *            the command to run when {@linkplain #parseArgs(String...) parsing} succeeds.
	 ...
	 * {
	 * 	&#64;code
	 * 	CommandLine cmd = new CommandLine(runnable).setOut(myOutWriter()) // System.out by default
	 * 			.setErr(myErrWriter()) // System.err by default
	 * 			.setColorScheme(myColorScheme()); // default color scheme, Ansi.AUTO by default
	 * 	int exitCode = cmd.execute(args);
	 * 	// System.exit(exitCode);
	 ...
	 * {@linkplain #printHelpIfRequested(List, PrintStream, PrintStream, Help.Ansi) requested}, and any exceptions
	 * thrown by the {@code Runnable} are caught and rethrown wrapped in an {@code ExecutionException}.
	 * </p>
	 * 
	 * @param runnable
	 ...
	 */
	@Deprecated
	public static <R extends Runnable> void run(R runnable, PrintStream out, PrintStream err, Help.Ansi ansi,
			String... args)
	{
	 ...
	 * 
	 * @param runnableClass
	 *            class of the command to run when {@linkplain #parseArgs(String...) parsing} succeeds.
	 * @param factory
	 *            the factory responsible for instantiating the specified Runnable class and potentially injecting other
	 *            components
	 * @param args
	 *            the command line arguments to parse
	 * @param <R>
	 ...
	 */
	@Deprecated
	public static <R extends Runnable> void run(Class<R> runnableClass, IFactory factory, PrintStream out,
			String... args)
	{
	...
	/**
	 * Convenience method to allow command line application authors to avoid some boilerplate code in their application.
	 * The specified {@linkplain IFactory factory} will create an instance of the specified {@code runnableClass}; use
	 * this method instead of {@link #run(Runnable, PrintStream, PrintStream, Help.Ansi, String...) run(Runnable, ...)}
	 * if you want to use a factory that performs Dependency Injection. The annotated class needs to implement
	 * {@link Runnable}.
	 * <p>
	 * Consider using the {@link #execute(String...)} method instead:
	 ...
	 * {
	 * 	&#64;code
	 * 	CommandLine cmd = new CommandLine(runnableClass, factory).setOut(myOutWriter()) // System.out by default
	 * 			.setErr(myErrWriter()) // System.err by default
	 * 			.setColorScheme(myColorScheme()); // default color scheme, Ansi.AUTO by default
	 * 	int exitCode = cmd.execute(args);
	 * 	// System.exit(exitCode);
	...
	/**
	 * Delegates to {@link #invoke(String, Class, PrintStream, PrintStream, Help.Ansi, String...)} with
	 * {@code System.out} for requested usage help messages, {@code System.err} for diagnostic error messages, and
	 * {@link Help.Ansi#AUTO}.
	 * 
	 * @param methodName
	 *            the {@code @Command}-annotated method to build a {@link CommandSpec} config from, and run when
	 ...
	 * @throws InitializationException
	 *             if the specified method does not have a {@link Command} annotation, or if the specified class
	 *             contains multiple {@code @Command}-annotated methods with the specified name
	 * @throws ExecutionException
	 *             if the Runnable throws an exception
	 * @see #parseWithHandlers(IParseResultHandler2, IExceptionHandler2, String...)
	 * @since 3.6
	...
	/**
	 * Delegates to {@link #invoke(String, Class, PrintStream, PrintStream, Help.Ansi, String...)} with the specified
	 * stream for requested usage help messages, {@code System.err} for diagnostic error messages, and
	 * {@link Help.Ansi#AUTO}.
	 * 
	 * @param methodName
	 *            the {@code @Command}-annotated method to build a {@link CommandSpec} config from, and run when
	...
	/**
	 * Delegates to {@link #invoke(String, Class, PrintStream, PrintStream, Help.Ansi, String...)} with the specified
	 * stream for requested usage help messages, {@code System.err} for diagnostic error messages, and the specified
	 * Ansi mode.
	 * 
	 * @param methodName
	 *            the {@code @Command}-annotated method to build a {@link CommandSpec} config from, and run when
	...
	/**
	 * Convenience method to allow command line application authors to avoid some boilerplate code in their application.
	 * Constructs a {@link CommandSpec} config from the {@code @Option} and {@code @Parameters}-annotated method
	 * parameters of the {@code @Command}-annotated method, parses the specified command line arguments and invokes the
	 * specified method.
	 * <p>
	 * Consider using the {@link #execute(String...)} method instead:
	 * </p>
	...
	/**
	 * Helper to get methods of a class annotated with {@link Command @Command} via reflection, optionally filtered by
	 * method name (not {@link Command#name() @Command.name}). Methods have to be either public (inherited) members or
	 * be declared by {@code cls}, that is "inherited" static or protected methods will not be picked up.
	 *
	 * @param cls
	 *            the class to search for methods annotated with {@code @Command}
	 * @param methodName
	 *            if not {@code null}, return only methods whose method name (not {@link Command#name() @Command.name})
	 *            equals this string. Ignored if {@code null}.
	 ...
	 * {@link Option}, the field's type is used as a lookup key to find the associated type converter, and this type
	 * converter converts the original command line argument string value to the correct type.
	 * <p>
	...
	/**
	 * Sets the String the parser uses to separate option names from option values to the specified value. The separator
	 * may also be set declaratively with the {@link CommandLine.Command#separator()} annotation attribute.
	 * <p>
	 * The specified setting will be registered with this {@code CommandLine} and the full hierarchy of its subcommands
	 * and nested sub-subcommands <em>at the moment this method is called</em>. Subcommands added later will have the
	 * default setting. To ensure a setting is applied to all subcommands, call the setter last, after adding
	 ...
	 * <p>
	 * The specified bundle will be registered with this {@code CommandLine} and the full hierarchy of its subcommands
	 * and nested sub-subcommands <em>at the moment this method is called</em>. Subcommands added later will not be
	 * impacted. To ensure a setting is applied to all subcommands, call the setter last, after adding subcommands.
	 * </p>
	 * 
	 * @param bundle
	...
	/**
	 * Returns whether picocli should attempt to detect the terminal size and adjust the usage help message width to
	 * take the full terminal width. End users may enable this by setting system property {@code "picocli.usage.width"}
	 * to {@code AUTO}, and may disable this by setting this system property to a {@linkplain UsageMessageSpec#width()
	 * numeric value}. This feature requires Java 7 or greater. The default is {@code false}.
	 * 
	...
	/**
	 * Sets whether picocli should attempt to detect the terminal size and adjust the usage help message width to take
	 * the full terminal width. The default is {@code false}.
	 * <p>
	 * The specified setting will be registered with this {@code CommandLine} and the full hierarchy of its subcommands
	 * and nested sub-subcommands <em>at the moment this method is called</em>. Subcommands added later will have the
	 * default setting. To ensure a setting is applied to all subcommands, call the setter last, after adding
	 ...
	 * </p>
	 * 
	 * @param detectTerminalSize
	 *            whether picocli should attempt to detect the terminal size
	 ...
	 * Note that this method only modifies the usage help message, it does not impact parsing behaviour. The command
	 * name may also be set declaratively with the {@link CommandLine.Command#name()} annotation attribute.
	 * 
	 * @param commandName
	 ...
	 * contents should be expanded into separate arguments for each line in the specified file. This property is
	 ...
	 * contents should be expanded into separate arguments for each line in the specified file. ({@code true} by
	 * default.)
	 * 
	 * @param expandAtFiles
	 ...
	 * be interpreted as arguments (without comments). If specified, all characters from the comment character to the
	...
	/**
	 * Sets the character that starts a single-line comment or {@code null} if all content of argument files should be
	 * interpreted as arguments (without comments). If specified, all characters from the comment character to the end
	 * of the line are ignored.
	 * 
	 * @param atFileCommentChar
	 *            the character that starts a single-line comment or {@code null}. The default is {@code '#'}.
	 * @return this {@code CommandLine} object, to allow method chaining
	...
	/**
	 * Returns whether to use a simplified argument file format that is compatible with JCommander. In this format,
	 * every line (except empty lines and comment lines) is interpreted as a single argument. Arguments containing
	 * whitespace do not need to be quoted. When system property {@code "picocli.useSimplifiedAtFiles"} is defined, the
	 * system property value overrides the programmatically set value.
	 * 
	...
	/**
	 * Sets whether to use a simplified argument file format that is compatible with JCommander. In this format, every
	 * line (except empty lines and comment lines) is interpreted as a single argument. Arguments containing whitespace
	 * do not need to be quoted. When system property {@code "picocli.useSimplifiedAtFiles"} is defined, the system
	 * property value overrides the programmatically set value.
	 * 
	 * @param simplifiedAtFiles
	 *            whether to use a simplified argument file format. The default is {@code false}.
	 * @return this {@code CommandLine} object, to allow method chaining
	...
	/**
	 * Returns the {@code INegatableOptionTransformer} used to create the negative form of
	 * {@linkplain Option#negatable() negatable} options. By default this returns the result of
	 * {@link RegexTransformer#createDefault()}.
	 * 
	...
	/**
	 * Sets the {@code INegatableOptionTransformer} used to create the negative form of {@linkplain Option#negatable()
	 * negatable} options.
	 * <p>
	 * The specified setting will be registered with this {@code CommandLine} and the full hierarchy of its subcommands
	 * and nested sub-subcommands <em>at the moment this method is called</em>. Subcommands added later will have the
	 ...
	 * 
	 * @param transformer
	 *            the {@code INegatableOptionTransformer} used to create negative option names.
	 * @return this {@code CommandLine} object, to allow method chaining
	 * @see Option#negatable()
	 ...
	 * </pre>
	 * <p>
	 * A field cannot be annotated with both {@code @Parameters} and {@code @Option} or a {@code ParameterException} is
	 * thrown.
	 * </p>
	 */
	...
	public @interface Option
	{
		 ...
		 * <p>
		 * Different environments have different conventions for naming options, but usually options have a prefix that
		 * sets them apart from parameters. Picocli supports all of the below styles. The default separator is
		 ...
		 * </p>
		 * <p>
		 * In Unix and Linux, options have a short (single-character) name, a long name or both. Short options
		 * (<a href="http://pubs.opengroup.org/onlinepubs/9699919799/basedefs/V1_chap12.html#tag_12_02">POSIX style</a>
		 * are single-character and are preceded by the {@code '-'} character, e.g., {@code `-v'}.
		 * <a href="https://www.gnu.org/software/tar/manual/html_node/Long-Options.html">GNU-style</a> long (or
		 * <em>mnemonic</em>) options start with two dashes in a row, e.g., {@code `--file'}.
		 * </p>
		 * <p>
		 * Picocli supports the POSIX convention that short options can be grouped, with the last option optionally
		 * taking a parameter, which may be attached to the option name or separated by a space or a {@code '='}
		 ...
		 * </p>
		 * <p>
		 * DOS options mostly have upper case single-character names and start with a single slash {@code '/'}
		 * character. Option parameters are separated by a {@code ':'} character. Options cannot be grouped together but
		 ...
		 * </p>
		 * <p>
		 * Windows PowerShell options generally are a word preceded by a single {@code '-'} character, e.g.,
		 * {@code `-Help'}. Option parameters are separated by a space or by a {@code ':'} character.
		 * </p>
		 * 
		 ...
		 * <p>
		 * If an option is required, but a user invokes the program without specifying the required option, a
		 * {@link MissingParameterException} is thrown from the {@link #parse(String...)} method.
		 * </p>
		 * <p>
		 * Required options that are part of a {@linkplain ArgGroup group} are required <em>within the group</em>, not
		 * required within the command: the group's {@linkplain ArgGroup#multiplicity() multiplicity} determines whether
		...
		/**
		 * Set {@code help=true} if this option should disable validation of the remaining arguments: If the
		 * {@code help} option is specified, no error message is generated for missing required options.
		 * <p>
		 * This attribute is useful for special options like help ({@code -h} and {@code --help} on unix, {@code -?} and
		 * {@code -Help} on Windows) or version ({@code -V} and {@code --version} on unix, {@code -Version} on Windows).
		 * </p>
		 * <p>
		 * Note that the {@link #parse(String...)} method will not print help documentation. It will only set the value
		 * of the annotated field. It is the responsibility of the caller to inspect the annotated fields and take the
		 * appropriate action.
		 * </p>
		 * 
		...
		/**
		 * Set {@code usageHelp=true} for the {@code --help} option that triggers display of the usage help message. The
		 * <a href="http://picocli.info/#_printing_help_automatically">convenience methods</a> {@code Commandline.call},
		 * {@code Commandline.run}, and {@code Commandline.parseWithHandler(s)} will automatically print usage help when
		 * an option with {@code usageHelp=true} was specified on the command line.
		 * <p>
		 * By default, <em>all</em> options and positional parameters are included in the usage help message <em>except
		 * when explicitly marked {@linkplain #hidden() hidden}.</em>
		 * </p>
		 * <p>
		 * If this option is specified on the command line, picocli will not validate the remaining arguments (so no
		 * "missing required option" errors) and the {@link CommandLine#isUsageHelpRequested()} method will return
		 * {@code true}.
		 * </p>
		 * <p>
		...
		/**
		 * Set {@code versionHelp=true} for the {@code --version} option that triggers display of the version
		 * information. The <a href="http://picocli.info/#_printing_help_automatically">convenience methods</a>
		 * {@code Commandline.call}, {@code Commandline.run}, and {@code Commandline.parseWithHandler(s)} will
		 * automatically print version information when an option with {@code versionHelp=true} was specified on the
		 * command line.
		 * <p>
		 * The version information string is obtained from the command's {@linkplain Command#version() version}
		 * annotation or from the {@linkplain Command#versionProvider() version provider}.
		 * </p>
		 * <p>
		 * If this option is specified on the command line, picocli will not validate the remaining arguments (so no
		 * "missing required option" errors) and the {@link CommandLine#isUsageHelpRequested()} method will return
		...
		/**
		 * Description of this option, used when generating the usage documentation. Each element of the array is
		 * rendered on a separate line.
		 * <p>
		 * May contain embedded {@linkplain java.util.Formatter format specifiers} like {@code %n} line separators.
		 * Literal percent {@code '%'} characters must be escaped with another {@code %}.
		 * </p>
		 * <p>
		 * The description may contain variables that are rendered when help is requested. The string
		 * {@code ${DEFAULT-VALUE}} is replaced with the default value of the option. This is regardless of the
		 * command's {@link Command#showDefaultValues() showDefaultValues} setting or the option's
		 * {@link #showDefaultValue() showDefaultValue} setting. The string {@code ${COMPLETION-CANDIDATES}} is replaced
		 * with the completion candidates generated by {@link #completionCandidates()} in the description for this
		...
		/**
		 * Specifies the minimum number of required parameters and the maximum number of accepted parameters. If an
		 * option declares a positive arity, and the user specifies an insufficient number of parameters on the command
		 * line, a {@link MissingParameterException} is thrown by the {@link #parse(String...)} method.
		 * <p>
		 * In many cases picocli can deduce the number of required parameters from the field's type. By default, flags
		 * (boolean options) have arity zero, and single-valued type fields (String, int, Integer, double, Double, File,
		 * Date, etc) have arity one. Generally, fields with types that cannot hold multiple values can omit the
		 * {@code arity} attribute.
		 * </p>
		 * <p>
		 ...
		 * </p>
		 * <p>
		 * For example, if an option has 2 required parameters and any number of optional parameters, specify
		 ...
		 * You can make a boolean option take a required parameter by annotating your field with {@code arity="1"}. For
		 * example:
		 * </p>
		 * 
		 * <pre>
		 ...
		 * or {@code <program> -v true} on the command line, or a {@link MissingParameterException} is thrown by the
		 * {@link #parse(String...)} method.
		 * </p>
		 * <p>
		 * To make the boolean parameter possible but optional, define the field with {@code arity = "0..1"}. For
		 * example:
		 * </p>
		 * 
		 * <pre>
		...
		/**
		 * Returns whether usage syntax decorations around the {@linkplain #paramLabel() paramLabel} should be
		 * suppressed. The default is {@code false}: by default, the paramLabel is surrounded with {@code '['} and
		 * {@code ']'} characters if the value is optional and followed by ellipses ("...") when multiple values can be
		 * specified.
		 * 
		 * @since 3.6.0
		 */
		...
		/**
		 * <p>
		 * Optionally specify a {@code type} to control exactly what Class the option parameter should be converted to.
		 * This may be useful when the field type is an interface or an abstract class. For example, a field can be
		 * declared to have type {@code java.lang.Number}, and annotating {@code @Option(type=Short.class)} ensures that
		 * the option parameter value is converted to a {@code Short} before setting the field value.
		 * </p>
		 * <p>
		 * For array fields whose <em>component</em> type is an interface or abstract class, specify the concrete
		 * <em>component</em> type. For example, a field with type {@code Number[]} may be annotated with
		 * {@code @Option(type=Short.class)} to ensure that option parameter values are converted to {@code Short}
		 * before adding an element to the array.
		 * </p>
		 * <p>
		 * Picocli will use the {@link ITypeConverter} that is {@linkplain #registerConverter(Class, ITypeConverter)
		 * registered} for the specified type to convert the raw String values before modifying the field value.
		 * </p>
		 * <p>
		 * Prior to 2.0, the {@code type} attribute was necessary for {@code Collection} and {@code Map} fields, but
		 * starting from 2.0 picocli will infer the component type from the generic type's type arguments. For example,
		 * for a field of type {@code Map<TimeUnit, Long>} picocli will know the option parameter should be split up in
		 * key=value pairs, where the key should be converted to a {@code java.util.concurrent.TimeUnit} enum value, and
		 * the value should be converted to a {@code Long}. No {@code @Option(type=...)} type attribute is required for
		 * this. For generic types with wildcards, picocli will take the specified upper or lower bound as the Class to
		 * convert to, unless the {@code @Option} annotation specifies an explicit {@code type} attribute.
		 * </p>
		 * <p>
		 * If the field type is a raw collection or a raw map, and you want it to contain other values than Strings, or
		 * if the generic type's type arguments are interfaces or abstract classes, you may specify a {@code type}
		...
		/**
		 * Optionally specify one or more {@link ITypeConverter} classes to use to convert the command line argument
		 * into a strongly typed value (or key-value pair for map fields). This is useful when a particular field should
		 ...
		 * {@link java.sql.Types java.sql.Types} to the {@code int} value of these constants, but any other {@code int}
		 * fields should not be affected by this and should continue to use the standard int converter that parses
		 * numeric values.
		 * </p>
		 * 
		...
		/**
		 * Specify a regular expression to use to split option parameter values before applying them to the field. All
		 * elements resulting from the split are added to the array or Collection. Previously ignored for single-value
		 * fields, from picocli 4.0 a {@code split} regex can only be specified on multi-value options and positional
		 * parameters.
		 * 
		 ...
		 * 
		 * @return a String that (after type conversion) will be used as the value for this option if the option was not
		 ...
		 * is set {@code true} on the command. Use this attribute to specify whether the default value for this specific
		 * option should always be shown or never be shown, regardless of the command setting.
		 * <p>
		 * Note that picocli 3.2 allows {@linkplain #description() embedding default values} anywhere in the description
		...
		/**
		 * Use this attribute to specify an {@code Iterable<String>} class that generates completion candidates for this
		 * option. For map fields, completion candidates should be in {@code key=value} form.
		 * <p>
		 * Completion candidates are used in bash completion scripts generated by the {@code picocli.AutoComplete}
		 * class. Bash has special completion options to generate file names and host names, and the bash completion
		 * scripts generated by {@code AutoComplete} delegate to these bash built-ins for {@code @Options} whose
		 * {@code type} is {@code java.io.File}, {@code java.nio.file.Path} or {@code java.net.InetAddress}.
		 * </p>
		 * <p>
		...
		/**
		 * Set {@code interactive=true} if this option will prompt the end user for a value (like a password). Only
		 * supported for single-value options and {@code char[]} arrays (no collections, maps or other array types).
		 * When running on Java 6 or greater, this will use the {@link Console#readPassword()} API to get a value
		 * without echoing input to the console.
		 * <p>
		 * Best security practice is to use type {@code char[]} instead of {@code String}, and to to null out the array
		 * after use.
		 * </p>
		 * <p>
		 * When defined with {@code arity = "0..1"}, the option can also take a value from the command line. (The user
		 * will still be prompted if no option parameter was specified on the command line.) This is useful for cli that
		 * need to be run interactively as well as in batch mode.
		 * </p>
		 * 
		 ...
		 * {@linkplain Command#resourceBundle() exists for this command}) an attempt is made to find the option
		...
		/**
		 * When {@link Command#sortOptions() @Command(sortOptions = false)} is specified, this attribute can be used to
		 * control the order in which options are listed in the usage help message.
		 * 
		 * @return the position in the options list at which this option should be shown. Options with a lower number
		 *         are shown before options with a higher number. Gaps are allowed.
		 * @since 3.9
		 */
		int order() default -1;

		/**
		 * (Only for boolean options): set this to automatically add a negative version for this boolean option. For
		 * example, for a {@code --force} option the negative version would be {@code --no-force}, and for a
		 * {@code -XX:+PrintGCDetails} option, the negative version would be {@code -XX:-PrintGCDetails}. The synopsis
		 * would show {@code --[no-]force} and {@code -XX:(+|-)PrintGCDetails}, respectively.
		 * <p>
		 * The form of the negative name can be customized by modifying the regular expressions used by
		 * {@linkplain RegexTransformer#createDefault() default}, or by replacing the default
		 * {@link INegatableOptionTransformer} with a custom implementation entirely.
		 * </p>
		 * <p>
		 * Negative option names used to parse the command line are collected when the command is constructed (so any
		 * variables in the option names will be resolved at that time). Documentation strings for negatable options are
		 * generated on demand when the usage help message is shown.
		 * </p>
		 * 
		...
		/**
		 * For options with an optional parameter (for example, {@code arity = "0..1"}), this value is assigned to the
		 * annotated element if the option is specified on the command line without an option parameter.
		 * <p>
		 * This is different from the {@link #defaultValue()}, which is assigned if the option is not specified at all
		 * on the command line.
		 * </p>
		 * <p>
		 ...
		 * </ul>
		 * <p>
		 * This is useful to define options that can function as a boolean "switch" and optionally allow users to
		 * provide a (strongly typed) extra parameter value.
		 * </p>
		 * 
		...
		/**
		 * Optionally specify a custom {@code IParameterConsumer} to temporarily suspend picocli's parsing logic and
		 * process one or more command line arguments in a custom manner. This may be useful when passing arguments
	 ...
	 * </p>
	 * <p>
	 * In the case of command methods (annotated with {@code @Command}), method parameters may be annotated with
	 * {@code @Parameters}, but are are considered positional parameters by default, unless they are annotated with
	 * {@code @Option}.
	 * </p>
	 * <p>
	 * Command class example:
	...
	public @interface Parameters
	{
		 ...
		 * field. For array or Collection fields, you can also specify an index range ("0..3", or "2..*", etc.) to
		 * assign a subset of the command line arguments to this field. The default is "*", meaning all command line
		 * arguments.
		 * 
		...
		/**
		 * Description of the parameter(s), used when generating the usage documentation. Each element of the array is
		 * rendered on a separate line.
		 * <p>
		 * May contain embedded {@linkplain java.util.Formatter format specifiers} like {@code %n} line separators.
		 * Literal percent {@code '%'} characters must be escaped with another {@code %}.
		 ...
		 * <p>
		 * The description may contain variables that are rendered when help is requested. The string
		 * {@code ${DEFAULT-VALUE}} is replaced with the default value of the positional parameter. This is regardless
		 * of the command's {@link Command#showDefaultValues() showDefaultValues} setting or the positional parameter's
		 * {@link #showDefaultValue() showDefaultValue} setting. The string {@code ${COMPLETION-CANDIDATES}} is replaced
		 * with the completion candidates generated by {@link #completionCandidates()} in the description for this
		 * positional parameter. Also, embedded {@code %n} newline markers are converted to actual newlines.
		 * </p>
		 * 
		 ...
		 * </p>
		 * <p>
		 * For single-value parameters, setting {@code arity = "0..1"} makes a positional parameter optional, while
		 * setting {@code arity = "1"} makes it required.
		 * </p>
		 * <p>
		 * Required parameters that are part of a {@linkplain ArgGroup group} are required <em>within the group</em>,
		 * not required within the command: the group's {@linkplain ArgGroup#multiplicity() multiplicity} determines
		 * whether the group itself is required or optional.
		 * </p>
		 * 
		...
		/**
		 * Specify a {@code paramLabel} for the parameter to be used in the usage help message. If omitted, picocli uses
		 * the field name in fish brackets ({@code '<'} and {@code '>'}) by default. Example:
		 * 
		 * <pre>
		 * class Example
		 * {
		 ...
		 * to. This may be useful when the field type is an interface or an abstract class. For example, a field can be
		 * declared to have type {@code java.lang.Number}, and annotating {@code @Parameters(type=Short.class)} ensures
		 * that the positional parameter value is converted to a {@code Short} before setting the field value.
		 * </p>
		 * <p>
		 * For array fields whose <em>component</em> type is an interface or abstract class, specify the concrete
		 * <em>component</em> type. For example, a field with type {@code Number[]} may be annotated with
		 * {@code @Parameters(type=Short.class)} to ensure that positional parameter values are converted to
		 * {@code Short} before adding an element to the array.
		 * </p>
		 * <p>
		 * Picocli will use the {@link ITypeConverter} that is {@linkplain #registerConverter(Class, ITypeConverter)
		 * registered} for the specified type to convert the raw String values before modifying the field value.
		 ...
		 * Prior to 2.0, the {@code type} attribute was necessary for {@code Collection} and {@code Map} fields, but
		 * starting from 2.0 picocli will infer the component type from the generic type's type arguments. For example,
		 * for a field of type {@code Map<TimeUnit, Long>} picocli will know the positional parameter should be split up
		 * in key=value pairs, where the key should be converted to a {@code java.util.concurrent.TimeUnit} enum value,
		 * and the value should be converted to a {@code Long}. No {@code @Parameters(type=...)} type attribute is
		 * required for this. For generic types with wildcards, picocli will take the specified upper or lower bound as
		 * the Class to convert to, unless the {@code @Parameters} annotation specifies an explicit {@code type}
		 * attribute.
		 * </p>
		 * <p>
		 * If the field type is a raw collection or a raw map, and you want it to contain other values than Strings, or
		 ...
		 * All elements resulting from the split are added to the array or Collection. Previously ignored for
		 * single-value fields, from picocli 4.0 a {@code split} regex can only be specified on multi-value options and
		 * positional parameters.
		 * 
		 ...
		 * 
		 * @return a String that (after type conversion) will be used as the value for this positional parameter if no
		...
		/**
		 * Use this attribute to control for a specific positional parameter whether its default value should be shown
		 * in the usage help message. If not specified, the default value is only shown when the
		 * {@link Command#showDefaultValues()} is set {@code true} on the command. Use this attribute to specify whether
		 * the default value for this specific positional parameter should always be shown or never be shown, regardless
		...
		/**
		 * Use this attribute to specify an {@code Iterable<String>} class that generates completion candidates for this
		 * positional parameter. For map fields, completion candidates should be in {@code key=value} form.
		 * <p>
		 * Completion candidates are used in bash completion scripts generated by the {@code picocli.AutoComplete}
		 * class. Unfortunately, {@code picocli.AutoComplete} is not very good yet at generating completions for
		 * positional parameters.
		 * </p>
		 *
		...
		/**
		 * Set {@code interactive=true} if this positional parameter will prompt the end user for a value (like a
		 * password). Only supported for single-value positional parameters (not arrays, collections or maps). When
		 * running on Java 6 or greater, this will use the {@link Console#readPassword()} API to get a value without
		 * echoing input to the console.
		 * 
		...
		/**
		 * ResourceBundle key for this option. If not specified, (and a ResourceBundle
		 * {@linkplain Command#resourceBundle() exists for this command}) an attempt is made to find the positional
		 * parameter description using {@code paramLabel() + "[" + index() + "]"} as key.
		 *
		...
		/**
		 * Optionally specify a custom {@code IParameterConsumer} to temporarily suspend picocli's parsing logic and
		 * process one or more command line arguments in a custom manner.
		 * 
		 * @since 4.0
		 */
		Class<? extends IParameterConsumer> parameterConsumer() default NullParameterConsumer.class;
	...
	/**
	 * <p>
	 * Fields annotated with {@code @ParentCommand} will be initialized with the parent command of the current
	 * subcommand. If the current command does not have a parent command, this annotation has no effect.
	 * </p>
	 * <p>
	 * Parent cli often define options that apply to all the subcommands. This annotation offers a convenient way to
	 * inject a reference to the parent command into a subcommand, so the subcommand can access its parent options. For
	 * example:
	 * </p>
	 * 
	 * <pre>
	...
	/**
	 * Fields annotated with {@code @Unmatched} will be initialized with the list of unmatched command line arguments,
	 * if any. If this annotation is found, picocli automatically sets
	 * {@linkplain CommandLine#setUnmatchedArgumentsAllowed(boolean) unmatchedArgumentsAllowed} to {@code true}.
	 * 
	 ...
	 * {@link Parameters @Parameters} in the mixin class are added to the options and positional parameters of this
	 * command. A {@link DuplicateOptionAnnotationsException} is thrown if any of the options in the mixin has the same
	 * name as an option in this command.
	 * </p>
	 * <p>
	 * The {@code Mixin} annotation provides a way to reuse common options and parameters without subclassing. For
	 * example:
	 * </p>
	 * 
	 * <pre>
	...
	{
		/**
		 * Optionally specify a name that the mixin object can be retrieved with from the {@code CommandSpec}. If not
		 * specified the name of the annotated field is used.
		 * 
		 * @return a String to register the mixin object with, or an empty String if the name of the annotated field
		 *         should be used
		 */
		String name() default "";
	}
	...
	/**
	 * Fields annotated with {@code @Spec} will be initialized with the {@code CommandSpec} for the command the field is
	 * part of. Example usage:
	 * 
	 * <pre>
	...
	public @interface Command
	{
		 ...
		 * {@linkplain #subcommands() declaratively added} subcommands, this attribute is also used by the parser to
		 * recognize subcommands in the command line arguments.
		 * 
		...
		/**
		 * A list of classes to instantiate and register as subcommands. When registering subcommands declaratively like
		 * this, you don't need to call the {@link CommandLine#addSubcommand(String, Object)} method. For example, this:
		 * 
		 * <pre>
		 * &#064;Command(subcommands = {
		 *         GitStatus.class,
		 ...
		 * enclosing {@code @Command} class. The default is {@code true}. For example:
		 * 
		 * <pre>
		 * &#064;Command
		 * public class Git {
		 ...
		 * 
		 * @return the string that separates options from option parameters, used both when parsing and when generating
		 *         usage help
		 ...
		 * {@linkplain Option#versionHelp() option} to request version help. Each element of the array is rendered on a
		 * separate line.
		 * <p>
		 * May contain embedded {@linkplain java.util.Formatter format specifiers} like {@code %n} line separators.
		 * Literal percent {@code '%'} characters must be escaped with another {@code %}.
		 ...
		 * </p>
		 *
		 * @return a string or an array of strings with version information about this command (each string in the array
		...
		/**
		 * Adds the standard {@code -h} and {@code --help} {@linkplain Option#usageHelp() usageHelp} options and
		 * {@code -V} and {@code --version} {@linkplain Option#versionHelp() versionHelp} options to the options of this
		 * command.
		 * <p>
		 * Note that if no {@link #version()} or {@link #versionProvider()} is specified, the {@code --version} option
		 ...
		 * parameters of the parent command should not be validation. If a subcommand marked as {@code helpCommand} is
		 * specified on the command line, picocli will not validate the parent arguments (so no "missing required
		 * option" errors) and the {@link CommandLine#printHelpIfRequested(List, PrintStream, PrintStream, Help.Ansi)}
		...
		/**
		 * Optional summary description of the command, shown before the synopsis. Each element of the array is rendered
		 * on a separate line.
		 * <p>
		 * May contain embedded {@linkplain java.util.Formatter format specifiers} like {@code %n} line separators.
		 * Literal percent {@code '%'} characters must be escaped with another {@code %}.
		 ...
		 * between the heading and the synopsis text).
		 * <p>
		 * May contain embedded {@linkplain java.util.Formatter format specifiers} like {@code %n} line separators.
		 * Literal percent {@code '%'} characters must be escaped with another {@code %}.
		 * </p>
		 ...
		 * {@code "<main> [OPTIONS] [PARAMETERS...] [COMMAND]"}. By default, a detailed synopsis with individual option
		 * names and parameters is generated.
		 * 
		...
		/**
		 * Specify one or more custom synopsis lines to display instead of an auto-generated synopsis. Each element of
		 * the array is rendered on a separate line.
		 * <p>
		 * May contain embedded {@linkplain java.util.Formatter format specifiers} like {@code %n} line separators.
		 * Literal percent {@code '%'} characters must be escaped with another {@code %}.
		 * </p>
		...
		/**
		 * Optional text to display between the synopsis line(s) and the list of options. Each element of the array is
		 * rendered on a separate line.
		 * <p>
		 * May contain embedded {@linkplain java.util.Formatter format specifiers} like {@code %n} line separators.
		 * Literal percent {@code '%'} characters must be escaped with another {@code %}.
		...
		/**
		 * Class that can provide default values dynamically at runtime. An implementation may return default value
		 * obtained from a configuration file like a properties file or some other source.
		 * 
		...
		/**
		 * Specify {@code true} to show default values in the description column of the options list (except for boolean
		...
		/**
		 * Set the heading preceding the subcommands list. The default heading is {@code "Commands:%n"} (with a line
		 * break at the end).
		 * <p>
		 * May contain embedded {@linkplain java.util.Formatter format specifiers} like {@code %n} line separators.
		 * Literal percent {@code '%'} characters must be escaped with another {@code %}.
		...
		/**
		 * Set {@code hidden=true} if this command should not be included in the list of cli in the usage help of the
		 * parent command.
		 * 
		...
		/**
		 * If {@code true}, picocli will attempt to detect the terminal width and adjust the usage help message
		 * accordingly. End users may enable this by setting system property {@code "picocli.usage.width"} to
		 * {@code AUTO}, and may disable this by setting this system property to a {@linkplain UsageMessageSpec#width()
		 * numeric value}. This feature requires Java 7 or greater. The default is {@code false}
		 * 
		 * @see UsageMessageSpec#autoWidth()
		 * @since 4.0
		 ...
		 * {@value CommandLine.ExitCode#OK} by default.
		 * 
		 * @see #execute(String...)
		 * @since 4.0
		 */
		...
		/**
		 * Exit code signifying that an exception occurred when invoking the Runnable, Callable or Method user object of
		 * a command. {@value CommandLine.ExitCode#SOFTWARE} by default.
		 * 
		 * @see #execute(String...)
		 * @since 4.0
		 */
		...
		/**
		 * Set the heading preceding the exit codes section, may contain {@code "%n"} line separators. {@code ""} (empty
		 * string) by default.
		 * 
		...
		/**
		 * Set the values to be displayed in the exit codes section as a list of {@code "key:value"} pairs: keys are
		 * exit codes, values are descriptions. Descriptions may contain {@code "%n"} line separators.
		 * <p>
		 * For example:
		 * </p>
		 * 
	...
	/**
	 * A {@code Command} may define one or more {@code ArgGroups}: a group of options, positional parameters or a
	 * mixture of the two. Groups can be used to:
	 * <ul>
	 * <li>define <b>mutually exclusive</b> arguments. By default, options and positional parameters in a group are
	 * mutually exclusive. This can be controlled with the {@link #exclusive() exclusive} attribute. Picocli will throw
	 * a {@link MutuallyExclusiveArgsException} if the command line contains multiple arguments that are mutually
	 * exclusive.</li>
	 * <li>define a set of arguments that <b>must co-occur</b>. Set {@link #exclusive() exclusive = false} to define a
	 * group of options and positional parameters that must always be specified together. Picocli will throw a
	 * {@link MissingParameterException MissingParameterException} if not all the options and positional parameters in a
	 * co-occurring group are specified together.</li>
	 * <li>create an <b>option section</b> in the usage help message. To be shown in the usage help message, a group
	 * needs to have a {@link #heading() heading} (which may come from a {@linkplain #headingKey() resource bundle}).
	 * Groups without a heading are only used for validation. Set {@link #validate() validate = false} for groups whose
	 * purpose is only to customize the usage help message.</li>
	 * <li>define <b>composite repeating argument groups</b>. Groups may contain other groups to create composite
	 * groups.</li>
	 * </ul>
	 * <p>
	 * Groups may be optional ({@code multiplicity = "0..1"}), required ({@code multiplicity = "1"}), or repeating
	 * groups ({@code multiplicity = "0..*"} or {@code multiplicity = "1..*"}). For a group of mutually exclusive
	 * arguments, making the group required means that one of the arguments in the group must appear on the command
	 * line, or a {@link MissingParameterException MissingParameterException} is thrown. For a group of co-occurring
	 * arguments, all arguments in the group must appear on the command line.
	 * </p>
	 * <p>
	 ...
	 * </ul>
	 * <p>
	 * Below is an example of an {@code ArgGroup} defining a set of dependent options that must occur together. All
	 * options are required <em>within the group</em>, while the group itself is optional:
	 * </p>
	 * 
	 * <pre>
	...
	{
		/**
		 * The heading of this group, used when generating the usage documentation. When neither a {@link #heading()
		 * heading} nor a {@link #headingKey() headingKey} are specified, this group is used for validation only and
		 * does not change the usage help message.
		 */
		String heading() default "__no_heading__";

		/**
		 * ResourceBundle key for this group's usage help message section heading. When neither a {@link #heading()
		 * heading} nor a {@link #headingKey() headingKey} are specified, this group is used for validation only and
		 * does not change the usage help message.
		 */
		...
		/**
		 * Determines whether this is a mutually exclusive group; {@code true} by default. If {@code false}, this is a
		 * co-occurring group. Ignored if {@link #validate()} is {@code false}.
		 */
		 ...
		 * For a group of co-occurring arguments, making the group required means that all arguments in the group must
		 * appear on the command line. Ignored if {@link #validate()} is {@code false}.
		 */
		String multiplicity() default "0..1";

		/**
		 * Determines whether picocli should validate the rules of this group ({@code true} by default). For a mutually
		 * exclusive group validation means verifying that no more than one elements of the group is specified on the
		 * command line; for a co-ocurring group validation means verifying that all elements of the group are specified
		 * on the command line. Set {@link #validate() validate = false} for groups whose purpose is only to customize
		 ...
		 * Groups with a lower number are shown before groups with a higher number. This attribute is only honored for
		 * groups that have a {@link #heading() heading} (or a {@link #headingKey() headingKey} with a non-{@code null}
	...
	/**
	 * <p>
	 * When parsing command line arguments and initializing fields annotated with {@link Option @Option} or
	 * {@link Parameters @Parameters}, String values can be converted to any type for which a {@code ITypeConverter} is
	 * registered.
	 * </p>
	 * <p>
	 * This interface defines the contract for classes that know how to convert a String into some domain object. Custom
	 * converters can be registered with the {@link #registerConverter(Class, ITypeConverter)} method.
	 * </p>
	 * <p>
	 * Java 8 lambdas make it easy to register custom type converters:
	 * </p>
	...
	public interface IDefaultValueProvider
	{
		...
		/**
		 * Returns the default value for an option or positional parameter or {@code null}. The returned value is
		 * converted to the type of the option/positional parameter via the same type converter used when populating
		 * this option/positional parameter from a command line argument.
		 * 
		 * @param argSpec
		 *            the option or positional parameter, never {@code null}
		 * @return the default value for the option or positional parameter, or {@code null} if this provider has no
		 *         default value for the specified option or positional parameter
		 * @throws Exception
	...
	/**
	 * Options or positional parameters can be assigned a {@code IParameterConsumer} that implements custom logic to
	 * process the parameters for this option or this position. When an option or positional parameter with a custom
	 * {@code IParameterConsumer} is matched on the command line, picocli's internal parser is temporarily suspended,
	 * and this object becomes responsible for consuming and processing as many command line arguments as needed.
	 * <p>
	...
	public interface IParameterConsumer
	{
		 ...
		 * Implementors are free to ignore the {@linkplain ArgSpec#arity() arity} of the option or positional parameter,
		 * they are free to consume arguments that would normally be matched as other options of the command, and they
		 * are free to consume arguments that would normally be matched as an end-of-options delimiter.
		 * <p>
		 * Implementors are responsible for saving the consumed values; if the user object of the option or positional
		 * parameter is a Collection or a Map, a common approach would be to obtain the current instance via the
		 * {@link ArgSpec#getValue()}, and add to this instance. If the user object is an array, the implementation
		 * would need to create a new array that contains the old values as well as the newly consumed values, and store
		 * this array in the user object via the {@link ArgSpec#setValue(Object)}.
		 * </p>
		 * <p>
		 * If the user input is invalid, implementations should throw a {@link ParameterException} with a message to
		 * display to the user.
		 * </p>
		 * <p>
		 ...
		 *            the command line arguments
		 * @param argSpec
		 *            the option or positional parameter for which to consume command line arguments
		 * @param commandSpec
	...
	/**
	 * Determines the option name transformation of {@linkplain Option#negatable() negatable} boolean options. Making an
	 * option negatable has two aspects:
	 * <ul>
	 ...
	 * </ul>
	 * <p>
	 * Additionally, this transformer controls which names of a negatable option are actually negatable: for example, by
	 * default short options like {@code -v} do not have a negative form, even if the same option's long form,
	...
	{
		/**
		 * Returns the negative form of the specified option name for the parser to recognize when parsing command line
		 * arguments.
		 * 
		 * @param optionName
		 *            the option name to create a negative form for, for example {@code --force}
		 * @param cmd
		 *            the command that the option is part of
		...
		/**
		 * Returns the documentation string to show in the synopsis and usage help message for the specified option. The
		 * returned value should be concise and clearly suggest that both the positive and the negative form are valid
		 * option names
		 * 
		 * @param optionName
		 *            the option name to create a documentation string for, for example {@code --force}, or
		 *            {@code -XX:+<option>}
		 * @param cmd
		 *            the command that the option is part of
		 * @return the documentation string for the negatable option, for example {@code --[no-]force}, or
		 *         {@code -XX:(+|-)<option>}
		 */
	 ...
	 * <p>
	 * A common way to negate GNU *nix long options is to prefix them with {@code "no-"}, so for a {@code --force}
	 * option the negative version would be {@code --no-force}. Java has the {@code -XX:[+|-]} JVM options, where
	 * "Boolean options are turned on with {@code -XX:+<option>} and turned off with {@code -XX:-<option>}". These are
	 * the negative forms {@linkplain #createDefault() supported by default} by this class.
	 * </p>
	 * <p>
	 * See the {@link CommandLine.RegexTransformer.Builder} for an example of customizing this to create negative forms
	 * for short options.
	 * </p>
	 * 
	 * @since 4.0
	...
	public static class RegexTransformer implements INegatableOptionTransformer
	{
		...
		public static class Builder
		{
			 ...
			 * @param negativeReplacement
			 *            the replacement to use to generate a {@linkplain #makeNegative(String, CommandSpec) negative
			 *            name} when the option name matches
			 * @param synopsisReplacement
			 *            the replacement to use to generate a {@linkplain #makeSynopsis(String, CommandSpec)
	...
	public interface IHelpFactory
	{
		 ...
		 * 
		 * @param commandSpec
		 *            the command to create usage help for
		 * @param colorScheme
		 *            the color scheme to use when rendering usage help
	 ...
	 * {@link Command#subcommands()}, {@link Option#converter()}, {@link Parameters#converter()} and
	 ...
	 * </p>
	 * <p>
	 * You may provide a custom implementation of this interface. For example, a custom factory implementation could
	 * delegate to a dependency injection container that provides the requested instance.
	 * </p>
	 * 
	...
	/**
	 * Returns the default {@link IFactory} implementation used if no factory was specified in the
	 * {@link #CommandLine(Object) CommandLine constructor}.
	 * 
	 * @since 4.0
	 */
	...
	private static class DefaultFactory implements IFactory
	{
		public <T> T create(Class<T> cls) throws Exception
		{
			...
			{
				try
				{ // if it is an implementation class, instantiate it
	...
	public static class Range implements Comparable<Range>
	{
		 ...
		 * @param unspecified
		 *            {@code true} if no arity was specified on the option/parameter (value is based on type)
		 * @param originalValue
		...
		/**
		 * Returns the default arity {@code Range}: for interactive options/positional parameters, this is 0; for
		 * {@link Option options} this is 0 for booleans and 1 for other types, for {@link Parameters parameters}
		 * booleans have arity 0, arrays or Collections have arity "0..*", and other types have arity 1.
		 * 
		 * @param field
		...
		private static Range defaultArity(IAnnotatedElement member)
		{
			...
			{
				boolean zeroArgs = info.isBoolean()
		...
		/**
		 * Returns the default arity {@code Range} for {@link Option options}: booleans have arity 0, other types have
		 * arity 1.
		 * 
		 * @param type
		...
		/**
		 * Leniently parses the specified String as an {@code Range} value and return the result. A range string can be
		 * a fixed integer value or a range of the form {@code MIN_VALUE + ".." + MAX_VALUE}. If the {@code MIN_VALUE}
		 * string is not numeric, the minimum is zero. If the {@code MAX_VALUE} is not numeric, the range is taken to be
		...
		/**
		 * Returns a new Range object with the {@code min} value replaced by the specified value. The {@code max} of the
		 * returned Range is guaranteed not to be less than the new {@code min} value.
		 * 
		 * @param newMin
		...
		/**
		 * Returns a new Range object with the {@code max} value replaced by the specified value. The {@code min} of the
		 * returned Range is guaranteed not to be greater than the new {@code max} value.
		 * 
		 * @param newMax
		...
		/**
		 * Returns {@code true} if this range contains variables that have not been expanded yet, {@code false} if this
		 * Range does not contain any variables.
		 * 
		 * @since 4.0
		 */
		...
		/**
		 * Returns the upper bound of this range (inclusive), or {@code Integer.MAX_VALUE} if this range has
		 * {@linkplain #isVariable() no upper bound}.
		 * 
		 * @since 4.0
		 */
		public int max()
		 ...
		 * @param value
		 *            the value to check
		 * @return {@code true} if the specified value is not less than the minimum and not greater than the maximum of
		 *         this Range
		 */
	...
	private static void validatePositionalParameters(List<PositionalParamSpec> positionalParametersFields)
	{
		...
		for (PositionalParamSpec positional : positionalParametersFields)
		{
			...
			if (index.min > min)
			{
				throw new ParameterIndexGapException("Command definition should have a positional parameter with index="
						+ min + ". Nearest positional parameter '" + positional.paramLabel() + "' has index="
						+ index.min);
			}
	...
	/**
	 * This class provides a namespace for classes and interfaces that config concepts and attributes of command line
	 * interfaces in picocli.
	 * 
	 * @since 3.0
	 */
	public static final class Model
	{
		...
		/**
		 * The scope of a binding is the context where the current value should be gotten from or set to. For a field,
		 * the scope is the object whose field value to get/set. For a method binding, it is the object on which the
		 * method should be invoked.
		 * <p>
		 * The getter and setter of the scope allow you to change the object onto which the option and positional
		 * parameter getters and setters should be applied.
		 * </p>
		 * 
		 * @since 4.0
		 */
		...
		/**
		 * Customizable getter for obtaining the current value of an option or positional parameter. When an option or
		 * positional parameter is matched on the command line, its getter or setter is invoked to capture the value.
		 * For example, an option can be bound to a field or a method, and when the option is matched on the command
		 * line, the field's value is set or the method is invoked with the option parameter value.
		 * 
		 * @since 3.0
		 */
		...
		{
			/**
			 * Returns the current value of the binding. For multi-value options and positional parameters, this method
			 * returns an array, collection or map to add values to.
			 * 
			 * @throws PicocliException
			 *             if a problem occurred while obtaining the current value
			 * @throws Exception
			 *             internally, picocli call sites will catch any exceptions thrown from here and rethrow them
			 *             wrapped in a PicocliException
			 */
			<T> T get() throws Exception;
		}

		/**
		 * Customizable setter for modifying the value of an option or positional parameter. When an option or
		 * positional parameter is matched on the command line, its setter is invoked to capture the value. For example,
		 * an option can be bound to a field or a method, and when the option is matched on the command line, the
		...
		/**
		 * The {@code CommandSpec} class models a command specification, including the options, positional parameters
		 * and subcommands supported by the command, as well as attributes for the version help message and the usage
		 * help message of the command.
		 * <p>
		 * Picocli views a command line application as a hierarchy of cli: there is a top-level command (usually the
		 * Java class with the {@code main} method) with optionally a set of command line options, positional parameters
		 * and subcommands. Subcommands themselves can have options, positional parameters and nested sub-subcommands to
		 * any level of depth.
		 * </p>
		 * <p>
		 * The object config has a corresponding hierarchy of {@code CommandSpec} objects, each with a set of
		 * {@link OptionSpec}, {@link PositionalParamSpec} and {@linkplain CommandLine subcommands} associated with it.
		 * This object config is used by the picocli command line interpreter and help message generator.
		 * </p>
		 * <p>
		 * Picocli can construct a {@code CommandSpec} automatically from classes with {@link Command @Command},
		 * {@link Option @Option} and {@link Parameters @Parameters} annotations. Alternatively a {@code CommandSpec}
		...
		public static class CommandSpec
		{
			...
			/**
			 * Constant Boolean holding the default setting for whether method cli should be added as subcommands:
			 * <code>{@value}</code>.
			 */
			static final Boolean DEFAULT_IS_ADD_METHOD_SUBCOMMANDS = true;

			/**
			 * Constant Boolean holding the default setting for whether variables should be interpolated in String
			...
			/**
			 * Creates and returns a new {@code CommandSpec} with the specified associated user object. The specified
			 * user object is <em>not</em> inspected for annotations.
			 * 
			 * @param userObject
			...
			/**
			 * Creates and returns a new {@code CommandSpec} initialized from the specified associated user object. The
			 * specified user object must have at least one {@link Command}, {@link Option} or {@link Parameters}
			 * annotation.
			 * 
			 * @param userObject
			 *            the user object annotated with {@link Command}, {@link Option} and/or {@link Parameters}
			 *            annotations.
			 * @throws InitializationException
			 ...
			 * @param factory
			 *            the factory used to create instances of {@linkplain Command#subcommands() subcommands},
			 *            {@linkplain Option#converter() converters}, etc., that are registered declaratively with
			...
			/**
			 * Creates and returns a new {@code CommandSpec} initialized from the specified associated user object. If
			 * the specified user object has no {@link Command}, {@link Option} or {@link Parameters} annotations, an
			 * empty {@code CommandSpec} is returned.
			 * 
			 * @param userObject
			 *            the user object annotated with {@link Command}, {@link Option} and/or {@link Parameters}
			 *            annotations.
			...
			/** Returns the CommandLine constructed with this {@code CommandSpec} config. */
			public CommandLine commandLine()
			{
				return commandLine;
			}

			/** Sets the CommandLine constructed with this {@code CommandSpec} config. */
			...
			/**
			 * Initializes the parser specification for this command from the specified settings and returns this
			 * commandSpec.
			 */
			...
			/**
			 * Initializes the usageMessage specification for this command from the specified settings and returns this
			 * commandSpec.
			 */
			...
			/**
			 * Initializes the resource bundle for this command: sets the {@link UsageMessageSpec#messages(Messages)
			 * UsageMessageSpec.messages} to a {@link Messages Messages} object created from this command spec and the
			 * specified bundle, and then sets the {@link ArgSpec#messages(Messages) ArgSpec.messages} of all options
			 * and positional parameters in this command to the same {@code Messages} instance. Subcommands are not
			 * modified.
			 * <p>
			 ...
			 *            the base name of the ResourceBundle to set, may be {@code null}
			...
			{
				ResourceBundle bundle = empty(resourceBundleBaseName) ? null
			...
			/**
			 * Adds the specified subcommand with the specified name. If the specified subcommand does not have a
			 * ResourceBundle set, it is initialized to the ResourceBundle of this command spec.
			 * 
			 * @param name
			 *            subcommand name - the preferred subcommand name to register the subcommand under. If
			 *            {@code null}, the {@linkplain CommandSpec#name() name} of the specified subcommand is used; if
			 *            this is also {@code null}, the first {@linkplain CommandSpec#aliases() alias} is used. When
			 *            this String is encountered in the command line arguments, the subcommand is invoked.
			 * @param subcommand
			 *            describes the subcommand to envoke when the name is encountered on the command line
			 * @return this {@code CommandSpec} object for method chaining
			 * @throws InitializationException
			 *             if the specified name is {@code null}, and no alternative name could be found, or if another
			 *             subcommand was already registered under the same name, or if one of the aliases of the
			 *             specified subcommand was already used by another subcommand.
			 */
			 ...
			 * @param name
			 *            subcommand name - the preferred subcommand name to register the subcommand under. If
			 *            {@code null}, the {@linkplain CommandLine#getCommandName() name} of the specified subcommand
			 *            is used; if this is also {@code null}, the first {@linkplain CommandSpec#aliases() alias} is
			 *            used. When this String is encountered in the command line arguments, the subcommand is
			 *            invoked.
			...
			public CommandSpec addSubcommand(String name, CommandLine subCommandLine)
			{
				...
				{
					throw new InitializationException("Another subcommand named '" + actualName
				...
				for (String alias : subSpec.aliases())
				{
					if (t.isDebug())
					{
						t.debug("Adding alias '%s' for subcommand '%s' to '%s'%n", alias, actualName,
								this.qualifiedName());
					}
			...
			{
				String result = name == null ? subSpec.name : name; // NOTE: check subSpec.name field, not
																	// subSpec.name()!
				...
				{
					throw new InitializationException(
							"Cannot add subcommand with null name to " + this.qualifiedName());
				}
				return result;
			}
			...
			private void initCommandHierarchyWithResourceBundle(String bundleBaseName, ResourceBundle rb)
			{
				...
				for (CommandLine sub : commands.values())
				{ // percolate down the hierarchy
					sub.getCommandSpec().initCommandHierarchyWithResourceBundle(bundleBaseName, rb);
				}
			}

			/**
			 * Returns whether method cli should be added as subcommands. True by default. Used by the annotation
			 * processor.
			 * 
			 * @since 4.0
			 */
			...
			/**
			 * Sets whether method cli should be added as subcommands. True by default. Used by the annotation
			 * processor.
			 * 
			 * @since 4.0
			 */
			 ...
			 * 
			 * @param factory
			 *            the factory used to create instances of subcommands, converters, etc., that are registered
			...
			/**
			 * Adds the specified option spec or positional parameter spec to the list of configured arguments to
			 * expect.
			 * 
			 * @param arg
			...
			/**
			 * Adds the specified option spec to the list of configured arguments to expect. The option's
			 * {@linkplain OptionSpec#description()} may now return Strings from this CommandSpec's
			 * {@linkplain UsageMessageSpec#messages() messages}. The option parameter's
			 * {@linkplain OptionSpec#defaultValueString()} may now return Strings from this CommandSpec's
			...
			/**
			 * Adds the specified positional parameter spec to the list of configured arguments to expect. The
			 * positional parameter's {@linkplain PositionalParamSpec#description()} may now return Strings from this
			 * CommandSpec's {@linkplain UsageMessageSpec#messages() messages}. The positional parameter's
			 * {@linkplain PositionalParamSpec#defaultValueString()} may now return Strings from this CommandSpec's
			 * {@link CommandSpec#defaultValueProvider()} IDefaultValueProvider}.
			 * 
			 * @param positional
			...
			private CommandSpec addArg(ArgSpec arg)
			{
				...
				if (arg.required() && arg.group() == null)
				{
			...
			/**
			 * (INCUBATING) Removes the specified option spec or positional parameter spec from the list of configured
			 * arguments to expect.
			 * 
			 * @param arg
			 *            the option spec or positional parameter spec to remove
			 * @return this CommandSpec for method chaining
			 * @throws UnsupportedOperationException
			 *             if the specified ArgSpec is part of a {@link ArgGroupSpec}
			 * @throws NoSuchElementException
			 *             if the specified ArgSpec is not part of this {@code CommandSpec}
			 * @since 4.0
			 */
			 ...
			 * 
			 * @param group
			 *            the group spec to add
			 * @return this CommandSpec for method chaining
			 * @throws InitializationException
			 *             if the specified group or one of its {@linkplain ArgGroupSpec#parentGroup() ancestors} has
			 *             already been added
			 * @since 4.0
			 */
			 ...
			 * @param mixin
			 *            the mixin whose options and positional parameters and other attributes to add to this command
			 * @return this CommandSpec for method chaining
			 */
			public CommandSpec addMixin(String name, CommandSpec mixin)
			{
				...
				Set<PositionalParamSpec> positionals = new LinkedHashSet<PositionalParamSpec>(
			...
			/**
			 * Adds the specified {@code UnmatchedArgsBinding} to the list of config objects to capture unmatched
			...
			/**
			 * Adds the specified {@code {@literal @}Spec}-annotated program element to the list of elements for this
			 * command.
			 * 
			 * @return this CommandSpec for method chaining
			 * @since 4.0
			...
			/**
			 * Adds the specified {@code {@literal @}ParentCommand}-annotated program element to the list of elements
			 * for this command.
			 * 
			 * @return this CommandSpec for method chaining
			 * @since 4.0
			...
			/**
			 * Returns a map of the short (single character) option names to option spec objects configured for this
			 * command.
			 * 
			 * @return an immutable map of options that this command recognizes.
			 */
			...
			/**
			 * Returns the list of required options and positional parameters configured for this command. This does not
			 * include options and positional parameters that are part of a {@linkplain ArgGroupSpec group}.
			 * 
			 ...
			 * each {@code UnmatchedArgsBinding} captures the arguments that could not be matched to any options or
			 * positional parameters.
			 */
			...
			/**
			 * Returns the list of program elements annotated with {@code {@literal @}ParentCommand} configured for this
			 * command.
			 * 
			 * @since 4.0
			 */
			 ...
			 * {@link #name() name}, preceded by the qualified name of the parent command, if any, separated by a space.
			 * 
			 * @return {@link #DEFAULT_COMMAND_NAME} by default, initialized from {@link Command#name()} and the parent
			 *         command if defined.
			 * @since 3.0.1
			 */
			...
			/**
			 * Returns this command's fully qualified name, which is its {@link #name() name}, preceded by the qualified
			 * name of the parent command, if this command has a parent command.
			 * 
			 * @return {@link #DEFAULT_COMMAND_NAME} by default, initialized from {@link Command#name()} and the parent
			 *         command if any.
			 * @param separator
			 *            the string to put between the names of the cli in the hierarchy
			 * @since 3.6
			 */
			 ...
			 * {@linkplain OptionSpec#versionHelp() option} to request version help. This is not part of the usage help
			 * message.
			 * 
			 * @return the version strings generated by the {@link #versionProvider() version provider} if one is set,
			 *         otherwise the {@linkplain #version(String...) version literals}
			 */
			 ...
			 * 
			 * @return the version provider or {@code null} if the version strings should be returned from the
			...
			/**
			 * Returns whether this subcommand is a help command, and required options and positional parameters of the
			 * parent command should not be validation.
			 * 
			 * @return {@code true} if this subcommand is a help command and picocli should not check for missing
			 *         required options and positional parameters on the parent command
			...
			/**
			 * Returns exit code for successful termination. {@value CommandLine.ExitCode#OK} by default, may be set
			 * programmatically or via the {@link Command#exitCodeOnSuccess() exitCodeOnSuccess} annotation.
			 * 
			 * @see #execute(String...)
			 * @since 4.0
			 */
			 ...
			 * {@value CommandLine.ExitCode#OK} by default, may be set programmatically or via the
			...
			/**
			 * Returns exit code for command line usage error. {@value CommandLine.ExitCode#USAGE} by default, may be
			 * set programmatically or via the {@link Command#exitCodeOnInvalidInput() exitCodeOnInvalidInput}
			 * annotation.
			 * 
			 * @see #execute(String...)
			 * @since 4.0
			...
			/**
			 * Returns exit code signifying that an exception occurred when invoking the Runnable, Callable or Method
			 * user object of a command. {@value CommandLine.ExitCode#SOFTWARE} by default, may be set programmatically
			 * or via the {@link Command#exitCodeOnExecutionException() exitCodeOnExecutionException} annotation.
			 * 
			 * @see #execute(String...)
			 * @since 4.0
			 */
			...
			/**
			 * Returns {@code true} if the standard help options have been mixed in with this command, {@code false}
			 * otherwise.
			 */
			...
			public String toString()
			{
				return toString == null ? String.valueOf(userObject) : toString;
			}
			 ...
			 * {@linkplain OptionSpec#versionHelp() option} to request version help. Only used if no
			 * {@link #versionProvider() versionProvider} is set.
			 * 
			 * @return this CommandSpec for method chaining
			 */
			 ...
			 *            the version provider to use to generate the version strings, or {@code null} if the
			 *            {@linkplain #version(String...) version literals} should be used.
			 * @return this CommandSpec for method chaining
			 */
			...
			/**
			 * Sets exit code signifying that an exception occurred when invoking the Runnable, Callable or Method user
			 * object of a command. {@value CommandLine.ExitCode#SOFTWARE} by default.
			 * 
			 * @see #execute(String...)
			 * @since 4.0
			 */
			...
			/**
			 * Sets the {@code INegatableOptionTransformer} used to create the negative form of
			 * {@linkplain Option#negatable() negatable} options.
			 * 
			 * @see Option#negatable()
			 * @since 4.0
			...
			/**
			 * Updates the following attributes from the specified {@code @Command} annotation: aliases,
			 * {@link ParserSpec#separator() parser separator}, command name, version, help command, version provider,
			 * default provider and {@link UsageMessageSpec usage message spec}.
			 * 
			 * @param cmd
			 *            the {@code @Command} annotation to get attribute values from
			 * @param factory
			 *            factory used to instantiate classes
			 * @since 3.7
			 */
			...
			/**
			 * Returns the option with the specified short name, or {@code null} if no option with that name is defined
			 * for this command.
			 */
			...
			/**
			 * Returns the option with the specified name, or {@code null} if no option with that name is defined for
			 * this command.
			 * 
			 * @param name
			...
			boolean resemblesOption(String arg, Tracer tracer)
			{
				if (parser().unmatchedOptionsArePositionalParams())
				{
					...
					{
						tracer.debug("Parser is configured to treat all unmatched options as positional parameter%n",
								arg);
					}
					return false;
				}
				if (arg.length() == 1)
				{
					...
					{
						tracer.debug(
								"Single-character arguments that don't match known options are considered positional parameters%n",
								arg);
					}
					return false;
				}
				...
				boolean result = count > 0 && count * 10 >= optionsMap().size() * 9; // at least one prefix char in
																						// common with 9 out of 10
																						// options
				if (tracer != null && tracer.isDebug())
				{
		 ...
		 * </p>
		 * <ul>
		 * <li>Change the text of the predefined sections (this may also be done declaratively using the
		 * annotations)</li>
		 ...
		 * {@link #description()} and {@link #description(String...)} or {@link #header()} and
		 * {@link #header(String...)}.
		 * </p>
		 * <p>
		 * Changing the section order, or adding custom sections can be accomplished with {@link #sectionKeys(List)} and
		 * {@link #sectionMap(Map)}. This gives complete freedom on how a usage help message section is rendered, but it
		 * also means that the {@linkplain IHelpSectionRenderer section renderer} is responsible for all aspects of
		 * rendering the section, including layout and emitting ANSI escape codes. The {@link Help.TextTable} and
		 * {@link Help.Ansi.Text} classes, and the {@link CommandLine.Help.Ansi#string(String)} and
		 ...
		 * 	sectionMap.put(SECTION_KEY_SYNOPSIS, help -> help.synopsis(help.synopsisHeadingLength())); // e.g. <cmd>
		 * 																								// [OPTIONS] <subcmd>
		 * 																								// [COMMAND-OPTIONS]
		 * 																								// [ARGUMENTS]
		 * 	sectionMap.put(SECTION_KEY_DESCRIPTION_HEADING, help -> help.descriptionHeading()); // e.g. %nDescription:%n%n
		 * 	sectionMap.put(SECTION_KEY_DESCRIPTION, help -> help.description()); // e.g. {"Converts foos to bars.", "Use
		 ...
		 * 	sectionMap.put(SECTION_KEY_OPTION_LIST, help -> help.optionList()); // e.g. -h, --help displays this help and
		 * 																		// exits
		...
		public static class UsageMessageSpec
		{
			...
			/**
			 * {@linkplain #sectionKeys() Section key} to {@linkplain #sectionMap() control} the
			 * {@linkplain IHelpSectionRenderer section renderer} for the Header Heading section. The default renderer
			 * for this section calls {@link Help#headerHeading(Object...)}.
			 * 
			 * @since 3.9
			 */
			...
			/**
			 * {@linkplain #sectionKeys() Section key} to {@linkplain #sectionMap() control} the
			 * {@linkplain IHelpSectionRenderer section renderer} for the Header section. The default renderer for this
			 * section calls {@link Help#header(Object...)}.
			 * 
			 * @since 3.9
			 */
			...
			/**
			 * {@linkplain #sectionKeys() Section key} to {@linkplain #sectionMap() control} the
			 * {@linkplain IHelpSectionRenderer section renderer} for the Synopsis Heading section. The default renderer
			 * for this section calls {@link Help#synopsisHeading(Object...)}.
			 * 
			 * @since 3.9
			 */
			...
			/**
			 * {@linkplain #sectionKeys() Section key} to {@linkplain #sectionMap() control} the
			 * {@linkplain IHelpSectionRenderer section renderer} for the Synopsis section. The default renderer for
			 * this section calls {@link Help#synopsis(int)}.
			 * 
			 * @since 3.9
			 */
			...
			/**
			 * {@linkplain #sectionKeys() Section key} to {@linkplain #sectionMap() control} the
			 * {@linkplain IHelpSectionRenderer section renderer} for the Description Heading section. The default
			 * renderer for this section calls {@link Help#descriptionHeading(Object...)}.
			 * 
			 * @since 3.9
			 */
			...
			/**
			 * {@linkplain #sectionKeys() Section key} to {@linkplain #sectionMap() control} the
			 * {@linkplain IHelpSectionRenderer section renderer} for the Description section. The default renderer for
			 * this section calls {@link Help#description(Object...)}.
			 * 
			 * @since 3.9
			 */
			...
			/**
			 * {@linkplain #sectionKeys() Section key} to {@linkplain #sectionMap() control} the
			 * {@linkplain IHelpSectionRenderer section renderer} for the Parameter List Heading section. The default
			 * renderer for this section calls {@link Help#parameterListHeading(Object...)}.
			 * 
			 * @since 3.9
			 */
			...
			/**
			 * {@linkplain #sectionKeys() Section key} to {@linkplain #sectionMap() control} the
			 * {@linkplain IHelpSectionRenderer section renderer} for the Parameter List section. The default renderer
			 * for this section calls {@link Help#parameterList()}.
			 * 
			 * @since 3.9
			 */
			...
			/**
			 * {@linkplain #sectionKeys() Section key} to {@linkplain #sectionMap() control} the
			 * {@linkplain IHelpSectionRenderer section renderer} for the Option List Heading section. The default
			 * renderer for this section calls {@link Help#optionListHeading(Object...)}.
			 * 
			 * @since 3.9
			 */
			...
			/**
			 * {@linkplain #sectionKeys() Section key} to {@linkplain #sectionMap() control} the
			 * {@linkplain IHelpSectionRenderer section renderer} for the Option List section. The default renderer for
			 * this section calls {@link Help#optionList()}.
			 * 
			 * @since 3.9
			 */
			...
			/**
			 * {@linkplain #sectionKeys() Section key} to {@linkplain #sectionMap() control} the
			 * {@linkplain IHelpSectionRenderer section renderer} for the Subcommand List Heading section. The default
			 * renderer for this section calls {@link Help#commandListHeading(Object...)}.
			 * 
			 * @since 3.9
			 */
			...
			/**
			 * {@linkplain #sectionKeys() Section key} to {@linkplain #sectionMap() control} the
			 * {@linkplain IHelpSectionRenderer section renderer} for the Subcommand List section. The default renderer
			 * for this section calls {@link Help#commandList()}.
			 * 
			 * @since 3.9
			 */
			...
			/**
			 * {@linkplain #sectionKeys() Section key} to {@linkplain #sectionMap() control} the
			 * {@linkplain IHelpSectionRenderer section renderer} for the Exit Code List Heading section. The default
			 * renderer for this section calls {@link Help#exitCodeListHeading(Object...)}.
			 * 
			 * @since 4.0
			 */
			...
			/**
			 * {@linkplain #sectionKeys() Section key} to {@linkplain #sectionMap() control} the
			 * {@linkplain IHelpSectionRenderer section renderer} for the Exit Code List section. The default renderer
			 * for this section calls {@link Help#exitCodeList()}.
			 * 
			 * @since 4.0
			 */
			...
			/**
			 * {@linkplain #sectionKeys() Section key} to {@linkplain #sectionMap() control} the
			 * {@linkplain IHelpSectionRenderer section renderer} for the Footer Heading section. The default renderer
			 * for this section calls {@link Help#footerHeading(Object...)}.
			 * 
			 * @since 3.9
			 */
			...
			/**
			 * {@linkplain #sectionKeys() Section key} to {@linkplain #sectionMap() control} the
			 * {@linkplain IHelpSectionRenderer section renderer} for the Footer section. The default renderer for this
			 * section calls {@link Help#footer(Object...)}.
			 * 
			 * @since 3.9
			 */
			...
			/**
			 * Constant Boolean holding the default setting for whether to attempt to adjust the width to the terminal
			 * width: <code>{@value}</code>.
			 */
			...
			/**
			 * Constant String holding the default string that separates options from option parameters: {@code ' '}
			 * ({@value}).
			 */
			...
			/**
			 * Constant Boolean holding the default setting for whether to show default values in the usage help
			 * message: <code>{@value}</code>.
			 */
			static final Boolean DEFAULT_SHOW_DEFAULT_VALUES = Boolean.FALSE;

			/**
			 * Constant Boolean holding the default setting for whether this command should be listed in the usage help
			 * of the parent command: <code>{@value}</code>.
			 */
			static final Boolean DEFAULT_HIDDEN = Boolean.FALSE;

			/**
			 * Constant Boolean holding the default setting for whether line breaks should take wide CJK characters into
			...
			private List<String> sectionKeys = Collections.unmodifiableList(Arrays.asList(SECTION_KEY_HEADER_HEADING,
					...
					SECTION_KEY_PARAMETER_LIST, SECTION_KEY_OPTION_LIST_HEADING, SECTION_KEY_OPTION_LIST,
					SECTION_KEY_COMMAND_LIST_HEADING, SECTION_KEY_COMMAND_LIST, SECTION_KEY_EXIT_CODE_LIST_HEADING,
					SECTION_KEY_EXIT_CODE_LIST, SECTION_KEY_FOOTER_HEADING, SECTION_KEY_FOOTER));
			...
			private int getSysPropertyWidthOrDefault(int defaultWidth, boolean detectTerminalSize)
			{
				...
				try
				{
					...
					{
						new Tracer().warn("Invalid picocli.usage.width value %d. Using minimum usage width %d.%n",
				...
				{
					new Tracer().warn("Invalid picocli.usage.width value '%s'. Using usage width %d.%n", userValue,
							defaultWidth);
					return defaultWidth;
				}
			}
			...
			{
				String userValue = System.getProperty("picocli.usage.width");
				boolean sysPropAutoWidth = Arrays.asList("AUTO", "TERM", "TERMINAL")
			...
			private static int getTerminalWidth()
			{
				...
				final String[] cmd = (Help.Ansi.isWindows() && !Help.Ansi.isPseudoTTY())
						...
						: (Help.Ansi.isMac() ? new String[] { "stty", "-a", "-f", "/dev/tty" }
				...
				Thread t = new Thread(new Runnable()
				{
					public void run()
					{
						...
						try
						{
							...
							Method redirectError = ProcessBuilder.class.getDeclaredMethod("redirectError",
									redirectClass);
			 ...
			 * if set, otherwise returns the value set via the {@link #width(int)} method, or if not set, the
			...
			public int width()
			{
				return getSysPropertyWidthOrDefault(width, autoWidth());
			}

			/**
			 * Returns whether picocli should attempt to detect the terminal size and adjust the usage help message
			 * width to take the full terminal width. End users may enable this by setting system property
			 * {@code "picocli.usage.width"} to {@code AUTO}, and may disable this by setting this system property to a
			 * {@linkplain #width() numeric value}. This feature requires Java 7 or greater. The default is
			 * {@code false}.
			 * 
			...
			/**
			 * Given a character, is this character considered to be a CJK character? Shamelessly stolen from <a href=
			...
			static boolean isCharCJK(char c)
			{
				...
				return (c == 0x00b1 || unicodeBlock == Character.UnicodeBlock.HIRAGANA)
			...
			/**
			 * Returns the section keys in the order that the usage help message should render the sections. This
			 * ordering may be modified with the {@link #sectionKeys(List) sectionKeys setter}. The default keys are (in
			 * order):
			 * <ol start="0">
			 * <li>{@link UsageMessageSpec#SECTION_KEY_HEADER_HEADING SECTION_KEY_HEADER_HEADING}</li>
			 * <li>{@link UsageMessageSpec#SECTION_KEY_HEADER SECTION_KEY_HEADER}</li>
			...
			/**
			 * Returns the map of section keys and renderers used to construct the usage help message. The usage help
			 * message can be customized by adding, replacing and removing section renderers from this map. Sections can
			 * be reordered with the {@link #sectionKeys(List) sectionKeys setter}. Sections that are either not in this
			 * map or not in the list returned by {@link #sectionKeys() sectionKeys} are omitted.
			 * 
			 * @see #sectionKeys
			 * @since 3.9
			 */
			...
			/**
			 * Sets the map of section keys and renderers used to construct the usage help message to a copy of the
			 * specified map.
			 * 
			 * @param map
			 *            the mapping of section keys to their renderers, must be non-{@code null}.
			...
			/**
			 * Returns the optional header lines displayed at the top of the help message. For subcommands, the first
			 * header line is displayed in the list of cli. Values are initialized from {@link Command#header()} if the
			 * {@code Command} annotation is present, otherwise this is an empty array and the help message has no
			...
			/**
			 * Returns the optional heading preceding the synopsis. Initialized from {@link Command#synopsisHeading()},
			...
			/**
			 * Returns the String representing the subcommands in the synopsis. Initialized from
			 * {@link Command#synopsisSubcommandLabel()}, {@code "[COMMANDS]"} by default.
			 * 
			 * @since 4.0
			 */
			...
			/**
			 * Returns the fraction of the usage help {@link #width()} that is the threshold up to which the 2nd line
			 * and subsequent lines of a multi-line synopsis should be aligned to the end of the command name. The
			 * default value of this attribute is {@code 0.5}. If the length of the synopsis heading plus the length of
			 * the fully qualified command name exceeds this fraction of the width, the 2nd and subsequent rows of a
			 * multi-line synopsis will be aligned to the {@link #synopsisIndent()} instead of the end of the command
			 * name.
			 * 
			 * @since 4.0
			 */
			...
			/**
			 * Returns the indentation to use on the 2nd line and subsequent lines of a multi-line synopsis when the
			 * length of the synopsis heading and the fully qualified command name exceed the {@link #width()} times the
			 * {@link #synopsisAutoIndentThreshold()}, {@code -1} by default. A negative value for this option means
			 * that the 2nd line and subsequent lines are aligned to the synopsis heading length. A positive value means
			 * the exact number of spaces to indent for the 2nd line and subsequent lines of the synopsis.
			 * 
			 * @since 4.0
			 */
			...
			/**
			 * Returns the optional custom synopsis lines to use instead of the auto-generated synopsis. Initialized
			 * from {@link Command#customSynopsis()} if the {@code Command} annotation is present, otherwise this is an
			 * empty array and the synopsis is generated. Applications may programmatically set this field to create a
			 * custom help message.
			 */
			...
			/**
			 * Returns the optional text lines to use as the description of the help message, displayed between the
			 * synopsis and the options list. Initialized from {@link Command#description()} if the {@code Command}
			 * annotation is present, otherwise this is an empty array and the help message has no description.
			...
			/**
			 * Returns whether the options list in the usage help message should show default values for all non-boolean
			 * options.
			 */
			...
			/**
			 * Returns the optional heading preceding the exit codes section, may contain {@code "%n"} line separators.
			 * {@code ""} (empty string) by default.
			 */
			...
			/**
			 * Returns an unmodifiable map with values to be displayed in the exit codes section: keys are exit codes,
			 * values are descriptions. Descriptions may contain {@code "%n"} line separators. Callers may be interested
			 * in the {@link UsageMessageSpec#keyValuesMap(String...) keyValuesMap} method for creating a map from a
			 * list of {@code "key:value"} Strings.
			 * <p>
			 * This may be configured in a resource bundle by listing up multiple {@code "key:value"} pairs. For
			 * example:
			 * </p>
			 * 
			 * <pre>
			 ...
			 * </pre>
			 * 
			 * @return an unmodifiable map with values to be displayed in the exit codes section, or an empty map if no
			 *         exit codes are {@linkplain #exitCodeList(Map) registered}.
			...
			/**
			 * Creates and returns a {@code Map} that contains an entry for each specified String that is in
			 * {@code "key:value"} format.
			 * 
			 * @param entries
			 *            the strings to process; values that are not in {@code "key:value"} format are ignored
			...
			public static Map<String, String> keyValuesMap(String... entries)
			{
				...
				for (int i = 0; i < entries.length; i++)
				{
					...
					{
						new Tracer().info("Ignoring line at index %d: cannot split '%s' into 'key:value'%n", i,
								entries[i]);
					}
				}
				return result;
			 ...
			 * {@link Command#footer()} if the {@code Command} annotation is present, otherwise this is an empty array
			 * and the help message has no footer. Applications may programmatically set this field to create a custom
			 * help message.
			 */
			...
			/**
			 * Sets the optional header lines displayed at the top of the help message. For subcommands, the first
			 * header line is displayed in the list of cli.
			 * 
			 * @return this UsageMessageSpec for method chaining
			 */
			...
			/**
			 * Sets the fraction of the usage help {@link #width()} that is the threshold up to which the 2nd line and
			 * subsequent lines of a multi-line synopsis should be aligned to the end of the command name. The default
			 * value of this attribute is {@code 0.5}. If the length of the synopsis heading plus the length of the
			 * fully qualified command name exceeds this fraction of the width, the 2nd and subsequent rows of a
			 * multi-line synopsis will be aligned to the {@link #synopsisIndent()} instead of the end of the command
			 * name.
			 * 
			 * @param newValue
			 *            the new threshold value. Must be a value between 0.0 and 0.9, inclusive
			 * @return this UsageMessageSpec for method chaining
			 * @throws IllegalArgumentException
			 *             if the specified value is less than 0.0 or greater than 0.9
			 * @since 4.0
			 */
			...
			/**
			 * Sets the indentation to use on the 2nd line and subsequent lines of a multi-line synopsis when the length
			 * of the synopsis heading and the fully qualified command name exceed the
			 * {@link #synopsisAutoIndentThreshold()} fraction of the {@link #width()}, {@code -1} by default. A
			 * negative value for this option means that the 2nd line and subsequent lines are aligned to the synopsis
			 * heading length. A positive value means the exact number of spaces to indent for the 2nd line and
			 * subsequent lines of the synopsis.
			 * 
			 * @return this UsageMessageSpec for method chaining
			 * @since 4.0
			 */
			...
			/**
			 * Sets the optional text lines to use as the description of the help message, displayed between the
			...
			/**
			 * Sets whether the options list in the usage help message should show default values for all non-boolean
			 * options.
			 * 
			 * @return this UsageMessageSpec for method chaining
			 */
			...
			/**
			 * Set the hidden flag on this command to control whether to show or hide it in the help usage text of the
			 * parent command.
			 * 
			 * @param value
			...
			/**
			 * Sets the optional heading preceding the exit codes section, may contain {@code "%n"} line separators.
			 * {@code ""} (empty string) by default.
			 * 
			 * @since 4.0
			 */
			 ...
			 * 
			 * @param newValue
			 *            a map with values to be displayed in the exit codes section
			 * @see #keyValuesMap(String...)
			 * @since 4.0
			 */
			 ...
			 * 
			 * @return the Messages object that encapsulates this {@linkplain CommandSpec#resourceBundle() command's
			 *         resource bundle}
			 * @since 3.6
			 */
			 ...
			 * 
			 * @param msgs
			 *            the new Messages value that encapsulates this {@linkplain CommandSpec#resourceBundle()
			 ...
			 * 
			 * @return true if wide Chinese, Japanese and Korean characters are counted as double the size of other
			 *         characters for line-breaking purposes
			 * @since 4.0
			 */
			...
			/**
			 * Sets whether line breaks should take wide Chinese, Japanese and Korean characters into account, and
			 * returns this UsageMessageSpec.
			 * 
			 * @param adjustForWideChars
			 *            if true, wide Chinese, Japanese and Korean characters are counted as double the size of other
			 *            characters for line-breaking purposes
			...
			{
				if (!empty(cmd.resourceBundle()))
				{ // else preserve superclass bundle
					if (loadResourceBundle)
					{ // error if resource bundle does not exist
						messages(new Messages(commandSpec, cmd.resourceBundle()));
					}
					else
					{
						ResourceBundle rb = null; // from annotation processor, we may not be able to load the
													// ResourceBundle
						try
						{
		...
		public static class ParserSpec
		{
			...
			/**
			 * Returns true if arguments should be split first before any further processing and the number of parts
			 * resulting from the split is limited to the max arity of the argument.
			 */
			...
			/**
			 * Returns true if options with attached arguments should not consume subsequent arguments and should not
			 * validate arity. The default is {@code false}.
			 */
			...
			/**
			 * Returns true if exceptions during parsing should be collected instead of thrown. Multiple errors may be
			 * encountered during parsing. These can be obtained from {@link ParseResult#errors()}.
			 * 
			 * @since 3.2
			 */
			...
			/**
			 * Sets whether exceptions during parsing should be collected instead of thrown. Multiple errors may be
			 * encountered during parsing. These can be obtained from {@link ParseResult#errors()}.
			 * 
			 * @since 3.2
			 */
			...
			/**
			 * Sets whether arguments should be {@linkplain ArgSpec#splitRegex() split} first before any further
			 * processing. If true, the original argument will only be split into as many parts as allowed by max arity.
			 */
			...
			public String toString()
			{
				return String.format("posixClusteredShortOptionsAllowed=%s, stopAtPositional=%s, stopAtUnmatched=%s, "
						...
						posixClusteredShortOptionsAllowed, stopAtPositional, stopAtUnmatched, separator,
						overwrittenOptionsAllowed, unmatchedArgumentsAllowed, expandAtFiles, atFileCommentChar,
						useSimplifiedAtFiles, endOfOptionsDelimiter, limitSplit, aritySatisfiedByAttachedOptionParam,
		...
		public abstract static class ArgSpec
		{
			...
			private <T extends Builder<T>> ArgSpec(Builder<T> builder)
			{
				...
				{
					List<String> list = new ArrayList<String>();
					for (Object c : typeInfo.getEnumConstantNames())
					{
						list.add(c.toString());
					}
			...
			void applyInitialValue(Tracer tracer)
			{
				if (hasInitialValue())
				{
					try
					{
						...
						tracer.debug("Set initial value for %s of type %s to %s.%n", this, type(),
								String.valueOf(initialValue()));
					}
					catch (Exception ex)
					{
						tracer.warn("Could not set initial value for %s of type %s to %s: %s%n", this, type(),
			...
			/**
			 * Returns whether this is a required option or positional parameter without a default value. If this
			 * argument is part of a {@linkplain ArgGroup group}, this method returns whether this argument is required
			 * <em>within the group</em> (so it is not necessarily a required argument for the command).
			 * 
			 * @see Option#required()
			 */
			 ...
			 * including the {@code ${DEFAULT-VALUE}} and {@code ${COMPLETION-CANDIDATES}} variables. Use
			 * {@link CommandSpec#interpolateVariables(Boolean)} to switch off variable expansion if needed.
			 * <p>
			 * If a resource bundle has been {@linkplain ArgSpec#messages(Messages) set}, this method will first try to
			 * find a value in the resource bundle: If the resource bundle has no entry for the
			 * {@code fully qualified commandName + "." + descriptionKey} or for the unqualified {@code descriptionKey},
			 * an attempt is made to find the option or positional parameter description using any of the
			 * {@linkplain #getAdditionalDescriptionKeys() additional description keys}, first with the
			...
			public String[] description()
			{
				...
				if (commandSpec == null || commandSpec.interpolateVariables())
				{ // expand variables
				...
				for (int i = 0; i < desc.length; i++)
				{
					result[i] = format(
							desc[i].replace(DESCRIPTION_VARIABLE_DEFAULT_VALUE, defaultValueString.replace("%", "%%"))
									.replace(DESCRIPTION_VARIABLE_FALLBACK_VALUE,
											fallbackValueString.replace("%", "%%"))
			 ...
			 * Returns whether usage syntax decorations around the {@linkplain #paramLabel() paramLabel} should be
			 * suppressed. The default is {@code false}: by default, the paramLabel is surrounded with {@code '['} and
			 * {@code ']'} characters if the value is optional and followed by ellipses ("...") when multiple values can
			 * be specified.
			 * 
			 * @since 3.6.0
			 */
			...
			/**
			 * Returns auxiliary type information used when the {@link #type()} is a generic {@code Collection},
			 * {@code Map} or an abstract class.
			 * 
			 * @see Option#type()
			 */
			 ...
			 * option or positional parameter should use a custom conversion that is different from the normal
			 * conversion for the arg spec's type.
			 * 
			...
			/**
			 * Returns a regular expression to split option parameter values or {@code ""} if the value should not be
			 * split.
			 * 
			 * @see Option#split()
			 */
			...
			/**
			 * Returns the type to convert the option or positional parameter to before {@linkplain #setValue(Object)
			 * setting} the value.
			 */
			public Class<?> type()
			{
			...
			/**
			 * Returns the {@code ITypeInfo} that can be used both at compile time (by annotation processors) and at
			 * runtime.
			 * 
			 * @since 4.0
			 */
			...
			/**
			 * Returns the default value to assign if this option or positional parameter was not specified on the
			 * command line, before splitting and type conversion. This method returns the programmatically set value;
			 * this may differ from the default value that is actually used: if this ArgSpec is part of a CommandSpec
			 * with a {@link IDefaultValueProvider}, picocli will first try to obtain the default value from the default
			 * value provider, and this method is only called if the default provider is {@code null} or returned a
			 * {@code null} value.
			 * 
			...
			/**
			 * Returns the initial value this option or positional parameter. If {@link #hasInitialValue()} is true, the
			 * option will be reset to the initial value before parsing (regardless of whether a default value exists),
			...
			/**
			 * Determines whether the option or positional parameter will be reset to the {@link #initialValue()} before
			 * parsing new input.
			 */
			...
			/**
			 * Returns the default value String for the purpose of displaying it in the description, without
			...
			/**
			 * Returns the default value String displayed in the description; interpolating variables if specified. If
			 * this ArgSpec is part of a CommandSpec with a {@link IDefaultValueProvider}, this method will first try to
			 * obtain the default value from the default value provider; if the provider is {@code null} or if it
			 * returns a {@code null} value, then next any value set to {@link ArgSpec#defaultValue()} is returned, and
			 * if this is also {@code null}, finally the {@linkplain ArgSpec#initialValue() initial value} is returned.
			 * 
			 * @param interpolateVariables
			 *            whether to interpolate variables in the {@code defaultValue} attribute of this ArgSpec
			 * @see CommandSpec#defaultValueProvider()
			 ...
			 * 
			 * @return the completion candidates for this option or positional parameter, valid enum constant names, or
			 *         {@code null}
			 * @since 3.2
			 */
			...
			/**
			 * Returns a custom {@code IParameterConsumer} to temporarily suspend picocli's parsing logic and process
			 * one or more command line arguments in a custom manner, or {@code null}. An example of when this may be
			 * useful is when passing arguments through to another program.
			 * 
			 * @since 4.0
			 */
			...
			/**
			 * Returns the {@link IScope} that determines on which object to set the value (or from which object to get
			...
			/**
			 * Sets the value of this argument to the specified value and returns the previous value. Delegates to the
			 * current {@link #setter()}.
			 */
			...
			/**
			 * Returns {@code true} if this argument's {@link #type()} is an array, a {@code Collection} or a
			 * {@code Map}, {@code false} otherwise.
			 */
			...
			/**
			 * Returns the groups this option or positional parameter belongs to, or {@code null} if this option is not
			 * part of a group.
			 * 
			 * @since 4.0
			 */
			 ...
			 * 
			 * @return the matched arguments after {@linkplain #splitRegex() splitting}, but before type conversion. For
			 *         map properties, {@code "key=value"} values are split into the key and the value part.
			 */
			 ...
			 * 
			 * @return the matched arguments after {@linkplain #splitRegex() splitting} and type conversion. For map
			 *         properties, {@code "key=value"} values are split into the key and the value part.
			 */
			 ...
			 *         values have not been {@linkplain #splitRegex() split}, and for map properties values may look
			 *         like {@code "key=value"}
			 */
			...
			/**
			 * Returns whether the default for this option or positional parameter should be shown, potentially
			 * overriding the specified global setting.
			 * 
			...
			private String[] debug(String[] result, String msg, String value)
			{
				...
				if (t.isDebug())
				{
					t.debug("%s with regex '%s' resulted in %s parts: %s%n", msg, splitRegex(), result.length,
			...
			{
				return 17 + 37 * Assert.hashCode(defaultValue) + 37 * Assert.hashCode(arity)
						+ 37 * Assert.hashCode(hidden) + 37 * Assert.hashCode(paramLabel)
						+ 37 * Assert.hashCode(hideParamSyntax) + 37 * Assert.hashCode(required)
			...
			abstract static class Builder<T extends Builder<T>>
			{
				...
				Builder(IAnnotatedElement source)
				{
					...
					try
					{
						initialValue = source.getter().get();
					}
					catch (Exception ex)
					{
						initialValue = null;
				...
				Builder(Parameters parameters, IAnnotatedElement source, IFactory factory)
				{
					...
					else
					{
						...
						if (factory != null)
						{ // annotation processors will pass a null factory
				...
				private static String inferLabel(String label, String fieldName, ITypeInfo typeInfo)
				{
					...
					if (typeInfo.isMap())
					{ // #195 better param labels for map fields
				...
				/**
				 * Returns whether usage syntax decorations around the {@linkplain #paramLabel() paramLabel} should be
				 * suppressed. The default is {@code false}: by default, the paramLabel is surrounded with {@code '['}
				 * and {@code ']'} characters if the value is optional and followed by ellipses ("...") when multiple
				...
				/**
				 * Returns one or more {@link CommandLine.ITypeConverter type converters} to use to convert the command
				 * line argument into a strongly typed value (or key-value pair for map fields). This is useful when a
				 * particular option or positional parameter should use a custom conversion that is different from the
				 * normal conversion for the arg spec's type.
				 * 
				 * @see Option#converter()
				 */
				public ITypeConverter<?>[] converters()
				...
				/**
				 * Returns a regular expression to split option parameter values or {@code ""} if the value should not
				 * be split.
				 * 
				 * @see Option#split()
				 */
				 ...
				 * 
				 * @return type information that does not require {@code Class} objects and be constructed both at
				 ...
				 * conversion. A value of {@code null} means this option or positional parameter does not have a
				 * default.
				 */
				public String defaultValue()
				{
				 ...
				 * the option will be reset to the initial value before parsing (regardless of whether a default value
				 * exists), to clear values that would otherwise remain from parsing previous input.
				 */
				public Object initialValue()
				{
					return initialValue;
				...
				/**
				 * Returns whether this option or positional parameter's default value should be shown in the usage
				 * help.
				 */
				public Help.Visibility showDefaultValue()
				{
				...
				/**
				 * Returns the {@link IScope} that determines where the setter sets the value (or the getter gets the
				...
				/**
				 * Sets whether this option prompts the user to enter a value on the command line, and returns this
				 * builder.
				 */
				...
				/**
				 * Sets the description of this option, used when generating the usage documentation, and returns this
				 * builder.
				 * 
				 * @see Option#description()
				 */
				...
				/**
				 * Sets the description key that is used to look up the description in a resource bundle, and returns
				 * this builder.
				 * 
				 * @see Option#descriptionKey()
				 * @see Parameters#descriptionKey()
				...
				/**
				 * Sets the name of the option or positional parameter used in the usage help message, and returns this
				 * builder.
				 */
				...
				/**
				 * Sets whether usage syntax decorations around the {@linkplain #paramLabel() paramLabel} should be
				 * suppressed. The default is {@code false}: by default, the paramLabel is surrounded with {@code '['}
				 * and {@code ']'} characters if the value is optional and followed by ellipses ("...") when multiple
				 * values can be specified.
				 * 
				 ...
				 * 
				 * @param types
				 *            the element type(s) when the {@link #type()} is a generic {@code Collection} or a
				 *            {@code Map}; or the concrete type when the {@link #type()} is an abstract class.
				 */
				...
				/**
				 * Sets a regular expression to split option parameter values or {@code ""} if the value should not be
				 * split, and returns this builder.
				 */
				...
				/**
				 * Sets the type to convert the option or positional parameter to before {@linkplain #setValue(Object)
				 * setting} the value, and returns this builder.
				 * 
				 * @param propertyType
				 *            the type of this option or parameter. For multi-value options and positional parameters
				 *            this can be an array, or a (sub-type of) Collection or Map.
				 */
				 ...
				 * 
				 * @param typeInfo
				 *            type information that does not require {@code Class} objects and be constructed both at
				 *            runtime and compile time
				 * @since 4.0
				 */
				 ...
				 * 
				 * @param userObject
				 *            may be the annotated program element, or some other useful object
				 * @since 4.0
				 */
				...
				/**
				 * Sets the default value of this option or positional parameter to the specified value, and returns
				 * this builder. Before parsing the command line, the result of {@linkplain #splitRegex() splitting} and
				 * {@linkplain #converters() type converting} this default value is applied to the option or positional
				...
				/**
				 * Sets the initial value of this option or positional parameter to the specified value, and returns
				 * this builder. If {@link #hasInitialValue()} is true, the option will be reset to the initial value
				 * before parsing (regardless of whether a default value exists), to clear values that would otherwise
				...
				/**
				 * Sets the {@link IGetter} that is responsible for getting the value of this argument, and returns this
				 * builder.
				 */
				...
				/**
				 * Sets the {@link ISetter} that is responsible for modifying the value of this argument, and returns
				 * this builder.
				 */
				...
				/**
				 * Sets the string respresentation of this option or positional parameter to the specified value, and
		...
		/**
		 * The {@code OptionSpec} class models aspects of a <em>named option</em> of a {@linkplain CommandSpec command},
		 * including whether it is required or optional, the option parameters supported (or required) by the option,
		 * and attributes for the usage help message describing the option.
		 * <p>
		 * An option has one or more names. The option is matched when the parser encounters one of the option names in
		 * the command line arguments. Depending on the option's {@link #arity() arity}, the parser may expect it to
		 * have option parameters. The parser will call {@link #setValue(Object) setValue} on the matched option for
		 * each of the option parameters encountered.
		 * </p>
		 * <p>
		 * For multi-value options, the {@code type} may be an array, a {@code Collection} or a {@code Map}. In this
		 * case the parser will get the data structure by calling {@link #getValue() getValue} and modify the contents
		 * of this data structure. (In the case of arrays, the array is replaced with a new instance with additional
		 * elements.)
		 * </p>
		 * <p>
		 * Before calling the setter, picocli converts the option parameter value from a String to the option
		 * parameter's type.
		 * </p>
		 * <ul>
		 * <li>If a option-specific {@link #converters() converter} is configured, this will be used for type
		 * conversion. If the option's type is a {@code Map}, the map may have different types for its keys and its
		 * values, so {@link #converters() converters} should provide two converters: one for the map keys and one for
		 * the map values.</li>
		 * <li>Otherwise, the option's {@link #type() type} is used to look up a converter in the list of
		 * {@linkplain CommandLine#registerConverter(Class, ITypeConverter) registered converters}. For multi-value
		 * options, the {@code type} may be an array, or a {@code Collection} or a {@code Map}. In that case the
		 * elements are converted based on the option's {@link #auxiliaryTypes() auxiliaryTypes}. The auxiliaryType is
		 * used to look up the converter(s) to use to convert the individual parameter values. Maps may have different
		 * types for its keys and its values, so {@link #auxiliaryTypes() auxiliaryTypes} should provide two types: one
		 * for the map keys and one for the map values.</li>
		 * </ul>
		 * <p>
		 ...
		 * </p>
		 * <p>
		 * When an {@code OptionSpec} is created from an {@link Option @Option} -annotated field or method, it is
		 * "bound" to that field or method: this field is set (or the method is invoked) when the option is matched and
		 * {@link #setValue(Object) setValue} is called. Programmatically constructed {@code OptionSpec} instances will
		 * remember the value passed to the {@link #setValue(Object) setValue} method so it can be retrieved with the
		 * {@link #getValue() getValue} method. This behaviour can be customized by installing a custom {@link IGetter}
		 * and {@link ISetter} on the {@code OptionSpec}.
		 * </p>
		 * 
		 * @since 3.0
		 */
		public static class OptionSpec extends ArgSpec implements IOrdered
		{
			...
			private OptionSpec(Builder builder)
			{
				...
				// throw new InitializationException("Option " + longestName() + " is not a boolean so should not be
				// defined with arity=" + arity());
				// }
			}

			/**
			 * Returns a new Builder initialized with the attributes from this {@code OptionSpec}. Calling {@code build}
			 * immediately will return a copy of this {@code OptionSpec}.
			 * 
			 ...
			 * 
			 * @return option names (after variable interpolation), without leading hyphens, slashes and other non-Java
			 ...
			 * Options with a lower number are shown before options with a higher number. This attribute is only honored
			 * if {@link UsageMessageSpec#sortOptions()} is {@code false} for this command.
			 * 
			 * @see Option#order()
			 * @since 3.9
			 */
			...
			/**
			 * Returns whether a negative version for this boolean option is automatically added. The form of the
			 * negative name is determined by the {@link INegatableOptionTransformer}.
			 * 
			 * @see Option#negatable()
			 * @since 4.0
			 */
			 ...
			 * parameter (for example, {@code arity = "0..1"}) if the option was specified on the command line without
			 * parameter.
			 * 
			...
			public boolean equals(Object obj)
			{
				...
				boolean result = super.equalsImpl(other) && help == other.help && usageHelp == other.usageHelp
			...
			public static class Builder extends ArgSpec.Builder<Builder>
			{
				...
				/**
				 * Returns the position in the options list in the usage help message at which this option should be
				 * shown. Options with a lower number are shown before options with a higher number. This attribute is
				 * only honored if {@link UsageMessageSpec#sortOptions()} is {@code false} for this command.
				 * 
				 * @see Option#order()
				 * @since 3.9
				 */
				...
				/**
				 * Replaces the option names with the specified values. At least one option name is required, and
				 * returns this builder.
				 * 
				...
				/**
				 * Sets whether a negative version for this boolean option is automatically added, and returns this
				 * builder.
				 * 
				 * @since 4.0
				 */
				 ...
				 * parameter if the option was specified on the command line without parameter, and returns this
				 * builder.
				 * 
				 * @see Option#fallbackValue()
				 * @since 4.0
				...
				/**
				 * Sets the position in the options list in the usage help message at which this option should be shown,
				 * and returns this builder.
				 * 
				 * @since 3.9
				 */
		...
		/**
		 * The {@code PositionalParamSpec} class models aspects of a <em>positional parameter</em> of a
		 * {@linkplain CommandSpec command}, including whether it is required or optional, and attributes for the usage
		 * help message describing the positional parameter.
		 * <p>
		 * Positional parameters have an {@link #index() index} (or a range of indices). A positional parameter is
		 * matched when the parser encounters a command line argument at that index. Named options and their parameters
		 * do not change the index counter, so the command line can contain a mixture of positional parameters and named
		 * options.
		 * </p>
		 * <p>
		 * Depending on the positional parameter's {@link #arity() arity}, the parser may consume multiple command line
		 * arguments starting from the current index. The parser will call {@link #setValue(Object) setValue} on the
		 * {@code PositionalParamSpec} for each of the parameters encountered. For multi-value positional parameters,
		 * the {@code type} may be an array, a {@code Collection} or a {@code Map}. In this case the parser will get the
		 * data structure by calling {@link #getValue() getValue} and modify the contents of this data structure. (In
		 * the case of arrays, the array is replaced with a new instance with additional elements.)
		 * </p>
		 * <p>
		 * Before calling the setter, picocli converts the positional parameter value from a String to the parameter's
		 * type.
		 * </p>
		 * <ul>
		 * <li>If a positional parameter-specific {@link #converters() converter} is configured, this will be used for
		 * type conversion. If the positional parameter's type is a {@code Map}, the map may have different types for
		 * its keys and its values, so {@link #converters() converters} should provide two converters: one for the map
		 * keys and one for the map values.</li>
		 * <li>Otherwise, the positional parameter's {@link #type() type} is used to look up a converter in the list of
		 * {@linkplain CommandLine#registerConverter(Class, ITypeConverter) registered converters}. For multi-value
		 * positional parameters, the {@code type} may be an array, or a {@code Collection} or a {@code Map}. In that
		 * case the elements are converted based on the positional parameter's {@link #auxiliaryTypes() auxiliaryTypes}.
		 * The auxiliaryType is used to look up the converter(s) to use to convert the individual parameter values. Maps
		 * may have different types for its keys and its values, so {@link #auxiliaryTypes() auxiliaryTypes} should
		 * provide two types: one for the map keys and one for the map values.</li>
		 * </ul>
		 * <p>
		 * {@code PositionalParamSpec} objects are used by the picocli command line interpreter and help message
		 * generator. Picocli can construct a {@code PositionalParamSpec} automatically from fields and methods with
		 * {@link Parameters @Parameters} annotations. Alternatively a {@code PositionalParamSpec} can be constructed
		 * programmatically.
		 * </p>
		 * <p>
		 * When a {@code PositionalParamSpec} is created from a {@link Parameters @Parameters} -annotated field or
		 * method, it is "bound" to that field or method: this field is set (or the method is invoked) when the position
		 * is matched and {@link #setValue(Object) setValue} is called. Programmatically constructed
		 * {@code PositionalParamSpec} instances will remember the value passed to the {@link #setValue(Object)
		 * setValue} method so it can be retrieved with the {@link #getValue() getValue} method. This behaviour can be
		 * customized by installing a custom {@link IGetter} and {@link ISetter} on the {@code PositionalParamSpec}.
		 * </p>
		 * 
		 * @since 3.0
		 */
		public static class PositionalParamSpec extends ArgSpec
		{
			...
			/**
			 * Ensures all attributes of this {@code PositionalParamSpec} have a valid value; throws an
			 * {@link InitializationException} if this cannot be achieved.
			 */
			...
			/**
			 * Returns a new Builder initialized with the attributes from this {@code PositionalParamSpec}. Calling
			 * {@code build} immediately will return a copy of this {@code PositionalParamSpec}.
			 * 
			 * @return a builder that can create a copy of this spec
			 */
			public Builder toBuilder()
			...
			/**
			 * Returns the additional lookup keys for finding description lines in the resource bundle for this
			...
			/**
			 * Returns an index or range specifying which of the command line arguments should be assigned to this
			 * positional parameter.
			 * 
			...
			public static class Builder extends ArgSpec.Builder<Builder>
			{
				...
				/**
				 * Sets the index or range specifying which of the command line arguments should be assigned to this
				 * positional parameter, and returns this builder.
				 */
		...
		public interface IOrdered
		{
			 ...
			 * Elements with a lower number are shown before elements with a higher number. For options, this attribute
			 * is only honored if {@link UsageMessageSpec#sortOptions()} is {@code false} for this command. For argument
			 * groups, this attribute is only honored for groups that have a {@link ArgGroup#heading() heading} (or a
			 * {@link ArgGroup#headingKey() headingKey} with a non-{@code null} value).
			 */
			int order();
		}

		/**
		 * The {@code ArgGroupSpec} class models a {@link ArgGroup group} of arguments (options, positional parameters
		...
		public static class ArgGroupSpec implements IOrdered
		{
			...
			ArgGroupSpec(ArgGroupSpec.Builder builder)
			{
				...
				{
					throw new InitializationException(
							"ArgGroup has no options or positional parameters, and no subgroups: " + scope + " "
									+ (getter == null ? setter : getter));
				}
			 ...
			 *            the annotated element containing {@code @Option} and {@code @Parameters}
			 * @return a new ArgGroupSpec.Builder instance
			 */
			...
			/**
			 * Returns whether this is a mutually exclusive group; {@code true} by default. If {@code false}, this is a
			 * co-occurring group. Ignored if {@link #validate()} is {@code false}.
			 * 
			 ...
			 * {@code "0..1"} (optional) by default. A group can be made required by specifying a multiplicity of
			 * {@code "1"}. For a group of mutually exclusive arguments, being required means that one of the arguments
			 * in the group must appear on the command line, or a MissingParameterException is thrown. For a group of
			 * co-occurring arguments, being required means that all arguments in the group must appear on the command
			 * line. Ignored if {@link #validate()} is {@code false}.
			 * 
			...
			/**
			 * Returns whether picocli should validate the rules of this group: for a mutually exclusive group this
			 * means that no more than one arguments in the group is specified on the command line; for a co-ocurring
			 * group this means that all arguments in the group are specified on the command line. {@code true} by
			 * default.
			 * 
			 ...
			 * Groups with a lower number are shown before groups with a higher number. This attribute is only honored
			 * for groups that have a {@link #heading() heading} (or a {@link #headingKey() headingKey} with a
			 * non-{@code null} resource bundle value).
			 */
			public int order()
			{
				return this.order;
			...
			/**
			 * Returns the heading key of this group (may be {@code null}), used to get the heading from a resource
			 * bundle.
			 * 
			...
			/**
			 * Returns the parent group that this group is part of, or {@code null} if this group is not part of a
			 * composite.
			 */
			...
			/**
			 * Returns {@code true} if this group is a subgroup (or a nested sub-subgroup, to any level of depth) of the
			 * specified group, {@code false} otherwise.
			 * 
			 * @param group
			 *            the group to check if it contains this group
			 ...
			 * 
			 * @return type information that does not require {@code Class} objects and be constructed both at runtime
			 *         and compile time
			 */
			public ITypeInfo typeInfo()
			{
			...
			/**
			 * Returns the {@link IGetter} that is responsible for supplying the value of the annotated program element
			...
			/**
			 * Returns the {@link ISetter} that is responsible for modifying the value of the annotated program element
			 * associated with this group.
			 */
			public ISetter setter()
			{
			...
			/**
			 * Returns the {@link IScope} that determines where the setter sets the value (or the getter gets the value)
			 * of the annotated program element associated with this group.
			 */
			public IScope scope()
			{
				return scope;
			 ...
			 * 
			 * @param colorScheme
			 *            the color scheme to use for options and positional parameters in this group and subgroups
			 * @param outparam_groupArgs
			 *            all options and positional parameters in the groups this method generates a synopsis for;
			 *            these options and positional parameters should be excluded from appearing elsewhere in the
			 *            synopsis
			...
			private Text concatOptionText(Text text, Help.ColorScheme colorScheme, OptionSpec option)
			{
				if (!option.hidden())
				{
					Text name = option.negatable()
							? colorScheme.optionText(option.commandSpec.negatableOptionTransformer()
									.makeSynopsis(option.shortestName(), option.commandSpec))
							: colorScheme.optionText(option.shortestName());
					Text param = createLabelRenderer(option.commandSpec).renderParameterLabel(option,
							colorScheme.ansi(), colorScheme.optionParamStyles);
			...
			private Text concatPositionalText(Text text, Help.ColorScheme colorScheme,
					PositionalParamSpec positionalParam)
			{
				if (!positionalParam.hidden())
				{
					Text label = createLabelRenderer(positionalParam.commandSpec).renderParameterLabel(positionalParam,
							colorScheme.ansi(), colorScheme.parameterStyles);
					text = text.concat(open(positionalParam)).concat(label).concat(close(positionalParam));
				}
				return text;
			}

			private String open(ArgSpec argSpec)
			{
				return argSpec.required() ? "" : "[";
			}

			private String close(ArgSpec argSpec)
			{
				return argSpec.required() ? "" : "]";
			}
			...
			void tryInitUserObject(CommandLine commandLine) throws Exception
			{
				...
				if (typeInfo() != null)
				{
					tracer.debug("Creating new user object of type %s for group %s%n",
							typeInfo().getAuxiliaryTypes()[0], synopsis());
					...
					{
						tracer.debug(
								"Initializing %s in group %s: setting scope to user object %s and initializing initial and default values%n",
					...
					for (ArgGroupSpec subgroup : subgroups())
					{
						tracer.debug("Setting scope for subgroup %s with setter=%s in group %s to user object %s%n",
				...
				{
					tracer.debug(
							"No type information available for group %s: cannot create new user object. Scope for arg setters is not changed.%n",
							synopsis());
				}
			...
			void setUserObject(Object userObject, IFactory factory) throws Exception
			{
				...
				{
					@SuppressWarnings("unchecked")
					Collection<Object> c = (Collection<Object>) getter().get();
					if (c == null)
					{
						@SuppressWarnings("unchecked")
			...
			public static class Builder
			{
				...
				/**
				 * Returns whether this is a mutually exclusive group; {@code true} by default. If {@code false}, this
				 * is a co-occurring group. Ignored if {@link #validate()} is {@code false}.
				 * 
				 * @see ArgGroup#exclusive()
				 */
				public boolean exclusive()
				...
				/**
				 * Sets whether this is a mutually exclusive group; {@code true} by default. If {@code false}, this is a
				 * co-occurring group. Ignored if {@link #validate()} is {@code false}.
				 * 
				 * @see ArgGroup#exclusive()
				 */
				 ...
				 * Returns the multiplicity of this group: how many occurrences it may have on the command line;
				 * {@code "0..1"} (optional) by default. A group can be made required by specifying a multiplicity of
				 * {@code "1"}. For a group of mutually exclusive arguments, being required means that one of the
				 * arguments in the group must appear on the command line, or a MissingParameterException is thrown. For
				 * a group of co-occurring arguments, being required means that all arguments in the group must appear
				...
				/**
				 * Returns whether picocli should validate the rules of this group: for a mutually exclusive group this
				 * means that no more than one arguments in the group is specified on the command line; for a
				 * co-ocurring group this means that all arguments in the group are specified on the command line.
				 * {@code true} by default.
				 * 
				 * @see ArgGroup#validate()
				 */
				...
				/**
				 * Sets whether picocli should validate the rules of this group: for a mutually exclusive group this
				 * means that no more than one arguments in the group is specified on the command line; for a
				 * co-ocurring group this means that all arguments in the group are specified on the command line.
				 * {@code true} by default.
				 * 
				...
				/**
				 * Returns the position in the options list in the usage help message at which this group should be
				 * shown. Groups with a lower number are shown before groups with a higher number. This attribute is
				 * only honored for groups that have a {@link #heading() heading} (or a {@link #headingKey() headingKey}
				 * with a non-{@code null} resource bundle value).
				 */
				public int order()
				{
					return order;
				...
				/**
				 * Sets the position in the options list in the usage help message at which this group should be shown,
				 * and returns this builder.
				 */
				public Builder order(int order)
				{
				...
				/**
				 * Sets the type info for the annotated program element associated with this group, and returns this
				 * builder.
				 * 
				 * @param newValue
				 *            type information that does not require {@code Class} objects and be constructed both at
				...
				/**
				 * Returns the {@link IGetter} that is responsible for supplying the value of the annotated program
				 * element associated with this group.
				 */
				public IGetter getter()
				{
					return getter;
				...
				/**
				 * Sets the {@link IGetter} that is responsible for getting the value of the annotated program element
				 * associated with this group, and returns this builder.
				 */
				...
				/**
				 * Returns the {@link ISetter} that is responsible for modifying the value of the annotated program
				 * element associated with this group.
				 */
				public ISetter setter()
				{
				...
				/**
				 * Sets the {@link ISetter} that is responsible for modifying the value of the annotated program element
				 * associated with this group, and returns this builder.
				 */
				...
				/**
				 * Returns the {@link IScope} that determines where the setter sets the value (or the getter gets the
				 * value) of the annotated program element associated with this group.
				 */
				public IScope scope()
				{
					return scope;
				...
				/**
				 * Sets the {@link IScope} that targets where the setter sets the value of the annotated program element
				 * associated with this group, and returns this builder.
				 */
				...
				/**
				 * Adds the specified argument to the list of options and positional parameters that depend on this
				 * group.
				 */
		...
		/**
		 * This class allows applications to specify a custom binding that will be invoked for unmatched arguments. A
		 * binding can be created with a {@code ISetter} that consumes the unmatched arguments {@code String[]}, or with
		 * a {@code IGetter} that produces a {@code Collection<String>} that the unmatched arguments can be added to.
		 * 
		 * @since 3.0
		 */
		public static class UnmatchedArgsBinding
		{
			...
			/**
			 * Creates a {@code UnmatchedArgsBinding} for a getter that produces a {@code Collection<String>} that the
			 * unmatched arguments can be added to.
			 * 
			 * @param getter
			 *            supplies a {@code Collection<String>} that the unmatched arguments can be added to.
			 */
			...
			/**
			 * Returns the getter responsible for producing a {@code Collection} that the unmatched arguments can be
			 * added to.
			 */
			public IGetter getter()
			{
		 ...
		 * and at compile time (when {@code Class} values are not available). Most of the methods in this interface (but
		 * not all!) are safe to use by annotation processors.
		 * 
		 * @since 4.0
		 */
		public interface ITypeInfo
		{
			...
			/**
			 * Returns the names of the type arguments if this is a generic type. For example, returns
			 * {@code ["java.lang.String"]} if this type is {@code List<String>}.
			 */
			 ...
			 * types of the key and the value of a generic Map. This method is <em>not</em> safe for annotation
			 * processors to use.
			 * 
		...
		static class RuntimeTypeInfo implements ITypeInfo
		{
					...
					Class<?> defaultType, boolean interactive)
			{
				...
				if (genericType instanceof ParameterizedType)
				{
					...
					for (Class<?> c : declaredTypeParameters)
					{
					...
					List<String> actualGenericTypeArguments, Range arity, Class<?> defaultType, boolean interactive)
			{
				...
				if (auxiliaryTypes == null || auxiliaryTypes.length == 0)
				{
					...
					{ // type is a collection but element type is unspecified
						auxiliaryTypes = new Class<?>[] { interactive ? char[].class : String.class }; // use String
																										// elements
					}
					else if (Map.class.isAssignableFrom(type))
					{ // type is a map but element type is unspecified
		...
		/**
		 * Internal interface to allow annotation processors to construct a command config at compile time.
		 * 
		 * @since 4.0
		 */
		 ...
		 * <a href="https://docs.oracle.com/javase/8/docs/api/java/util/ResourceBundle.html">ResourceBundle</a> with
		 * support for String arrays and qualified keys: keys that may or may not be prefixed with the fully qualified
		 * command name.
		 * <p>
		 ...
		 * </pre>
		 * <p>
		 * Resources for multiple cli can be specified in a single ResourceBundle. Keys and their value can be shared by
		 * multiple cli (so you don't need to repeat them for every command), but keys can be prefixed with
		 * {@code fully qualified command name + "."} to specify different values for different cli. The most specific
		 * key wins. For example:
		 * </p>
		 * 
		 * <pre>
		 * jfrog.rt.usage.header = Artifactory cli
		 * jfrog.rt.config.usage.header = Configure Artifactory details.
		 * jfrog.rt.upload.usage.header = Upload files.
		 *
		 * jfrog.bt.usage.header = Bintray cli
		 * jfrog.bt.config.usage.header = Configure Bintray details.
		 * jfrog.bt.upload.usage.header = Upload files.
		 *
		 * # shared between all cli
		...
		public static class Messages
		{
			...
			public Messages(CommandSpec spec, String baseName, ResourceBundle rb)
			{
				...
				this.rb = rb;
				this.keys = keys(rb);
				new Tracer().debug("Created Messages from resourceBundle[base=%s] for command '%s' (%s)%n", baseName,
						spec.name(), spec);
			}
			...
			{
				try
				{ // ResourceBundle.getBaseBundleName was introduced in Java 8
			 ...
			 * @return a Messages object with the specified CommandSpec and the ResourceBundle of the specified Messages
			 *         object
			 */
			...
			/**
			 * Returns the String value found in the resource bundle for the specified key, or the specified default
			 * value if not found.
			 * 
			 * @param key
			 *            unqualified resource bundle key. This method will first try to find a value by qualifying the
			 *            key with the command's fully qualified name, and if not found, it will try with the
			 *            unqualified key.
			 * @param defaultValue
			 *            value to return if the resource bundle is null or empty, or if no value was found by the
			 *            qualified or unqualified key
			 * @return the String value found in the resource bundle for the specified key, or the specified default
			 *         value
			 */
			...
			boolean isEmpty()
			{
				return rb == null || keys.isEmpty();
			}

			/**
			 * Returns the String array value found in the resource bundle for the specified key, or the specified
			 * default value if not found. Multi-line strings can be specified in the resource bundle with
			 * {@code key.0}, {@code key.1}, {@code key.2}, etc.
			 * 
			 * @param key
			 *            unqualified resource bundle key. This method will first try to find a value by qualifying the
			 *            key with the command's fully qualified name, and if not found, it will try with the
			 ...
			 *            value to return if the resource bundle is null or empty, or if no value was found by the
			 *            qualified or unqualified key
			 * @return the String array value found in the resource bundle for the specified key, or the specified
			 *         default value
			 */
			...
			/**
			 * Returns the ResourceBundle of the specified Messages object or {@code null} if the specified Messages
			 * object is {@code null}.
			 * 
			 * @since 4.0
			 */
		...
		private static class CommandReflection
		{
			...
			static CommandSpec extractCommandSpec(Object command, IFactory factory, boolean annotationsAreMandatory)
			{
				...
				result.mixinStandardHelpOptions(mixinStandardHelpOptions); // #377 Standard help options should be added
																			// last
				if (command instanceof Method)
				{
					...
					result.updateCommandAttributes(cmd, factory);
					result.setAddMethodSubcommands(false); // method cli don't have method subcommands
			...
			private static boolean initFromAnnotatedFields(IScope scope, Class<?> cls, CommandSpec receiver,
					ArgGroupSpec.Builder groupBuilder, IFactory factory)
			{
				boolean result = false;
				for (Field field : cls.getDeclaredFields())
				{
					result |= initFromAnnotatedTypedMembers(TypedMember.createIfAnnotated(field, scope), receiver,
							groupBuilder, factory);
				}
			...
			@SuppressWarnings("unchecked")
			private static boolean initFromAnnotatedTypedMembers(TypedMember member, CommandSpec commandSpec,
					ArgGroupSpec.Builder groupBuilder, IFactory factory)
			{
				boolean result = false;
				if (member == null)
					...
					ArgGroupSpec.Builder groupBuilder, IFactory factory)
			{
				...
				for (int i = 0, count = method.getParameterTypes().length; i < count; i++)
				{
					...
					result |= initFromAnnotatedTypedMembers(new TypedMember(param, scope), receiver, groupBuilder,
							factory);
				}
				return result;
			}
			...
			private static void validateInjectSpec(TypedMember member)
			{
				...
				{
					throw new InitializationException(
							"@codit.CommandLine.Spec annotation is only supported on fields of type "
			...
			{
				ITypeInfo info = member.getTypeInfo();
				if (!(info.getClassName().equals(String[].class.getName()) || (info.isCollection()
						&& info.getActualGenericTypeArguments().equals(Arrays.asList(String.class.getName())))))
				{
					throw new InitializationException(
		...
		static class FieldBinding implements IGetter, ISetter
		{
			...
			private static IScope asScope(Object scope)
			{
	...
	public static class ParseResult
	{
		 ...
		 * {@linkplain GroupMatch match}. Details of the {@linkplain GroupMatchContainer matched groups} encountered on
		 * the command line can be obtained via its {@link GroupMatch#matchedSubgroups() matchedSubgroups()} method. The
		 * top-level match returned by this method contains no {@linkplain GroupMatch#matchedValues(ArgSpec) matched
		 * arguments}.
		 * </p>
		 * <p>
		 * If the returned list contains more than one {@linkplain GroupMatch match}, the user input was invalid: the
		 * maximum {@linkplain ArgGroup#multiplicity() multiplicity} of a group was exceeded, and the parser created an
		 * extra {@code match} to capture the values. Usually this results in a {@link ParameterException
		 * ParameterException} being thrown by the {@code parse} method, unless the parser is configured to
		...
		/**
		 * Returns the option with the specified short name, or {@code null} if no option with that name was matched on
		 * the command line.
		 * <p>
		 * Use {@link OptionSpec#getValue() getValue} on the returned {@code OptionSpec} to get the matched value (or
		 * values), converted to the type of the option. Alternatively, use {@link OptionSpec#stringValues()
		 * stringValues} to get the matched String values after they were {@linkplain OptionSpec#splitRegex() split}
		 * into parts, or {@link OptionSpec#originalStringValues() originalStringValues} to get the original String
		...
		/**
		 * Returns the option with the specified name, or {@code null} if no option with that name was matched on the
		 * command line.
		 * <p>
		 * Use {@link OptionSpec#getValue() getValue} on the returned {@code OptionSpec} to get the matched value (or
		 * values), converted to the type of the option. Alternatively, use {@link OptionSpec#stringValues()
		 ...
		 * @param name
		 *            used to search the matched options. May be an alias of the option name that was actually specified
		 *            on the command line. The specified name may include option name prefix characters or not.
		 */
		...
		/**
		 * Returns the first {@code PositionalParamSpec} that matched an argument at the specified position, or
		 * {@code null} if no positional parameters were matched at that position.
		 */
		...
		/**
		 * Returns all {@code PositionalParamSpec} objects that matched an argument at the specified position, or an
		 * empty list if no positional parameters were matched at that position.
		 */
		...
		/**
		 * Returns a list of matched options, in order they were matched on the command line. The returned list may
		 * contain the same {@code OptionSpec} multiple times, if the option was matched multiple times on the command
		 * line.
		 */
		...
		/**
		 * Returns a list of matched positional parameters, in order they were matched on the command line. The returned
		 * list may contain the same {@code PositionalParamSpec} multiple times, if the parameter was matched multiple
		 ...
		 * The returned list may contain an {@code OptionSpec} or {@code PositionalParamSpec} multiple times, if the
		...
		/**
		 * If {@link ParserSpec#collectErrors} is {@code true}, returns the list of exceptions that were encountered
		 * during parsing, otherwise, returns an empty list.
		 * 
		 * @since 3.2
		 */
		 ...
		 * {@linkplain OptionSpec#type() type} of the option, or the specified default value if no option with the
		 * specified name was matched.
		 */
		 ...
		 * {@linkplain OptionSpec#type() type} of the option, or the specified default value if the specified option is
		 * {@code null}.
		 */
		@SuppressWarnings("unchecked")
		...
		/**
		 * Returns the command line argument value of the positional parameter at the specified position, converted to
		 * the {@linkplain PositionalParamSpec#type() type} of the positional parameter, or the specified default value
		...
		/**
		 * Returns the command line argument value of the specified positional parameter, converted to the
		 * {@linkplain PositionalParamSpec#type() type} of the positional parameter, or the specified default value if
		 * the specified positional parameter is {@code null}.
		 */
		@SuppressWarnings("unchecked")
		...
		/**
		 * Returns the {@code ParseResult} for the subcommand of this command that was matched on the command line, or
		 * {@code null} if no subcommand was matched.
		 */
		...
		/**
		 * Returns this {@code ParseResult} as a list of {@code CommandLine} objects, one for each matched
		 * command/subcommand. For backwards compatibility with pre-3.0 methods.
		 */
		...
		public static class Builder
		{
			...
			/**
			 * Adds the specified {@code OptionSpec} or {@code PositionalParamSpec} to the list of options and
			 * parameters that were matched on the command line.
			 * 
			 * @param arg
			 *            the matched {@code OptionSpec} or {@code PositionalParamSpec}
			 * @param position
			 *            the command line position at which the {@code PositionalParamSpec} was matched. Ignored for
			 *            {@code OptionSpec}s.
			 * @return this builder for method chaining
			 */
			...
			/**
			 * Adds the specified {@code PositionalParamSpec} to the list of parameters that were matched on the command
			 * line.
			 * 
			 * @param positionalParam
			 *            the matched {@code PositionalParamSpec}
			 * @param position
			 *            the command line position at which the {@code PositionalParamSpec} was matched.
			 * @return this builder for method chaining
			 */
			...
			/**
			 * Adds all elements of the specified command line arguments stack to the list of unmatched command line
			 * arguments.
			 */
			...
			void addOriginalStringValue(ArgSpec argSpec, String value)
			{
				if (!isInitializingDefaultValues)
				{
					...
					{
						GroupMatchContainer groupMatchContainer = this.groupMatchContainer
				...
				if (!isInitializingDefaultValues)
				{
					...
					else
					{
						...
						groupMatchContainer.lastMatch().addMatchedValue(argSpec, position, typedValue,
								commandSpec.commandLine.tracer);
					}
				}
			}
			...
			void beforeMatchingGroupElement(ArgSpec argSpec) throws Exception
			{
				...
				GroupMatchContainer foundGroupMatchContainer = this.groupMatchContainer
				...
				{
					// we need to create a new match; if maxMultiplicity has been reached, we need to add a new
					// GroupMatchContainer.
					...
					tracer.info(
							"GroupMatch %s is complete: its mandatory elements are all matched. (User object: %s.) %s %s in the group, so it starts a new GroupMatch.%n",
		 ...
		 * <p>
		 * The {@code ParseResult} may have more than one {@code GroupMatchContainer} for an {@code ArgGroupSpec}, when
		 * the group was matched more often than its maximum {@linkplain ArgGroup#multiplicity() multiplicity}. This is
		 * not necessarily a problem: the parser will add a match to the parent matched group until the maximum
		 * multiplicity of the parent group is exceeded, in which case parser will add a match to the parent's parent
		 * group, etc.
		 * </p>
		 * <p>
		 * Ultimately, as long as the {@link ParseResult#getGroupMatches()} method does not return more than one match,
		 * the maximum number of elements is not exceeded.
		 * </p>
		 * 
		 * @since 4.0
		 */
		public static class GroupMatchContainer
		{
			...
			// /** Returns the {@code GroupMatchContainer} of the parent {@code ArgGroupSpec}, or {@code null} if this
			// group has no parent. */
			// public GroupMatchContainer parentContainer() { return parentContainer; }
			/**
			 * Returns the list of {@code GroupMatch} instances: {@code ArgGroupSpec}s with a multiplicity greater than
			 * one may be matched multiple times.
			 */
			...
			void addMatch(CommandLine commandLine)
			{
				...
				else
				{
					if (group != null)
					{
						tracer.info("Adding match to GroupMatchContainer %s (group=%s %s).%n", this,
								group == null ? "?" : group.id(), group == null ? "ROOT" : group.synopsisUnit());
					}
			...
			/**
			 * Returns {@code true} if no more {@code MatchedGroupMultiples} can be added to this
			 * {@code GroupMatchContainer}. Each multiple may be a complete or an incomplete match.
			 */
			...
			/**
			 * Returns {@code true} if this {@code GroupMatchContainer} has at least the minimum number of
			 * {@code MatchedGroupMultiples}. Each multiple may be a complete or an incomplete match.
			 */
			...
			/**
			 * Returns {@code true} if the minimum number of multiples has been matched for the multiplicity of this
			 * group, and each multiple has matched at least the {@linkplain GroupMatch#matchedMinElements() minimum
			 * number of elements}.
			 */
			...
			/**
			 * Returns {@code true} if the maximum number of multiples has been matched for the multiplicity of this
			 * group, and the last multiple has {@linkplain GroupMatch#matchedMaxElements() matched the maximum number
			 * of elements}, while all other multiples have matched at least the
			...
			void updateUnmatchedGroups(final ArgGroupSpec group)
			{
				...
				List<GroupMatchContainer> groupMatchContainers = findMatchContainers(group,
			...
			void validate(CommandLine commandLine)
			{
				...
				// was forced to "spill over" matches into additional GroupMatches because max multiplicity was
				// exceeded.
		...
		/**
		 * A group's {@linkplain ArgGroup#multiplicity() multiplicity} specifies how many matches of a group may appear
		 * on the command line. This class models a single "match". For example, this group: {@code (-a -b) (-a -b)}
		 * requires two matches of its arguments to fully match.
		 * 
		 * @since 4.0
		 */
		public static class GroupMatch
		{
			...
			/**
			 * Returns {@code true} if the minimum number of elements have been reached for this match: all required
			 * arguments have been matched, and for each subgroup, the
			 * {@linkplain GroupMatchContainer#matchedMinElements() minimum number of elements have been matched}.
			 */
			boolean matchedMinElements()
			{
				return matchedFully(false);
			...
			/**
			 * Returns {@code true} if the maximum number of matches has been reached for this match: all arguments
			 * (required or not) have been matched, and for each subgroup, the
			 * {@linkplain GroupMatchContainer#matchedMaxElements() maximum number of elements have been matched}.
			 */
			boolean matchedMaxElements()
			{
				return matchedFully(true);
	...
	private class Interpreter
	{
		...
		private void registerBuiltInConverters()
		{
			...
			if (!excl("java.sql.Time"))
			{
				...
				{
					BuiltIn.handle(e, "java.sql.Time", tracer);
				}
			}
			...
			if (!excl("java.sql.Connection"))
			{
				...
				{
					BuiltIn.handle(e, "java.sql.Connection", tracer);
				}
			}
			if (!excl("java.sql.Driver"))
			{
				...
				{
					BuiltIn.handle(e, "java.sql.DriverManager", tracer);
				}
			}
			if (!excl("java.sql.Timestamp"))
			{
				...
				{
					BuiltIn.handle(e, "java.sql.Timestamp", tracer);
				}
			}
			...
			if (!excl("java.time.Duration"))
			{
				...
				{
					BuiltIn.handle(e, "java.time.Duration", tracer);
				}
			}
			if (!excl("java.time.Instant"))
			{
				...
				{
					BuiltIn.handle(e, "java.time.Instant", tracer);
				}
			}
			if (!excl("java.time.LocalDate"))
			{
				...
				{
					BuiltIn.handle(e, "java.time.LocalDate", tracer);
				}
			}
			if (!excl("java.time.LocalDateTime"))
			{
				...
				{
					BuiltIn.handle(e, "java.time.LocalDateTime", tracer);
				}
			}
			if (!excl("java.time.LocalTime"))
			{
				...
				{
					BuiltIn.handle(e, "java.time.LocalTime", tracer);
				}
			}
			if (!excl("java.time.MonthDay"))
			{
				...
				{
					BuiltIn.handle(e, "java.time.MonthDay", tracer);
				}
			}
			if (!excl("java.time.OffsetDateTime"))
			{
				...
				{
					BuiltIn.handle(e, "java.time.OffsetDateTime", tracer);
				}
			}
			if (!excl("java.time.OffsetTime"))
			{
				...
				{
					BuiltIn.handle(e, "java.time.OffsetTime", tracer);
				}
			}
			if (!excl("java.time.Period"))
			{
				...
				{
					BuiltIn.handle(e, "java.time.Period", tracer);
				}
			}
			if (!excl("java.time.Year"))
			{
				...
				{
					BuiltIn.handle(e, "java.time.Year", tracer);
				}
			}
			if (!excl("java.time.YearMonth"))
			{
				...
				{
					BuiltIn.handle(e, "java.time.YearMonth", tracer);
				}
			}
			if (!excl("java.time.ZonedDateTime"))
			{
				...
				{
					BuiltIn.handle(e, "java.time.ZonedDateTime", tracer);
				}
			}
			if (!excl("java.time.ZoneId"))
			{
				...
				{
					BuiltIn.handle(e, "java.time.ZoneId", tracer);
				}
			}
			if (!excl("java.time.ZoneOffset"))
			{
				...
				{
					BuiltIn.handle(e, "java.time.ZoneOffset", tracer);
				}
			}
			...
			if (!excl("java.nio.file.Path"))
			{
				...
				{
					BuiltIn.handle(e, "java.nio.file.Path", tracer);
				}
			}
		}
		...
		{
			return BuiltIn.excluded(fqcn, tracer);
		}
		 ...
		 * @param args
		 *            the command line arguments
		 * @return a list with all cli and subcommands initialized by this method
		 * @throws ParameterException
		 *             if the specified command line arguments are invalid
		 */
		List<CommandLine> parse(String... args)
		{
			...
			{
				tracer.debug(
						"(ANSI is %s by default: systemproperty[picocli.ansi]=%s, isatty=%s, TERM=%s, OSTYPE=%s, isWindows=%s, JansiConsoleInstalled=%s, ANSICON=%s, ConEmuANSI=%s, NO_COLOR=%s, CLICOLOR=%s, CLICOLOR_FORCE=%s)%n",
		...
		private void clear()
		{
			...
			for (ArgGroupSpec group : getCommandSpec().argGroups())
			{
				clear(group);
			}
		}
		...
		private void clear(ArgSpec argSpec)
		{
			...
			} // groups do their own initialization
		}
				...
				List<Object> nowProcessing)
		{
			...
			{
				tracer.debug(
						"Initializing %s: %d options, %d positional parameters, %d required, %d groups, %d subcommands.%n",
		...
		private void validateConstraints(Stack<String> argumentStack, List<ArgSpec> required, Set<ArgSpec> matched)
		{
			...
			if (!parseResultBuilder.unmatched.isEmpty())
			{
				...
				if (tracer.isInfo())
				{
					tracer.info("Unmatched arguments: %s%n", parseResultBuilder.unmatched);
				}
			}
				...
				List<Object> nowProcessing) throws Exception
		{
			...
			while (!args.isEmpty())
			{
				...
				if (tracer.isDebug())
				{
					if (actuallyUnquoted)
					{
						tracer.debug("Processing argument '%s' (trimmed from '%s'). Remainder=%s%n", arg, originalArg,
								reverse(copy(args)));
					}
					else
					{
				...
				{
					tracer.info("Found end-of-options delimiter '%s'. Treating remainder as positional parameters.%n",
				...
				if (separatorIndex > 0)
				{
					...
					{
						tracer.warn("Both '%s' and '%s' are valid option names in %s. Using '%s'...%n", arg, key,
								getCommandName(), arg);
					}
					...
					else
					{
						if (tracer.isDebug())
						{
							tracer.debug("'%s' contains separator '%s' but '%s' is not a known option%n", arg,
									separator, key);
						}
					}
				}
				...
				else
				{
					...
					if (tracer.isDebug())
					{
						tracer.debug("No option named '%s' found. Processing as positional parameter%n", arg);
					}
				...
				boolean alreadyUnquoted, Stack<String> args) throws Exception
		{
			if (tracer.isDebug())
			{
				tracer.debug("Processing next arg as a positional parameter. Command-local position=%d. Remainder=%s%n",
						position, reverse(copy(args)));
			}
			...
			for (PositionalParamSpec positionalParam : commandSpec.positionalParameters())
			{
				...
				{ // does the positionalParam's index range contain the current position in the currently matching group
					GroupMatchContainer groupMatchContainer = parseResultBuilder.groupMatchContainer
							.findOrCreateMatchingGroup(positionalParam, commandSpec.commandLine());
					if (!indexRange.contains(localPosition) || (groupMatchContainer != null && groupMatchContainer
							.lastMatch().hasMatchedValueAtPosition(positionalParam, localPosition)))
					{
						continue;
					}
				}
				...
				if (tracer.isDebug())
				{
					tracer.debug("Position %s is in index range %s. Trying to assign args to %s, arity=%s%n",
							positionDesc(positionalParam), indexRange, positionalParam, arity);
				}
				...
				int actuallyConsumed = applyOption(positionalParam, false, LookBehind.SEPARATE, alreadyUnquoted, arity,
						argsCopy, initialized, "args[" + indexRange + "] at position " + localPosition);
				...
				if (positionalParam.group() == null)
				{ // don't update the command-level position for group args
			...
			{
				tracer.debug(
						"Consumed %d arguments and %d interactive values, moving command-local position to index %d.%n",
						argsConsumed, interactiveConsumed, position);
			}
			for (PositionalParamSpec positional : newPositions.keySet())
			{
				GroupMatchContainer inProgress = parseResultBuilder.groupMatchContainer
						.findOrCreateMatchingGroup(positional, commandSpec.commandLine());
				if (inProgress != null)
				{
					inProgress.lastMatch().position = newPositions.get(positional);
					if (tracer.isDebug())
					{
						tracer.debug("Updated group position to %s for group %s.%n", inProgress.lastMatch().position,
								inProgress);
					}
				}
			}
				...
				boolean alreadyUnquoted, Stack<String> args) throws Exception
		{
			...
			do
			{
				if (cluster.length() > 0 && commandSpec.posixOptionsMap().containsKey(cluster.charAt(0)))
				{
					...
					if (tracer.isDebug())
					{
						tracer.debug("Found option '%s%s' in %s: %s, arity=%s%n", prefix, cluster.charAt(0), arg,
								argSpec, arity);
					}
					required.remove(argSpec);
					...
					if (cluster.startsWith(config().separator()))
					{// attached with separator, like -f=FILE or -v=true
						lookBehind = LookBehind.ATTACHED_WITH_SEPARATOR;
					...
					int consumed = applyOption(argSpec, false, lookBehind, alreadyUnquoted, arity, args, initialized,
							argDescription);
					// if cluster was consumed as a parameter or if this field was the last in the cluster we're done;
				...
				{ // cluster is empty || cluster.charAt(0) is not a short option key
					if (cluster.length() == 0)
					{ // we finished parsing a group of short options like -rxv
					...
					if (arg.endsWith(cluster))
					{
						...
						{ // #149 be consistent between unmatched short and long options
							if (tracer.isDebug())
							{
								tracer.debug(
				...
				Range arity, Stack<String> args, Set<ArgSpec> initialized, String argDescription) throws Exception
		{
			...
			boolean consumeOnlyOne = commandSpec.parser().aritySatisfiedByAttachedOptionParam()
				...
				String argDescription) throws Exception
		{
			...
			Class<?> cls = argSpec.auxiliaryTypes()[0]; // field may be interface/abstract type, use annotation to get
														// concrete type
			if (arity.min <= 0)
			{ // value may be optional
				...
				if (cls == Boolean.class || cls == Boolean.TYPE)
				{
					...
					{ // if attached, try converting the value to boolean (and fail if invalid value)
						Boolean defaultValue = booleanValue(argSpec, String.valueOf(argSpec.calcDefaultValue(true))); // #712
																														// flip
																														// the
																														// default
																														// value
				...
				{ // non-boolean option with optional value #325, #279
					...
					{ // stack is empty, option with arity=0..1 was the last arg
						actualValue = fallbackValue;
						optionalValueExists = false;
						consumed = 0;
					}
			...
			if (consumed == 0)
			{ // optional value was not specified on command line, we made something up
				if (value != null)
				{
			...
			{ // value was non-optional or optional value was actually specified
			...
			if (tracer.isInfo())
			{
				tracer.info(traceMessage, argSpec.toString(), String.valueOf(oldValue), String.valueOf(newValue),
						argDescription);
			}
				...
				Stack<String> args, Set<ArgSpec> initialized, String argDescription) throws Exception
		{
			...
			{
				tracer.debug("Initializing binding for %s with empty %s%n", optionDescription("", argSpec, 0),
			...
			int pos = getPosition(argSpec);
			consumeMapArguments(argSpec, lookBehind, alreadyUnquoted, arity, args, classes, keyConverter,
					valueConverter, map, argDescription);
			parseResultBuilder.add(argSpec, pos);
			argSpec.setValue(map);
		...
		private void consumeMapArguments(ArgSpec argSpec, LookBehind lookBehind, boolean alreadyUnquoted, Range arity,
				...
				ITypeConverter<?> valueConverter, Map<Object, Object> result, String argDescription) throws Exception
		{
			...
			String fallback = consumed == 0 && argSpec.isOption()
			...
			for (int i = consumed; consumed < arity.max && !args.isEmpty(); i++)
			{
				...
				{
					break; // leave empty map at argSpec.typedValueAtPosition[currentPosition] so we won't try to
							// consume that position again
				}
				consumeOneMapArgument(argSpec, lookBehind, alreadyUnquoted, arity, consumed, args.pop(), classes,
						keyConverter, valueConverter, typedValuesAtPosition, i, argDescription);
				result.putAll(typedValuesAtPosition);
			...
			for (String value : values)
			{
				...
				if (tracer.isInfo())
				{
					tracer.info("Putting [%s : %s] in %s<%s, %s> %s for %s%n", String.valueOf(mapKey),
							String.valueOf(mapValue), result.getClass().getSimpleName(), classes[0].getSimpleName(),
		...
		{
			String raw = lookBehind.isAttached() && alreadyUnquoted ? arg : smartUnquoteIfEnabled(arg); // if attached,
																										// we already
																										// trimmed
																										// quotes once
				...
				ITypeConverter<?> valueConverter, String argDescription)
		{
			...
			{
				tracer.debug("$s cannot be assigned to %s: type conversion fails: %s.%n", arg, argDescription,
						ex.getMessage());
				return false;
			}
		}
		...
		private void assertNoMissingMandatoryParameter(ArgSpec argSpec, Stack<String> args, int i, Range arity)
		{
			if (!varargCanConsumeNextValue(argSpec, args.peek()))
			{
				...
				throw new MissingParameterException(CommandLine.this, argSpec, "Expected parameter " + desc + "for "
				...
				String argDescription) throws Exception
		{
			...
			int pos = getPosition(argSpec);
			List<Object> converted = consumeArguments(argSpec, negated, lookBehind, alreadyUnquoted, alreadyUnquoted,
					arity, args, type, argDescription);
			List<Object> newValues = new ArrayList<Object>();
			if (initialized.contains(argSpec))
			{ // existing values are default values if initialized does NOT contain argsSpec
		...
		private List<Object> consumeArguments(ArgSpec argSpec, boolean negated, LookBehind lookBehind,
				boolean alreadyUnquoted, boolean unquoted, Range arity, Stack<String> args, Class<?> type,
				String argDescription) throws Exception
		{
			List<Object> result = new ArrayList<Object>();
			...
			for (int i = 0; consumed < arity.min && !args.isEmpty(); i++)
			{
				...
				parseResultBuilder.addTypedValues(argSpec, currentPosition++, typedValuesAtPosition);
				assertNoMissingMandatoryParameter(argSpec, args, i, arity);
				consumeOneArgument(argSpec, lookBehind, alreadyUnquoted, arity, consumed, args.pop(), type,
			...
			for (int i = consumed; consumed < arity.max && !args.isEmpty(); i++)
			{
				...
				{
					// if interactive and arity = 0..1, we consume from command line if possible (if next arg not an
				...
				else
				{
					...
					List<Object> typedValuesAtPosition = new ArrayList<Object>();
					parseResultBuilder.addTypedValues(argSpec, currentPosition++, typedValuesAtPosition);
					if (!canConsumeOneArgument(argSpec, lookBehind, alreadyUnquoted, arity, consumed, args.peek(), type,
							argDescription))
					{
						break; // leave empty list at argSpec.typedValueAtPosition[currentPosition] so we won't try to
								// consume that position again
					}
					consumeOneArgument(argSpec, lookBehind, alreadyUnquoted, arity, consumed, args.pop(), type,
							typedValuesAtPosition, i, argDescription);
		...
		private int addPasswordToList(ArgSpec argSpec, Class<?> type, List<Object> result, int consumed,
				String argDescription)
		{
			...
			{
				Object value = tryConvert(argSpec, consumed, getTypeConverter(type, argSpec, consumed),
						new String(password), type);
				result.add(value);
			}
			else
			{
		...
		private int consumeOneArgument(ArgSpec argSpec, LookBehind lookBehind, boolean alreadyUnquoted, Range arity,
				int consumed, String arg, Class<?> type, List<Object> result, int index, String argDescription)
		{
			if (!lookBehind.isAttached())
			{
				parseResultBuilder.nowProcessing(argSpec, arg);
			...
			for (int j = 0; j < values.length; j++)
			{
				...
				if (tracer.isInfo())
				{
					tracer.info("Adding [%s] to %s for %s%n", String.valueOf(result.get(result.size() - 1)),
		...
		private boolean canConsumeOneArgument(ArgSpec argSpec, LookBehind lookBehind, boolean alreadyUnquoted,
				Range arity, int consumed, String arg, Class<?> type, String argDescription)
		{
			if (char[].class.equals(argSpec.auxiliaryTypes()[0]) || char[].class.equals(argSpec.type()))
			{
				return true;
			...
			try
			{
				...
				// tracer.debug("$s would split into %s values but %s cannot accept that many values.%n", arg,
				// values.length, argDescription);
				// return false;
				// }
				for (String value : values)
				{
		 ...
		 * <p>
		 * Usually, we stop if we encounter '--', a command, or another option. However, if end-of-options has been
		 * reached, positional parameters may consume all remaining arguments.
		 * </p>
		 */
		 ...
		 * Called when parsing varargs parameters for a multi-value option. When an option is encountered, the remainder
		 * should not be interpreted as vararg elements.
		 * 
		 * @param arg
		...
		private boolean isOption(String arg)
		{
			...
			{ // -v or -f or --file (not attached to param or other option)
				return true;
			}
			int separatorIndex = arg.indexOf(config().separator());
			if (separatorIndex > 0)
			{ // -f=FILE or --file==FILE (attached to param via separator)
				...
				throws ParameterException
		{
			...
			{
				String msg = String.format("Invalid value for %s: %s", optionDescription("", argSpec, index),
						ex.getMessage());
		...
		private ITypeConverter<?> getTypeConverter(final Class<?> type, ArgSpec argSpec, int index)
		{
			...
			// consider adding ParserSpec.charArraysCanCaptureStrings() to allow non-interactive options to capture
			// multi-char values in a char[] array
				...
				return new ITypeConverter<Object>()
				{
					...
					public Object convert(String value) throws Exception
					{
						...
						catch (Exception ex)
						{
							...
							throw new TypeConversionException(String.format("expected one of %s (%s) but was '%s'",
									Arrays.asList(names), sensitivity, value));
						}
					}
				};
			}
		...
		private boolean assertNoMissingParameters(ArgSpec argSpec, Range arity, Stack<String> args)
		{
			...
			if (arity.min > available)
			{
				if (arity.min == 1)
				{
					if (argSpec.isOption())
					{
						maybeThrow(new MissingParameterException(CommandLine.this, argSpec,
				...
				{
					maybeThrow(
							new MissingParameterException(CommandLine.this, argSpec, optionDescription("", argSpec, 0)
									+ " requires at least " + arity.min + " values, but none were specified."));
				}
				else
				{
					maybeThrow(new MissingParameterException(CommandLine.this, argSpec,
							optionDescription("", argSpec, 0) + " requires at least " + arity.min + " values, but only "
		...
		char[] readPassword(ArgSpec argSpec)
		{
			...
			if (tracer.isDebug())
			{
				tracer.debug("User entered %d characters for %s.%n", result.length, name);
			}
			return result;
		}
		...
		char[] readPassword(String prompt)
		{
			try
			{
				...
				Method method = Class.forName("java.io.Console").getDeclaredMethod("readPassword", String.class,
						Object[].class);
	...
	private static class BuiltIn
	{
		...
		static boolean excluded(String fqcn, Tracer tracer)
		{
			...
			for (String regex : excludes)
			{
				...
				{
					tracer.debug("BuiltIn type converter for %s is not loaded: (picocli.converters.excludes=%s)%n",
	...
	/**
	 * Help command that can be installed as a subcommand on all application cli. When invoked with a subcommand
	...
	public static final class HelpCommand implements IHelpCommandInitializable, IHelpCommandInitializable2, Runnable
	{
		...
		/**
		 * Invokes {@link #usage(PrintStream, Help.ColorScheme) usage} for the specified command, or for the parent
		 * command.
		 */
		public void run()
		{
			...
			if (commands.length > 0)
			{
				...
				{
					throw new ParameterException(parent, "Unknown subcommand '" + commands[0] + "'.", null,
							commands[0]);
				}
			}
			else
	...
	/**
	 * Help cli that provide usage help for other cli can implement this interface to be initialized with the
	 * information they need.
	 * <p>
	 * The {@link #printHelpIfRequested(List, PrintStream, PrintStream, Help.Ansi) CommandLine::printHelpIfRequested}
	 * method calls the {@link #init(CommandLine, CommandLine.Help.Ansi, PrintStream, PrintStream) init} method on cli
	 * marked as {@link Command#helpCommand() helpCommand} before the help command's {@code run} or {@code call} method
	 * is called.
	 * </p>
	 * <p>
	 * <b>Implementation note:</b>
	 * </p>
	 * <p>
	 * If an error occurs in the {@code run} or {@code call} method while processing the help request, it is recommended
	 * custom Help cli throw a {@link ParameterException ParameterException} with a reference to the parent command. The
	 * {@link DefaultExceptionHandler DefaultExceptionHandler} will print the error message and the usage for the parent
	 * command, and will terminate with the exit code of the exception handler if one was set.
	 * </p>
	 * 
	...
	{
		/**
		 * Initializes this object with the information needed to implement a help command that provides usage help for
		 * other cli.
		 * 
		 * @param helpCommandLine
		 *            the {@code CommandLine} object associated with this help command. Implementors can use this to
		 *            walk the command hierarchy and get access to the help command's parent and sibling cli.
		 * @param ansi
		 *            whether to use Ansi colors or not
		 * @param out
		 *            the stream to print the usage help message to
		 * @param err
		 *            the error stream to print any diagnostic messages to, in addition to the output from the exception
		 *            handler
		 */
		@Deprecated
	 ...
	 * information they need.
	 * <p>
	 * The {@link #executeHelpRequest(List) CommandLine::printHelpIfRequested} method calls the
	 * {@link #init(CommandLine, CommandLine.Help.ColorScheme, PrintWriter, PrintWriter) init} method on cli marked as
	 * {@link Command#helpCommand() helpCommand} before the help command's {@code run} or {@code call} method is called.
	 * </p>
	 * <p>
	 * <b>Implementation note:</b>
	 * </p>
	 ...
	 * If an error occurs in the {@code run} or {@code call} method while processing the help request, it is recommended
	 * custom Help cli throw a {@link ParameterException ParameterException} with a reference to the parent command. The
	 * {@link DefaultExceptionHandler default ParameterException handler} will print the error message and the usage for
	 * the parent command.
	 * </p>
	 * 
	 * @since 4.0
	...
	public static interface IHelpCommandInitializable2
	{
		 ...
		 *            walk the command hierarchy and get access to the help command's parent and sibling cli.
		 * @param colorScheme
		 *            the color scheme to use when printing help, including whether to use Ansi colors or not
		 * @param outWriter
		 *            the output writer to print the usage help message to
		 * @param errWriter
		 *            the error writer to print any diagnostic messages to, in addition to the output from the exception
		 *            handler
		 */
	...
	/**
	 * Renders a section of the usage help message. The usage help message can be customized: use the
	 * {@link #setHelpSectionKeys(List)} and {@link #setHelpSectionMap(Map)} to change the order of sections, delete
	 * standard sections, add custom sections or replace the renderer of a standard sections with a custom one.
	 * <p>
	 * This gives complete freedom on how a usage help message section is rendered, but it also means that the section
	 * renderer is responsible for all aspects of rendering the section, including layout and emitting ANSI escape
	 * codes. The {@link Help.TextTable} and {@link Help.Ansi.Text} classes, and the
	 * {@link CommandLine.Help.Ansi#string(String)} and {@link CommandLine.Help.Ansi#text(String)} methods may be
	 * useful.
	 * </p>
	 * 
	 * @see UsageMessageSpec
	...
	{
		/**
		 * Renders a section of the usage help, like header heading, header, synopsis heading, synopsis, description
		 * heading, description, etc.
		 * 
		 * @param help
		 *            the {@code Help} instance for which to render a section
	...
	/**
	 * A collection of methods and inner classes that provide fine-grained control over the contents and layout of the
	 * usage help message to display to end users when help is requested or invalid input values were specified.
	 ...
	 * <h2>Layered API</h2>
	 * <p>
	 * The {@link Command} annotation and the {@link UsageMessageSpec} programmatic API equivalent provide the easiest
	 * way to configure the usage help message. See the
	 * <a href="https://remkop.github.io/picocli/index.html#_usage_help">Manual</a> for details.
	 * </p>
	 * <p>
	 * This Help class provides high-level functions to create sections of the usage help message and headings for these
	 * sections. Instead of calling the {@link CommandLine#usage(PrintStream, CommandLine.Help.ColorScheme)} method,
	 * application authors may want to create a custom usage help message by reorganizing sections in a different order
	 * and/or adding custom sections.
	 * </p>
	 * <p>
	 ...
	 * <p>
	 * Renders a field annotated with {@link Option} or {@link Parameters} to an array of {@link Text} values. By
	 ...
	 * <h3>Layout</h3>
	 * <p>
	 * Delegates to the renderers to create {@link Text} values for the annotated fields, and uses a {@link TextTable}
	 * to display these values in tabular format. Layout is responsible for deciding which values to display where in
	 * the table. By default, Layout shows one option or parameter per table row.
	 * </p>
	 * <h3>TextTable</h3>
	 * <p>
	 * Responsible for spacing out {@link Text} values according to the {@link Column} definitions the table was created
	 * with. Columns have a width, indentation, and an overflow policy that decides what to do if a value is longer than
	 * the column's width.
	 * </p>
	 * <h3>Text</h3>
	 * <p>
	 * Encapsulates rich text with styles and colors in a way that other components like {@link TextTable} are unaware
	 * of the embedded ANSI escape codes.
	 * </p>
	 */
	public static class Help
	{
		 ...
		 * {@link ParserSpec#DEFAULT_SEPARATOR}.
		 */
		...
		/**
		 * Constructs a new {@code Help} instance with a default color scheme, initialized from annotatations on the
		 * specified class and superclasses.
		 * 
		 * @param command
		...
		/**
		 * Constructs a new {@code Help} instance with the specified color scheme, initialized from annotatations on the
		 * specified class and superclasses.
		 * 
		 * @param command
		 *            the annotated object to create usage help for
		 ...
		 * 
		 * @param commandSpec
		 *            the command config to create usage help for
		 * @param colorScheme
		 *            the color scheme to use
		 */
		...
		/**
		 * Returns the {@code CommandSpec} config that this Help was constructed with.
		 * 
		 * @since 3.9
		 */
		public CommandSpec commandSpec()
		...
		/**
		 * Returns the {@code ColorScheme} config that this Help was constructed with.
		 * 
		 * @since 3.0
		 */
		...
		/**
		 * Option and positional parameter value label renderer used for the synopsis line(s) and the option list. By
		 * default initialized to the result of {@link #createDefaultParamLabelRenderer()}, which takes a snapshot of
		 * the {@link ParserSpec#separator()} at construction time. If the separator is modified after Help
		 * construction, you may need to re-initialize this field by calling {@link #createDefaultParamLabelRenderer()}
		 * again.
		 */
		...
		public Help addAllSubcommands(Map<String, CommandLine> commands)
		{
			if (commands != null)
			{
				...
				// then loop over all names that the command was registered with and add this name to the front of the
				...
				// Now we loop over the cli in the order that they were registered on their parent command.
				for (Map.Entry<String, CommandLine> entry : commands.entrySet())
				{
					// not registering hidden cli is easier than suppressing display in Help.commandList():
					...
					if (!entry.getValue().getCommandSpec().usageMessage().hidden())
					{
						...
						if (aliases != null)
						{ // otherwise we already processed this command by another alias
		...
		Help addSubcommand(List<String> commandNames, CommandLine commandLine)
		{
			...
			commands.put(all.substring(1, all.length() - 1),
					getHelpFactory().create(commandLine.commandSpec, colorScheme).withCommandNames(commandNames));
			return this;
		}
		 ...
		 * @param command
		 *            the {@code CommandSpec} or {@code @Command} annotated object to get more information from
		 * @return this Help instance (for method chaining)
		 * @deprecated
		 */
		@Deprecated
		public Help addSubcommand(String commandName, Object command)
		{
			commands.put(commandName,
					getHelpFactory().create(CommandSpec.forAnnotatedObject(command, commandSpec.commandLine().factory),
		 ...
		 * 
		 * @param synopsisHeadingLength
		 *            the length of the synopsis heading that will be displayed on the same line
		 * @return a synopsis
		 * @see #abbreviatedSynopsis()
		 * @see #detailedSynopsis(Comparator, boolean)
		 * @see #synopsisHeading
		...
		/**
		 * Generates a generic synopsis like {@code <command name> [OPTIONS] [PARAM1 [PARAM2]...]}, omitting parts that
		 * don't apply to the command (e.g., does not show [OPTIONS] if the command has no options).
		 * 
		...
		{
			StringBuilder sb = new StringBuilder();
			if (!commandSpec.optionsMap().isEmpty())
			{ // only show if annotated object actually has options
				sb.append(" [OPTIONS]");
			}
			// sb.append(" [--] "); // implied
			for (PositionalParamSpec positionalParam : commandSpec.positionalParameters())
		 ...
		 * 
		 * @param optionSort
		 *            comparator to sort options or {@code null} if options should not be sorted
		 * @param clusterBooleanOptions
		 *            {@code true} if boolean short options should be clustered into a single string
		...
		/**
		 * Returns a Text object containing a partial detailed synopsis showing only the options and positional
		 * parameters in the specified {@linkplain ArgGroup#validate() validating} {@linkplain ArgGroup groups},
		 * starting with a {@code " "} space.
		 * 
		 * @param outparam_groupArgs
		 *            all options and positional parameters in the groups this method generates a synopsis for; these
		 *            options and positional parameters should be excluded from appearing elsewhere in the synopsis
		 * @return the formatted groups synopsis elements, starting with a {@code " "} space, or an empty Text if this
		 *         command has no validating groups
		 * @since 4.0
		 */
		protected Text createDetailedSynopsisGroupsText(Set<ArgSpec> outparam_groupArgs)
		{
			...
			{
				if (group.validate())
				{ // non-validating groups are not shown in the synopsis
		 ...
		 * {@code " "} space. Follows the unix convention of showing optional options and parameters in square brackets
		 * ({@code [ ]}).
		 * 
		 * @param done
		 *            the list of options and positional parameters for which a synopsis was already generated. Options
		 *            in this set should be excluded.
		 * @param optionSort
		 *            comparator to sort options or {@code null} if options should not be sorted
		 * @param clusterBooleanOptions
		 *            {@code true} if boolean short options should be clustered into a single string
		 * @return the formatted options, starting with a {@code " "} space, or an empty Text if this command has no
		 *         named options
		 * @since 3.9
		 */
				...
				if (!option.hidden())
				{
					Text name = option.negatable()
							? colorScheme.optionText(option.commandSpec.negatableOptionTransformer()
									.makeSynopsis(option.shortestName(), option.commandSpec))
							: colorScheme.optionText(option.shortestName());
					Text param = parameterLabelRenderer().renderParameterLabel(option, colorScheme.ansi(),
					...
					else
					{
						optionText = optionText.concat(" [").concat(name).concat(param).concat("]");
						if (option.isMultiValue())
						{ // add ellipsis to show option is repeatable
		...
		/**
		 * Returns a Text object containing a partial detailed synopsis showing only the positional parameters, starting
		 * with a {@code " "} space. Follows the unix convention of showing optional options and parameters in square
		 * brackets ({@code [ ]}).
		 * 
		 * @param done
		 *            the list of options and positional parameters for which a synopsis was already generated.
		 *            Positional parameters in this set should be excluded.
		 * @return the formatted positional parameters, starting with a {@code " "} space, or an empty Text if this
		...
		protected Text createDetailedSynopsisPositionalsText(Collection<ArgSpec> done)
		{
			...
			for (PositionalParamSpec positionalParam : positionals)
			{
				if (!positionalParam.hidden())
				{
					...
					Text label = parameterLabelRenderer().renderParameterLabel(positionalParam, colorScheme.ansi(),
							colorScheme.parameterStyles);
		 ...
		 * {@code " "} space. Follows the unix convention of showing optional elements in square brackets ({@code [ ]}).
		 * 
		 * @return this implementation returns " " + {@link UsageMessageSpec#synopsisSubcommandLabel()} if this command
		...
		/**
		 * Returns the detailed synopsis text by inserting the command name before the specified text with options and
		 ...
		 * @param optionsAndPositionalsAndCommandsDetails
		 *            formatted string with options, positional parameters and subcommands. Follows the unix convention
		 *            of showing optional options and parameters in square brackets ({@code [ ]}).
		...
		{
			String[] lines = Ansi.OFF.new Text(commandSpec.usageMessage().synopsisHeading()).toString()
					.split("\\r?\\n|\\r|%n", -1);
		...
		/**
		 * <p>
		 * Returns a description of the {@linkplain Option options} supported by the application. This implementation
		 * {@linkplain #createShortOptionNameComparator() sorts options alphabetically}, and shows only the
		 * {@linkplain Option#hidden() non-hidden} options in a {@linkplain TextTable tabular format} using the
		 * {@linkplain #createDefaultOptionRenderer() default renderer} and {@linkplain Layout default layout}.
		 * </p>
		 * 
		...
		/**
		 * Sorts all {@code Options} with the specified {@code comparator} (if the comparator is non-{@code null}), then
		 * {@linkplain Layout#addOption(CommandLine.Model.OptionSpec, CommandLine.Help.IParamLabelRenderer) adds} all
		 * non-hidden options to the specified TextTable and returns the result of TextTable.toString().
		 * 
		 * @param layout
		 *            responsible for rendering the option list
		 * @param valueLabelRenderer
		 *            used for options with a parameter
		 * @return the fully formatted option list
		 * @since 3.0
		 */
		...
		{
			List<OptionSpec> options = new ArrayList<OptionSpec>(commandSpec.options()); // options are stored in order
																							// of declaration
			if (optionSort != null)
			{
		 ...
		 * @param adjustCJK
		 *            true if wide Chinese, Japanese and Korean characters should be counted as double the size of other
		 *            characters for line-breaking purposes
		 * @param values
		 ...
		 * specified declaratively with the {@link Command#customSynopsis()} annotation attribute or programmatically by
		 * setting the Help instance's {@link Help#customSynopsis} field.
		 * 
		 * @param params
		 *            Arguments referenced by the format specifiers in the synopsis strings
		...
		/**
		 * Returns command description text as a string. Description text can be zero or more lines, and can be
		 * specified declaratively with the {@link Command#description()} annotation attribute or programmatically by
		 * setting the Help instance's {@link Help#description} field.
		 * 
		 * @param params
		 *            Arguments referenced by the format specifiers in the description strings
		 ...
		 * declaratively with the {@link Command#header()} annotation attribute or programmatically by setting the Help
		 * instance's {@link Help#header} field.
		 * 
		 * @param params
		 *            Arguments referenced by the format specifiers in the header strings
		 ...
		 * declaratively with the {@link Command#footer()} annotation attribute or programmatically by setting the Help
		 * instance's {@link Help#footer} field.
		 * 
		 * @param params
		 *            Arguments referenced by the format specifiers in the footer strings
		 ...
		 * 
		 * @param params
		 *            the parameters to use to format the header heading
		 ...
		 * 
		 * @param params
		 *            the parameters to use to format the synopsis heading
		...
		/**
		 * Returns the text displayed before the description text; an empty string if there is no description, otherwise
		 * the result of {@code String.format(descriptionHeading, params)}.
		 * 
		 * @param params
		 *            the parameters to use to format the description heading
		...
		public String descriptionHeading(Object... params)
		{
			return empty(commandSpec.usageMessage().descriptionHeading()) ? ""
					: heading(ansi(), width(), adjustCJK(), commandSpec.usageMessage().descriptionHeading(), params);
		}
		 ...
		 * 
		 * @param params
		 *            the parameters to use to format the parameter list heading
		 * @return the formatted parameter list heading
		 */
		public String parameterListHeading(Object... params)
		{
			return commandSpec.positionalParameters().isEmpty() ? ""
					: heading(ansi(), width(), adjustCJK(), commandSpec.usageMessage().parameterListHeading(), params);
		}

		/**
		 * Returns the text displayed before the option list; an empty string if there are no options, otherwise the
		 * result of {@code String.format(optionListHeading, params)}.
		 * 
		 * @param params
		 *            the parameters to use to format the option list heading
		 * @return the formatted option list heading
		 */
		public String optionListHeading(Object... params)
		{
			return commandSpec.optionsMap().isEmpty() ? ""
					: heading(ansi(), width(), adjustCJK(), commandSpec.usageMessage().optionListHeading(), params);
		}

		/**
		 * Returns the text displayed before the command list; an empty string if there are no cli, otherwise the result
		 * of {@code String.format(commandListHeading, params)}.
		 * 
		 * @param params
		 *            the parameters to use to format the command list heading
		 * @return the formatted command list heading
		 */
		public String commandListHeading(Object... params)
		{
			return commands.isEmpty() ? ""
					: heading(ansi(), width(), adjustCJK(), commandSpec.usageMessage().commandListHeading(), params);
		}
		 ...
		 * 
		 * @param params
		 *            the parameters to use to format the footer heading
		 ...
		 * 
		 * @param params
		 *            the parameters to use to format the exit code heading
		...
		/**
		 * Returns a 2-column list with exit codes and their description. Descriptions containing {@code "%n"} line
		 * separators are broken up into multiple lines.
		 * 
		 ...
		 * 
		 * @return a usage help section describing the added cli
		 */
		...
		/**
		 * Returns a {@code Text} object containing the command name and all aliases, separated with the specified
		 * separator. Command names will use the {@link ColorScheme#commandText(String) command style} for the color
		 * scheme of this Help.
		 * 
		 * @since 3.9
		 */
		...
		/**
		 * Returns a new default OptionRenderer which converts {@link OptionSpec Options} to five columns of text to
		 * match the default {@linkplain TextTable TextTable} column layout. The first row of values looks like this:
		 * <ol>
		...
		/**
		 * Returns a new minimal OptionRenderer which converts {@link OptionSpec Options} to a single row with two
		 * columns of text: an option name and a description. If multiple names or descriptions exist, the first value
		 * is used.
		 * 
		...
		/**
		 * Returns a new default ParameterRenderer which converts {@linkplain PositionalParamSpec positional parameters}
		 * to four columns of text to match the default {@linkplain TextTable TextTable} column layout. The first row of
		 ...
		 * to a single row with two columns of text: an option name and a description. If multiple descriptions exist,
		...
		/**
		 * Returns a new default param label renderer that separates option parameters from their option name with the
		 * specified separator string, and, unless {@link ArgSpec#hideParamSyntax()} is true, surrounds optional
		 * parameters with {@code '['} and {@code ']'} characters and uses ellipses ("...") to indicate that any number
		...
		/**
		 * Sorts {@link OptionSpec OptionSpecs} by their option name in case-insensitive alphabetic order. If an option
		 * has multiple names, the shortest name is used for the sorting. Help options follow non-help options.
		 * 
		 ...
		 * {@linkplain Range#min min arity} next, and by {@linkplain #createShortOptionNameComparator() option name}
		 * last.
		 * 
		...
		/**
		 * Sorts {@link OptionSpec options} by their option {@linkplain IOrdered#order() order}, lowest first, highest
		 * last.
		 * 
		 ...
		 * used to create textual representation of an Option in a tabular format: one or more rows, each containing one
		 * or more columns. The {@link Layout Layout} is responsible for placing these text values in the
		...
		/**
		 * The MinimalParameterRenderer converts {@linkplain PositionalParamSpec positional parameters} to a single row
		 * with two columns of text: the parameters label and a description. If multiple description lines exist, the
		 * first value is used.
		 */
		...
		/**
		 * When customizing online help for {@linkplain PositionalParamSpec positional parameters} details, a custom
		 * {@code IParameterRenderer} can be used to create textual representation of a Parameters field in a tabular
		 * format: one or more rows, each containing one or more columns. The {@link Layout Layout} is responsible for
		 * placing these text values in the {@link TextTable TextTable}.
		 */
		...
		/**
		 * The DefaultParameterRenderer converts {@linkplain PositionalParamSpec positional parameters} to five columns
		 * of text to match the default {@linkplain TextTable TextTable} column layout. The first row of values looks
		 * like this:
		 * <ol>
		...
		private static void addTrailingDefaultLine(List<Text[]> result, ArgSpec arg, ColorScheme scheme)
		{
			...
			result.add(new Text[] { EMPTY, EMPTY, EMPTY, EMPTY,
					scheme.ansi().new Text("  Default: " + arg.defaultValueString(true)) });
		}
		 ...
		 * {@link ArgSpec#hideParamSyntax()} is true, surrounds optional values with {@code '['} and {@code ']'}
		 * characters and uses ellipses ("...") to indicate that any number of values is allowed for options or
		 * parameters with variable arity.
		 */
		 ...
		 * </p>
		 * <p>
		 * Customize by overriding the {@link #layout(CommandLine.Model.ArgSpec, CommandLine.Help.Ansi.Text[][])}
		 * method.
		 * </p>
		 * 
		...
		public static class Layout
		{
			...
			/**
			 * Constructs a Layout with the specified color scheme, the specified TextTable, the specified option
			 * renderer and the specified parameter renderer.
			 * 
			 * @param colorScheme
			 *            the color scheme to use for common, auto-generated parts of the usage help message
			...
			/**
			 * Calls {@link #addOption(CommandLine.Model.OptionSpec, CommandLine.Help.IParamLabelRenderer)} for all
			...
			/**
			 * Delegates to the {@link #optionRenderer option renderer} of this layout to obtain text values for the
			 * specified {@link OptionSpec}, and then calls the
			 * {@link #layout(CommandLine.Model.ArgSpec, CommandLine.Help.Ansi.Text[][])} method to write these text
			...
			/**
			 * Delegates to the {@link #parameterRenderer parameter renderer} of this layout to obtain text values for
			 * the specified {@linkplain PositionalParamSpec positional parameter}, and then calls
			 * {@link #layout(CommandLine.Model.ArgSpec, CommandLine.Help.Ansi.Text[][])} to write these text values
		...
		/**
		 * Sorts {@code OptionSpec} instances by their max arity first, then their min arity, then delegates to super
		 * class.
		 */
		...
		public static class TextTable
		{
			...
			/**
			 * Delegates to {@link #addRowValues(CommandLine.Help.Ansi.Text...)}, after ensuring that multi-line values
			 * are layed out in the correct row and column.
			 * 
			 * @param values
			 ...
			 * {@link TextTable#putValue(int, int, CommandLine.Help.Ansi.Text) putValue} for each of the specified
			 * values, adding more empty rows if the return value indicates that the value spanned multiple columns or
			 ...
			 * @throws IllegalArgumentException
			 *             if the specified row exceeds the table's {@linkplain TextTable#rowCount() row count}
			...
			private int copy(BreakIterator line, Text text, Text columnValue, int offset)
			{
				...
				for (int start = line.first(), end = line.next(); end != BreakIterator.DONE; start = end, end = line
						.next())
				{
		...
		public static class Column
		{
			...
			/**
			 * Policy for handling text that is longer than the column width: span multiple columns, wrap to the next
			 * row, or simply truncate the portion that doesn't fit.
			 */
		 ...
		 * parts of a usage message: the command name, options, positional parameters and option parameters. Users may
		 * customize these styles by creating Help with a custom color scheme.
		 * <p>
		...
		public static class ColorScheme
		{
			 ...
			 * @see Help#defaultColorScheme(Ansi)
			 * @param builder
			 *            contains the color scheme attributes to use
			 */
			...
			/**
			 * Returns the registered styles for cli in this color scheme.
			 * 
			 * @since 4.0
			 */
			...
			public static class Builder
			{
				...
				/** Returns the registered styles for cli in this color scheme builder. */
				public List<IStyle> commandStyles()
				{
					return commandStyles;
				}
				...
				/**
				 * Adds the specified styles to the registered styles for cli in this color scheme builder and returns
				 * this builder.
				 * 
				 * @param styles
				 *            the styles to add to the registered styles for cli in this color scheme builder
				...
				/**
				 * Adds the specified styles to the registered styles for options in this color scheme and returns this
				 * color scheme.
				 * 
				 * @param styles
				 *            the styles to add to registered the styles for options in this color scheme builder
				 * @return this color scheme builder to enable method chaining for a more fluent API
				 */
				...
				/**
				 * Adds the specified styles to the registered styles for positional parameters in this color scheme
				 * builder and returns this builder.
				 * 
				 * @param styles
				 *            the styles to add to registered the styles for parameters in this color scheme builder
				 * @return this color scheme builder to enable method chaining for a more fluent API
				 */
				...
				/**
				 * Adds the specified styles to the registered styles for option parameters in this color scheme builder
				 * and returns this builder.
				 * 
				 * @param styles
				 *            the styles to add to the registered styles for option parameters in this color scheme
				 *            builder
				 * @return this color scheme builder to enable method chaining for a more fluent API
				 */
				...
				/**
				 * Replaces colors and styles in this scheme builder with ones specified in system properties, and
				 * returns this builder. Supported property names:
				 * <ul>
				 * <li>{@code picocli.color.cli}</li>
				...
				{
					replace(commandStyles, System.getProperty("picocli.color.cli"));
		...
		/**
		 * Creates and returns a new {@link ColorScheme} initialized with picocli default values: cli are bold, options
		 * and parameters use a yellow foreground, and option parameters use italic.
		 * 
		 * @param ansi
		...
		public static ColorScheme defaultColorScheme(Ansi ansi)
		{
			return new ColorScheme.Builder(ansi).commands(Style.bold).options(Style.fg_yellow)
					.parameters(Style.fg_yellow).optionParams(Style.italic).build();
		}
		...
		{
			/**
			 * Only emit ANSI escape codes if the platform supports it and system property {@code "picocli.ansi"} is not
			 * set to any value other than {@code "true"} (case insensitive).
			 */
			AUTO,
			 ...
			 * 
			 * @return ON: {@code true}, OFF: {@code false}, AUTO: if system property {@code "picocli.ansi"} is defined
			 *         then return its boolean value, otherwise return whether the platform supports ANSI escape codes
			 */
			...
			/**
			 * Returns a new Text object for this Ansi mode, encapsulating the specified string which may contain markup
			 * like {@code @|bg(red),white,underline some text|@}.
			 * <p>
			 * Calling {@code toString()} on the returned Text will either include ANSI escape codes (if this Ansi mode
			 * is ON), or suppress ANSI escape codes (if this Ansi mode is OFF).
			 * <p>
			...
			/**
			 * Returns a String where any markup like {@code @|bg(red),white,underline some text|@} is converted to ANSI
			 * escape codes if this Ansi is ON, or suppressed if this Ansi is OFF.
			 * <p>
			 ...
			 * text with embedded markup style names, as well as convenience methods for converting styles to strings
			 * with embedded escape codes.
			 */
			public enum Style implements IStyle
			{
				reset(0, 0), bold(1, 21), faint(2, 22), italic(3, 23), underline(4, 24), blink(5, 25), reverse(7,
						27), fg_black(30, 39), fg_red(31, 39), fg_green(32, 39), fg_yellow(33, 39), fg_blue(34,
								39), fg_magenta(35, 39), fg_cyan(36, 39), fg_white(37, 39), bg_black(40, 49), bg_red(41,
										49), bg_green(42, 49), bg_yellow(43, 49), bg_blue(44,
												49), bg_magenta(45, 49), bg_cyan(46, 49), bg_white(47, 49),;
				...
				/**
				 * Parses the specified style markup and returns the associated style. The markup may be one of the
				 * Style enum value names, or it may be one of the Style enum value names when {@code "fg_"} is
				 * prepended, or it may be one of the indexed colors in the 256 color palette.
				 * 
				 * @param str
				 *            the case-insensitive style markup to convert, e.g. {@code "blue"} or {@code "fg_blue"}, or
				 *            {@code "46"} (indexed color) or {@code "0;5;0"} (RGB components of an indexed color)
				...
				/**
				 * Parses the specified style markup and returns the associated style. The markup may be one of the
				 * Style enum value names, or it may be one of the Style enum value names when {@code "bg_"} is
				 * prepended, or it may be one of the indexed colors in the 256 color palette.
				 * 
				 * @param str
				 *            the case-insensitive style markup to convert, e.g. {@code "blue"} or {@code "bg_blue"}, or
				 *            {@code "46"} (indexed color) or {@code "0;5;0"} (RGB components of an indexed color)
				 * @return the IStyle for the specified converter
				 */
				 ...
				 * For each markup, strings starting with {@code "bg("} are delegated to {@link #bg(String)}, others are
				 * delegated to {@link #bg(String)}.
				 * 
			...
			/**
			 * Defines a palette map of 216 colors: 6 * 6 * 6 cube (216 colors): 16 + 36 * r + 6 * g + b (0 &lt;= r, g,
			 * b &lt;= 5).
			 */
			...
			/**
			 * Returns a new Text object where all the specified styles are applied to the full length of the specified
			 * plain text.
			 * 
			 * @param plainText
			...
			public class Text implements Cloneable
			{
				...
				/**
				 * Returns a copy of this {@code Text} instance with the specified text concatenated to the end. Does
				...
				/**
				 * Returns the number of columns that the specified portion of this Text will occupy on the console,
				 * adjusted for wide CJK characters.
				 * 
				 * @param fromPosition
				 *            the position to start counting
				 * @param charCount
				 *            the number of characters in this Text to consider
				 * @return the number of columns that the specified portion of this Text will occupy on the console,
				 *         adjusted for wide CJK characters
				 * @since 4.0
				 */
	...
	/**
	 * Uses cosine similarity to find matches from a candidate set for a specified input. Based on code from
	...
	/**
	 * Base class of all exceptions thrown by {@code codit.CommandLine}.
	...
	/**
	 * Exception indicating a problem while invoking a command or subcommand. Keeps a reference to the
	 * {@code CommandLine} object where the cause exception occurred, so that client code can tailor their handling for
	 * the specific command (print the command's usage help message, for example).
	 * 
	 * @since 2.0
	 */
		...
		private static MissingParameterException create(CommandLine cmd, Collection<ArgSpec> missing, String separator)
		{
			if (missing.size() == 1)
			{
				return new MissingParameterException(cmd, missing,
						"Missing required option '" + ArgSpec.describe(missing.iterator().next(), separator) + "'");
			}
	...
	public static class UnmatchedArgumentException extends ParameterException
	{
		...
		{
			this(commandLine,
					describe(Assert.notNull(args, "unmatched list"), commandLine) + ": " + quoteElements(args));
			unmatched = new ArrayList<String>(args);
		}

		/**
		 * Returns {@code true} and prints suggested solutions to the specified stream if such solutions exist,
		...
		/**
		 * Returns {@code true} and prints suggested solutions to the specified writer if such solutions exist,
		 * otherwise returns {@code false}.
		 * 
		 * @since 4.0
		 */
		...
		/**
		 * Returns {@code true} if the first unmatched command line arguments resembles an option, {@code false}
		 * otherwise.
		 * 
		 * @since 3.3.0
		 */
		...
		{
			return printSuggestions(new PrintWriter(out, true));
		}
		...
		private static String describe(List<String> unmatch, CommandLine cmd)
		{
			...
			return isUnknownOption(unmatch, cmd) ? "Unknown option" + plural
					: "Unmatched argument" + plural + at + " index " + (cmd.interpreter.parseResultBuilder == null ? "0"
							: cmd.interpreter.parseResultBuilder.originalArgList.indexOf(unmatch.get(0)));
		}
	...
	/**
	 * Exception indicating that more values were specified for an option or parameter than its {@link Option#arity()
	 * arity} allows.
	 */
	...
	/**
	 * Exception indicating that an option for a single-value option field has been specified multiple times on the
	 * command line.
	 */
